...
**Architecture triangle (Proposition 22):** Recognition (banned), tracking (banned), dynamics (current frontier). Post-ban, the substrate lives at the dynamics vertex. Action selection at the dynamics vertex is exhausted within the 916 architecture. The Hebbian RNN proved action selection CAN work differently (1/10 seeds), but the 6-experiment kill cascade proves it cannot be made robust within this family. The next families (ESN, population) test whether the action bottleneck is family-specific or universal.
