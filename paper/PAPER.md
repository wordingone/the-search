...
**Connection to Step 776 (R3 counterfactual FAIL):** The graph stored WHERE the substrate has been (visit counts), producing negative transfer when the destination changed. A forward model stores HOW things change (transition dynamics), which could generalize across environments — the same physics applies in different rooms. Whether a forward model's state produces positive R3 counterfactual (warm > cold) is the critical open question for post-ban work.
