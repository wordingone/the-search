...
22. **Post-ban action selection (PB4).** No known R1-compliant mechanism selects actions effectively without per-(state, action) tracking. Candidates: prediction-contrast, compression progress, empowerment maximization, action cycling. Steps 778-888 test systematically.
23. **D-only substrate feasibility.** Proposition 20 predicts D(s)-only substrates could achieve positive R3 counterfactual. No D-only substrate has been built or tested. Steps 778-806 are first contact.
24. **Noisy TV under post-ban regime.** Prediction-error exploration (the natural replacement for visit-count exploration) is vulnerable to the noisy TV problem (Burda et al. 2018). Whether compression progress (Schmidhuber 1991) — rewarding prediction IMPROVEMENT, not prediction error — resolves this in our framework is untested.
