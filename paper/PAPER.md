...
**First post-ban navigation mechanism (Step 800b, confirmed):** 80% argmax of global per-action observation change EMA + 20% random. Cold start (no pretraining): 237-377 L1/seed across 4 substrate seeds (6.5-10× random baseline of 36.4/seed). Robust. Mechanism: learn which action produces the most observation change, use it 80% of the time. On LS20, this converges to the most productive movement direction. R3_cf (warm vs cold): inconsistent (2/4 seeds pass, 2/4 warm hurts). **This is a navigation finding, not a transfer finding.** The mechanism works cold — pretraining is not needed and may hurt. The mechanism is LS20-specific: on FT09, global per-action change is uniform across all 68 actions (Step 800), so the heuristic degenerates to random.
