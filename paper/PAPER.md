...
19. **Interpreter entailment (Proposition 14).** The abstract interpreter is entailed by R1-R6. The concrete interpreter is not. The computability question is resolved: Mossio & Longo (2009) showed closure to efficient causation IS computable in reflexive domains. The remaining question is expressiveness: can a specific interpreter + state space combination encode enough operations to discover its own implementations? Rosen's β-map must be postulated (Letelier et al. 2006); our interpreter is analogously postulated. Whether the constructive gap is bridgeable — transitioning from externally imposed constraints (R1-R6) to internally maintained constraint closure (Montévil & Mossio 2015) — connects to the ℓ_1 → ℓ_F bridge and remains the central open problem.
