#!/usr/bin/env python3
"""
Step 357 -- Sampled thresh + local-sim epsilon + original spawn/attract.

No always-attract (Step 356's over-compression removed).
Original: spawn when target class sim < thresh, attract otherwise.
Sampled thresh: 500 random entries, O(500*n).
Thresh only on spawn.
Local-sim epsilon: P(random) = sims.max().

2000 steps on LS20. Kill: cb > 500, sim range > 0.2.
Script: scripts/run_step357_no_overcompress.py
"""
...
class CompressedFold:
    ...
    def process(self, x, label=None):
        ...
        # Local-sim epsilon: P(random) = nearest_sim
        ...
        # Original spawn/attract: spawn when TARGET CLASS sim < thresh
        ...
        if target_mask.sum() == 0 or sims[target_mask].max() < self.thresh:
            ...
            # Attract nearest in target class
...
def main():
    t0 = time.time()
    print("Step 357 -- sampled thresh + local-sim epsilon + original spawn/attract", flush=True)
    print(f"Device: {DEVICE}", flush=True)
    print("No always-attract. Original: spawn when target sim < thresh.", flush=True)
    print(flush=True)
    ...
    print(flush=True)
    print("=" * 60, flush=True)
    print("STEP 357 SUMMARY", flush=True)
    print("=" * 60, flush=True)
    ...
    print("Per-100-step bins (avg_nearest_sim, entropy, cb, thresh):", flush=True)
    ...
    if len(r['bins_data']) >= 3:
        ...
        print(f"cb_final={r['cb_final']} ({'PASS' if r['cb_final'] > 500 else 'FAIL'}: need >500)", flush=True)
        print(f"sim delta={'PASS' if sim_range > 0.2 else 'FAIL'}: need >0.2 (got {sim_range:.4f})", flush=True)
