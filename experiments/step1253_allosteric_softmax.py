"""
Step 1253 — Allosteric Substrate + Adaptive Softmax (one change from Step 1252)

R3 hypothesis: Same as Step 1252. Real test: can adaptive softmax selection recover
coverage (I3/L1) that GPR argmax killed in 1252, while preserving temporal structure (I4)?

Core change from Step 1252: Replace GPR argmax with adaptive-temperature softmax.

Selection (was GPR argmax, now):
  salience = W @ centered_obs
  T = 1.0 / max(std(salience), 1e-8)   # temperature from W's own spread
  probs = softmax(|salience| / T)        # action probabilities
  action = sample(probs)                 # stochastic, not deterministic

Explore/exploit transition emerges from W:
  - Early (W small): std(salience) ≈ low → T high → near-uniform → exploration
  - Later (W develops structure): std(salience) grows → T drops → focused → exploitation

No new frozen parameter vs 1252 (1e-8 is numerical stability only).

Step 1252 finding: I4=1.00 (temporal structure) but I3↓ and VC33 L1 dropped (GPR argmax
locked onto one action, killed coverage). This experiment tests: do I4 and I3 coexist
under softmax? If yes → the trade-off was argmax-specific, not principle-specific.

Design notes:
- 200-step bootstrap unchanged (LPL warmup).
- W persists across level transitions (trajectory context).
- Games: ls20, vc33, sp80. 5 draws each vs ControlC. 30 runs total.
"""
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), 'results', 'allosteric_softmax_experiment')
...
class AlloSubstrate:
    ...
    def process(self, obs_raw) -> int:
        ...
        if self.step < N_BOOTSTRAP:
            ...
            # Adaptive-temperature softmax selection
            s_abs = np.abs(enc)
            T = 1.0 / max(float(np.std(enc)), 1e-8)   # temperature from W's spread
            logits = s_abs * T
...
def main():
    ...
    print("=" * 70)
    print("STEP 1253 — ALLOSTERIC + ADAPTIVE SOFTMAX (one change from 1252)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"Selection: adaptive-temperature softmax(|enc| / T) where T=1/std(enc)")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1253 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}")
    print()
    ...
    # Save
    out_path = os.path.join(RESULTS_DIR, 'step1253_results.json')
