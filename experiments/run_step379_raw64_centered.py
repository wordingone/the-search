#!/usr/bin/env python3
"""
Step 379 -- Raw 64x64 + centering. LS20.

frame.flatten() / 15.0 -> normalize -> center by codebook mean.
Centering should remove timer's constant contribution.
2K steps. Compare sim variance to Step 377.
Script: scripts/run_step379_raw64_centered.py
"""
...
class CF:
    ...
    def pn(self,x,nc):
        ...
        # Center by codebook mean before similarity
        if self.V.shape[0]>2:
            mean_V=self.V.mean(dim=0)
            cx=F.normalize(x-mean_V,dim=0)
            cV=F.normalize(self.V-mean_V.unsqueeze(0),dim=1)
            si=cV@cx
        else:
            si=self.V@x
...
def main():
    t0=time.time()
    print(f"Step 379 -- Raw 64x64 + centering on LS20. {D} dims.",flush=True)
    ...
    print("STEP 379 SUMMARY",flush=True); print("="*60,flush=True)
