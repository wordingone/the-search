"""
Step 572u — Energy boost detour: route through (34,5) to pick up iri sprite.

Root cause of 572t L3=0 (deaths=~1900/run, lhs_ap=0):
  SPAWN(9,45)→GIC(29,45) path is 20 steps, NOT 4.
  Wall column at x=14 (y=10-35) blocks direct east; agent must go north to y=5
  then east, then south: 8N+4E+8S = 20 steps.
  Full sequence: 20(SPAWN→GIC) + 13(GIC→KDY) + 4(KDY bounce×2) + 9(KDY→LHS)
  = 46 steps > 42 life budget → agent ALWAYS dies before completing.

Fix: route through energy boost at grid (34,5) [iri sprite at pixel (35,6)].
  New sequence: SPAWN→(34,5)[boost, 13 steps]→GIC[9 steps]→KDY[13]→bounce×2[4]→LHS[9]
  Total: 48 steps with boost restoring energy at step 13.
  Energy at step 13 (before boost): 42-12=30. Boost: rzt(42)→42.
  Energy at LHS: 42 - 9(boost→GIC) - 13(GIC→KDY) - 4(bounce) - 9(KDY→LHS) = 7 ✓

Chain: 572l→572m→572n→572o→572p→572q→572r→572s→572t→572u
  572o: level persistence confirmed
  572p: persistent level fix (prev_cl=-1 sentinel)
  572q: positional tuv detection (replace broken kdj diff for puq)
  572r: death flash detection + DR reset
  572s: no-fallback-gap waypoint navigator (loop until action found)
  572t: xhp-step DR desync fix
  572u: energy boost detour — route through (34,5) to survive 48-step sequence
"""
import numpy as np
import time
import sys
...
PUQ_SPAWN = (9, 45)
PUQ_BOOST1 = (34, 5)   # grid cell of iri sprite at pixel (35,6) — restores full energy
PUQ_GIC_GRID = (29, 45)
PUQ_KDY_GRID = (49, 10)
PUQ_KDY_BOUNCE = (49, 5)
PUQ_LHS_GRID = (54, 50)
...
PUQ_WAYPOINTS = [
    PUQ_BOOST1,      # 0: energy boost → restores energy to 42
    PUQ_GIC_GRID,    # 1: gic touch → tmx=1
    PUQ_KDY_GRID,    # 2: bgt touch → tuv=1
    PUQ_KDY_BOUNCE,  # 3: bounce
    PUQ_KDY_GRID,    # 4: bgt touch → tuv=2
    PUQ_KDY_BOUNCE,  # 5: bounce
    PUQ_KDY_GRID,    # 6: bgt touch → tuv=3
    PUQ_LHS_GRID,    # 7: mae touch → win
]
PUQ_WP_AVOID = {
    1: PUQ_GIC_GRID,   # after GIC touch, avoid revisiting it
    6: PUQ_KDY_GRID,   # after 3rd KDY touch, avoid it on path to LHS
}
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self._puq_xhp_step = False     # True: next action is wasted (xhp handler)
    ...
    def _on_puq_death(self):
        ...
        self._puq_bfs_path = []
        self._puq_avoid_set = set()
        self._puq_xhp_step = True     # mark: next env.step() won't move player
    ...
    def _puq_dr_action(self):
        ...
        for _ in range(len(PUQ_WAYPOINTS) + 2):
            ...
            if wp == len(PUQ_WAYPOINTS) - 1:
                self._puq_lhs_ap += 1
                return None  # at LHS, win should have already fired
    ...
    def act(self):
        ...
        if self.game_level == 1:
            ...
            # xhp step: game will not process player movement (egb flash clearing)
            # Don't advance DR — both DR and player are at spawn, keep them synced
            if self._puq_xhp_step:
                self._puq_xhp_step = False
                return 0  # dummy action, game ignores it during xhp handler
...
def main():
    ...
    for r in R:
        print(f"  s{r['seed']}: L2={r['l2c']} L3={r['l3c']} wall={r['puq_wall']} "
              ...
              f"wp={r['wp_idx']} L3@{r['l3_steps'][:3] if r['l3_steps'] else []}", flush=True)
