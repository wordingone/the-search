#!/usr/bin/env python3
"""
STAGE 3 EXPERIMENT A: Self-Referential Eta

Eta[i,k] becomes per-cell adaptive state, updated by the SAME
plasticity rule as alpha, using the SAME resp_z signal.

Meta-rate = eta itself (multiplicative self-modification):
  push_e = eta[i,k] * f(resp_z)

Frozen frame accounting:
  Removed: eta as global scalar (was 0.0003)
  Added:   0 new elements (reuses all existing constants)
  Net:     7 -> 6 = -1

ADVERSARIAL WARNING: Multiplicative self-reference creates
exponential growth/decay. Watch for bimodal eta at clip bounds.
"""
...
class Organism:
    """Stage 3-A: Self-referential adaptive eta."""
    ...
    def __init__(self, seed=42, alive=False, eta=0.0003, adaptive_eta=False):
        ...
        if adaptive_eta:
            # Per-cell eta, initialized from same range as control
            self.eta = [
        ...
        # Alpha clip bounds: [0.3, 1.8], ratio = 1.8/0.3 = 6.0
        # Eta clip bounds: share same ratio from init center
        # Init center ~ 0.0005, range factor 6x -> [0.00005, 0.003]
        self.eta_lo = 0.00005
        self.eta_hi = 0.003
        ...
        if self.alive and signal:
            ...
            for i in range(NC):
                for k in range(D):
                    ...
                    # ── Eta plasticity (self-referential) ───────
                    # Same rule structure: eta updates itself using
                    # the same resp_z signal and same branch logic.
                    # The "column mean" for eta is computed across
                    # cells in the same dimension, just like alpha.
                    if self.adaptive_eta:
                        eta_col_mean = sum(
                            self.eta[j][k] for j in range(NC)) / NC
                        eta_dev = self.eta[i][k] - eta_col_mean

                        # Meta-rate IS eta itself (self-referential)
                        meta = eta_ik

                        if abs(eta_dev) < 0.01 * eta_col_mean:
                            push_e = meta * 0.3 * random.gauss(0, 1.0)
                        elif resp_z > 0:
                            direction_e = 1.0 if eta_dev > 0 else -1.0
                            push_e = meta * math.tanh(resp_z) * direction_e * 0.5
                        else:
                            push_e = meta * 0.1 * random.gauss(0, 1.0)
...
def run():
    print("=" * W)
    print("  STAGE 3 EXPERIMENT A: SELF-REFERENTIAL ETA")
    print("  eta[i,k] adapts via same resp_z signal as alpha")
    print("  Meta-rate = eta itself (multiplicative)")
    ...
    conditions = {
        ...
        'ALIVE-s3-A': dict(alive=True,  eta=0.0003, adaptive_eta=True),
    }
    ...
    # ── Verdict ───────────────────────────────────────────────
    print(f"\n{'=' * W}")
    print(f"  EXPERIMENT A VERDICT")
    print(f"{'=' * W}\n")

    s3 = results['ALIVE-s3-A']['overall']
    s2 = results['ALIVE-s2']['overall']
    st = results['STILL']['overall']

    print(f"  STILL:      {st:+.4f}")
    print(f"  ALIVE-s2:   {s2:+.4f} (delta vs STILL: {s2 - st:+.4f})")
    print(f"  ALIVE-s3-A: {s3:+.4f} (delta vs STILL: {s3 - st:+.4f})")
    print(f"  s3 vs s2:   {s3 - s2:+.4f}")

    print(f"\n  Novel:")
    print(f"  STILL:      {novel['STILL']:+.4f}")
    print(f"  ALIVE-s2:   {novel['ALIVE-s2']:+.4f}")
    print(f"  ALIVE-s3-A: {novel['ALIVE-s3-A']:+.4f}")
    print(f"  s3 vs s2:   {novel['ALIVE-s3-A'] - novel['ALIVE-s2']:+.4f}")

    s3_beats_s2 = s3 > s2
    s3_beats_still = s3 > st
    s2_beats_still = s2 > st
    novel_s3_beats = novel['ALIVE-s3-A'] > novel['ALIVE-s2']

    ground_truth = s3_beats_still  # system still discriminates
    ...
    if s3_beats_s2 and ground_truth:
        print(f"\n  STAGE 3-A: PASS")
        print(f"  Adaptive eta beats fixed eta. Frozen frame reduced 7->6.")
    elif s3_beats_still and not s3_beats_s2:
        print(f"\n  STAGE 3-A: PARTIAL")
        print(f"  Adaptive eta beats STILL but not fixed eta.")
        print(f"  Adaptation helps but self-referential meta-rate may be suboptimal.")
    else:
        print(f"\n  STAGE 3-A: FAIL")
        print(f"  Self-referential eta does not improve over fixed eta.")
