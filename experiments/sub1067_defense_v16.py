"""
sub1067_defense_v16.py — Defense v16: z-score normalized SPSA goal (ℓ₁).

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1067 --substrate experiments/sub1067_defense_v16.py

FAMILY: parametric-goal
R3 HYPOTHESIS: ℓ₁ SPSA goal adaptation is sufficient when paired with
  statistical z-score change detection. Prosecution v14 showed z-scores
  find more responsive games (80%/70%/0% vs defense v15's 20%/0%/0%).
  But the z-score detection is NOT inherently ℓ_π — it's a better SIGNAL
  for ANY adaptation mechanism. Defense v16 uses z-score detection with
  SPSA goal adaptation (ℓ₁) to test whether the improvement comes from
  z-scores (shared) or from attention encoding (ℓ_π specific).

  If defense v16 matches prosecution v14: z-scores are the key, not ℓ_π.
  If defense v16 < prosecution v14: ℓ_π attention gives additional benefit.

KILL: ALL games 0%
SUCCESS: Match or beat prosecution v14 (80%/70%/0%)
"""
import sys
...
# Probe boundaries (matching prosecution v14)
BASELINE_END = 200
KB_PROBE_END = 700
CLICK_PROBE_END = 1200
SEQ_PROBE_END = 2200

# Statistical detection (same as prosecution v14)
Z_THRESH = 3.0
EPSILON = 1e-6
PROBE_SIGNAL_THRESH = 0.03

# SPSA R3 parameters (ℓ₁)
SIGMA_INIT = 1.0
ALPHA_INIT = 0.999
...
class DefenseV16Substrate:
    """
    ℓ₁ R3 v16: z-score detection + SPSA goal adaptation.
    Same statistical detection as prosecution v14, but SPSA goal (ℓ₁)
    instead of attention encoding (ℓ_π). Tests whether z-scores or
    ...
    def _init_state(self):
        # Statistical baseline (same as prosecution v14)
    ...
    def _fitness(self, obs_start, obs_end):
        ...
        # Use z_change_map as weight (statistical significance)
        weight = self.z_change_map if self._baseline_done else self.change_map
        return float(np.sum(weight * reduction))

    def _r3_spsa_update(self, obs_before, obs_after, cx, cy):
        r = 3
        y0, y1 = max(0, cy - r), min(64, cy + r + 1)
        x0, x1 = max(0, cx - r), min(64, cx + r + 1)
        # Use z-score for zone significance (not just absolute change)
        if self._baseline_done:
            z = self._compute_z_score(obs_after)
            zone_significant = float(z[y0:y1, x0:x1].mean()) > Z_THRESH
        else:
            local_change = float(np.abs(
                obs_after[y0:y1, x0:x1] - obs_before[y0:y1, x0:x1]).mean())
            zone_significant = local_change > CF_CHANGE_THRESH
        ...
        def _step(val, delta, lo, hi, kwarg):
            ...
            s_p = mm_p if zone_significant else -mm_p
            s_m = mm_m if zone_significant else -mm_m
    ...
    def _do_kb_bootloader(self, arr):
        goal = self._goal(arr)
        weight = self.z_change_map if self._baseline_done else self.change_map
        mismatch = np.abs(arr - goal) * weight
    ...
    def on_level_transition(self):
        ...
        self._baseline_count = 0
        self._baseline_done = False
        # Keep SPSA params + kb_influence across levels (ℓ₁ transfer)
...
CONFIG = {
    ...
    "spsa_lr": SPSA_LR,
    "v16_features": "z-score detection + SPSA goal adaptation — isolates z-score vs attention contribution (l_1)",
}

SUBSTRATE_CLASS = DefenseV16Substrate
