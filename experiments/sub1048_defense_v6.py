"""
sub1048_defense_v6.py — Defense v6: full-pixel random exploration + SPSA (ℓ₁).

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1048 --substrate experiments/sub1048_defense_v6.py

FAMILY: parametric-goal
R3 HYPOTHESIS: ℓ₁ R3 with full-pixel exploration + multi-signal goal.
  v4 used 8x8 coarse grid (64 positions) for click exploration.
  v6 fix: sample clicks from FULL 64x64 pixel space during warmup.
  Coverage: ~2250 unique positions (vs 64 in v4) in 2500 warmup steps.

  WHY: LF52/SC25 at 0% across 70 runs may be caused by coarse grid
  missing small interactive zones. Full-pixel sampling covers 55% of
  pixel space vs 1.5% with coarse grid.

  Phase 1 (0-210): ALL 7 keyboard actions × 30.
  Phase 2 (210-2500): Random FULL-PIXEL clicks (90% if click-game).
  Phase 3 (2500+): SPSA-guided exploitation with multi-signal goal.

  If LF52/SC25 interactive zones are smaller than 8px grid spacing,
  full-pixel sampling discovers them where coarse grid cannot.
  Falsified if: same 0% (zones aren't discoverable at pixel resolution either).
...
class DefenseV6Substrate:
    """
    ℓ₁ R3 v6: full-pixel random exploration + multi-signal goal.
    Fixes v4's coarse grid (64 positions) with full pixel-space sampling.
    """
    ...
    def process(self, obs: np.ndarray) -> int:
        ...
        # ── Phase 2: random warmup (210-2500) — full pixel sampling ──
        if self.step_count < WARMUP_STEPS:
            if self._supports_click and self._rng.random() < 0.9:
                cx, cy = int(self._rng.randint(64)), int(self._rng.randint(64))
...
CONFIG = {
    ...
    "spsa_lr": SPSA_LR,
    "v6_features": "keyboard-first warmup + full-pixel random exploration + multi-signal goal",
}

SUBSTRATE_CLASS = DefenseV6Substrate
