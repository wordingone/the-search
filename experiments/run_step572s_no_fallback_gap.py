"""
Step 572s — Eliminate fallback gap during puq waypoint navigation.

Root cause of 572r L3=0:
  When _puq_dr_action() returns None on waypoint arrival, the actual player
  executes a FALLBACK action that DR doesn't track → DR desyncs from actual
  player position. After GIC arrival, fallback might move player away; then
  the BFS from DR=GIC sends next BFS action that doesn't match player's
  actual position.

Fix: loop in _puq_dr_action() until a non-trivial BFS path is found.
  On arrival at each waypoint, immediately register the event, compute the
  next BFS, and return the first action. No None returned except at the
  final LHS arrival (win should have fired).

  Also: keep death flash reset (from 572r) — deaths still reset pxr() and
  player teleports to spawn, so DR must reset too.

Waypoint sequence:
  SPAWN → GIC (touch gic: tmx++) → KDY (touch bgt: tuv=1) →
  KDY_BOUNCE → KDY (tuv=2) → KDY_BOUNCE → KDY (tuv=3) → LHS (win!)
"""
import numpy as np
import time
import sys
...
# Full waypoint sequence: SPAWN not included (starting position), then:
# GIC(x1) → KDY → BOUNCE → KDY → BOUNCE → KDY → LHS
PUQ_WAYPOINTS = [
    PUQ_GIC_GRID,       # 0: touch gic → tmx=1
    PUQ_KDY_GRID,       # 1: touch bgt → tuv=1
    PUQ_KDY_BOUNCE,     # 2: bounce (no sprite effect)
    PUQ_KDY_GRID,       # 3: touch bgt → tuv=2
    PUQ_KDY_BOUNCE,     # 4: bounce
    PUQ_KDY_GRID,       # 5: touch bgt → tuv=3
    PUQ_LHS_GRID,       # 6: touch mae → win (if state correct)
]
# avoid_set additions per waypoint arrival:
PUQ_WP_AVOID = {
    0: PUQ_GIC_GRID,    # after GIC touch, don't re-visit
    5: PUQ_KDY_GRID,    # after 3rd KDY touch, don't re-visit (would decrement tuv)
}
...
class SubDual:
    ...
    def _puq_dr_action(self):
        """No-fallback-gap waypoint navigator. Loops until it finds an action."""
        ...
        max_iter = len(PUQ_WAYPOINTS) + 2  # safety limit

        for _ in range(max_iter):
            if self._puq_wp_idx >= len(PUQ_WAYPOINTS):
                return None  # all waypoints visited (shouldn't happen — win should fire at LHS)
            ...
            # path_len==1: arrived at current waypoint
            wp = self._puq_wp_idx
            if wp in PUQ_WP_AVOID:
                self._puq_avoid_set.add(PUQ_WP_AVOID[wp])
            if wp == len(PUQ_WAYPOINTS) - 1:
                # Final waypoint (LHS): win should fire when player steps onto LHS.
                # If we're here, it means we're at LHS already. Track and return None.
                self._puq_lhs_ap += 1
                return None

            # Advance to next waypoint and loop immediately
            self._puq_wp_idx += 1
            self._puq_bfs_path = []
            combined_walls = self._puq_wall_set | self._puq_avoid_set  # update after avoid_set change

        return None  # safety fallback (shouldn't reach here)
