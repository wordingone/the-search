"""
step0895e_alpha_clamped.py -- Alpha-800b with clamped concentration.

R3 hypothesis: alpha self-modification with MAX concentration ratio ≤ 50
enables stable warm transfer without cold-start concentration instability.

Problem from step895c: cold seeds hit alpha_conc=61.93 → L1=0 (over-concentration).
Warm seeds stable at ~16-17 → L1=77.8 (works). Clamping prevents cold runaway.

Leo mail 2623 fix: clip alpha to (0.1, 5.0) — max/min ratio ≤ 50.
(vs step895c: clip(0.01, 10.0) → max/min ratio ≤ 1000)

Architecture: identical to step895c/d except:
  alpha = clip(alpha, 0.1, 5.0)   ← tighter clamping

Expected: cold alpha_conc ≤ 50 → no instability → cold L1 > 23.0
Warm alpha still starts at pretrained values (within clamped range) → should still help.

Protocol: LS20, 25K, 10 seeds (sub_seed=seed%4). Pretrain seeds 1-5 (5K).
Report: mean L1, std, zero-seed fraction (per Leo mail 2626 — variance is key metric).
"""
import sys, time
...
EPSILON = 0.20
SOFTMAX_TEMP = 0.10
ALPHA_LO = 0.10   # tighter lower bound (vs 0.01 in 895c)
ALPHA_HI = 5.00   # tighter upper bound (vs 10.0 in 895c) → max/min ≤ 50
...
class AlphaClamped895e(BaseSubstrate):
    """Alpha-800b with tighter clamping clip(0.1, 5.0) — prevents cold instability."""
    ...
    def _update_alpha(self):
        ...
        self.alpha = np.clip(self.alpha, ALPHA_LO, ALPHA_HI)  # ← TIGHTER
...
print("=" * 70)
print("STEP 895e — ALPHA-800b CLAMPED (clip 0.1-5.0, max/min ≤ 50)")
print("=" * 70)
print(f"Prevents cold-start alpha instability (895c had conc=61.93 → L1=0).")
print(f"Hypothesis: cold L1 > 23.0 (no runaway). Warm L1 maintained.")
print(f"Key metric: zero-seed fraction (should be 0/10 for both cold+warm).")

t0 = time.time()

sub_p = AlphaClamped895e(n_actions=N_ACTIONS, seed=0)
sub_p.reset(0)
...
for ts in TEST_SEEDS:
    ...
    sub_c = AlphaClamped895e(n_actions=N_ACTIONS, seed=sub_seed)
    ...
    sub_w = AlphaClamped895e(n_actions=N_ACTIONS, seed=sub_seed)
...
print(f"\nComparison:")
print(f"  868b (plain 800b, 25K): mean=72.1 std=112 zero=5/10")
print(f"  895c warm (unclamped, 10K): mean=77.8 std=35 zero=0/5")
print(f"  895e warm (clamped 0.1-5.0, 25K): mean={mw:.1f} std={sw:.1f} zero={zw}/{len(warm_comps)}")
print(f"  Clamping fixed cold instability: {'YES' if max(cold_concs) <= 50 else f'NO (max_conc={max(cold_concs):.1f})'}")
print(f"\nTotal elapsed: {time.time()-t0:.1f}s")
print("STEP 895e DONE")
