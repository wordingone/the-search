"""
step0968_action_conditioned_h.py -- Action-conditioned recurrent h.

R3 HYPOTHESIS: h encoding action history breaks FT09's sequential dependency problem.
916's h = f(obs history only). 968's h = f(obs history + action history).
"I saw X, did A, now see Y" → different h than "I saw X, did B, now see Z."
W_pred can now learn: after action A → predict action B context → FT09 sequence learning.

One change from 916: h_new += W_a_input @ one_hot(prev_action) (random fixed W_a_input).

Kill: LS20 < 74.7 (916@10K). Success: FT09 > 0 at 10K.

Architecture: action-conditioned echo-state.
...
class RecurrentTrajectory916:
    ...
    def __init__(self, n_actions, seed):
        ...
        # Action-conditioned input (one change from 916)
        self.W_a_input = rs.randn(H_DIM, n_actions).astype(np.float32) * 0.01
        self._prev_action_oh = np.zeros(n_actions, dtype=np.float32)
    ...
    def _encode(self, obs):
        ...
        # Update recurrent state — action-conditioned (one change from 916)
        self.h = np.tanh(self.W_h @ self.h + self.W_x @ enc
                         + self.W_a_input @ self._prev_action_oh)
    ...
    def process(self, obs):
        ...
        # Update prev_action one-hot for next step
        self._prev_action_oh[:] = 0.0
        self._prev_action_oh[action] = 1.0
        return action
...
print("=" * 70)
print("STEP 968 — ACTION-CONDITIONED h (h encodes action+obs history)")
print("=" * 70)
...
print(f"\n{'='*70}")
print(f"STEP 968 RESULTS (916@10K: LS20=74.7/8/10, FT09=0/10):")
