"""
Step 1280 — fixed I3 metric + re-run Step 1276 (LPL PE, unnormalized).

I3 fix: replace Spearman(visit_freq, kb_responsiveness) with coverage
uniformity (coefficient of variation). I3_cv = std(counts)/mean(counts).
Lower CV = more uniform = better coverage. Index-order independent.

Also: I3_perm diagnostic on KB games (LS20/TR87/TU93) — control_c_permuted
confirms artifact extends beyond LS20.

Answers:
1. Does 1276 have REAL I3 regressions (by CV) or only artifact (by rho)?
2. Is the I3 rho artifact LS20-specific or universal on KB games?
3. If no real CV regressions: 1276 IS the composition (FT09 L1=4/5).

Composition: LPLPESubstrate (lpl_pe — unnormalized LPL prediction error, Step 1276)
Control:     Control C (argmin-alone)
Diagnostic:  Control C Permuted (random tie-breaking, KB games only)
Games:       ft09, ls20, vc33, tr87, sp80, sb26, tu93, cn04, cd82, lp85
Draws:       5 per main condition per game = 100 runs
             + 5 permuted draws on LS20/TR87/TU93 = 15 extra runs
Budget:      10K steps, one config

Spec: Leo mail 3566, 2026-03-28.
"""
...
# ─── Config ───
GAMES = ['ft09', 'ls20', 'vc33', 'tr87', 'sp80', 'sb26', 'tu93', 'cn04', 'cd82', 'lp85']
KB_GAMES = ['ls20', 'tr87', 'tu93']  # I3_perm diagnostic games
N_DRAWS = 5
MAX_STEPS = 10_000
MAX_SECONDS = 300
...
ETA_PRED = 0.01
PE_EMA_ALPHA = 0.05
SELECTION_ALPHA = 0.1  # unnormalized pe influence (same as 1276)
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1280')
...
class ControlC:
    """Pure argmin — index-ordered tie breaking."""
...
class PermutedControlC:
    ...
    first in perm order (random) rather than the lowest index.
    Used to confirm I3 rho artifact extends to TR87/TU93.
    """
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        self._perm = rng_p.permutation(n_actions)
...
class LPLPESubstrate:
    """Step 1276/1280 — unnormalized LPL prediction error as action selection.

    W_pred (ENC_DIM x ENC_DIM) predicts enc_{t+1} from enc_t.
    pe_a = EMA of ||enc_{t+1} - W_pred @ enc_t|| per action.
    Selection: argmin(visit_count - SELECTION_ALPHA * pe_ema).
    W_action maintained via Hebbian for R3 measurement only.
    """
    ...
    def process(self, obs_raw) -> int:
        ...
        # Unnormalized pe selection (Step 1276 config)
...
def compute_i3_rho(action_counts_200, kb_delta):
    """Legacy I3 rho (broken on KB games — kept for comparison)."""
...
def compute_i3_cv(action_counts_200, n_actions):
    """I3 replacement: coefficient of variation of visit counts.

    Lower CV = more uniform coverage. Independent of index ordering.
    CV = std(counts) / mean(counts).
    For pure argmin on n_actions at step 200: theoretical CV is fixed.
    """
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    if condition == 'lpl_pe':
        substrate = LPLPESubstrate(n_actions=n_actions, seed=seed)
    elif condition == 'control_c':
        substrate = ControlC(n_actions=n_actions, seed=seed)
    elif condition == 'control_c_permuted':
        substrate = PermutedControlC(n_actions=n_actions, seed=seed)
    else:
    ...
    while steps < MAX_STEPS:
        ...
        if steps in PE_SNAPSHOT_STEPS and condition == 'lpl_pe' and hasattr(substrate, 'pe_ema'):
            pe = substrate.pe_ema
            n7 = min(7, n_actions)
            pe_snapshots[steps] = {
                'mean': round(float(pe.mean()), 6),
                'min': round(float(pe.min()), 6),
    ...
    # I3: both rho (legacy, broken on KB) and cv (new metric)
    ...
    if condition == 'lpl_pe' and r3_snapshot and r3_obs_sample:
        r3_result = compute_r3_jacobian(substrate, r3_obs_sample, r3_snapshot, n_actions)
    elif condition in ('control_c', 'control_c_permuted') and r3_obs_sample:
    ...
    cond_labels = {'lpl_pe': 'LPE', 'control_c': 'CTL', 'control_c_permuted': 'CTLp'}
    label = cond_labels.get(condition, condition)
    ...
    print(f" [{label}] Lmax={max_level} | {i3_str} | {i1_str(i1_enc_result,'I1enc')} | {r3_str} | {arc_str} | enc_n={n_repr_enc}(L={n_levels_enc}) | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1280 — LPL PE (1276 config) with fixed I3 metric + I3_perm diag")
    print("LPLPESubstrate (unnormalized) vs Control C. I3 = CV not rho.")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"KB_GAMES (I3_perm): {KB_GAMES}")
    print(f"Draws: {N_DRAWS} per main condition per game = {N_DRAWS * 2 * len(GAMES)} main runs")
    print(f"       + {N_DRAWS} permuted draws on {len(KB_GAMES)} KB games = {N_DRAWS * len(KB_GAMES)} extra runs")
    ...
    for game_name in GAMES:
        ...
        for condition in ['lpl_pe', 'control_c']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'lpl_pe' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    # ─── I3_perm diagnostic: 15 extra runs ───
    print(f"\n{'='*60}")
    print("I3_PERM DIAGNOSTIC — KB games: control_c_permuted")
    print(f"{'='*60}")
    for game_name in KB_GAMES:
        try:
            diag = load_game_diag(game_name)
            kb_delta = build_kb_profile(diag)
            game_n_actions = diag.get('n_actions', 4103)
            ...
            continue
        solver_level_steps = solver_level_steps_by_game.get(game_name, {})
        print(f"\n  {game_name.upper()} | control_c_permuted")
        for draw in range(1, N_DRAWS + 1):
            seed = draw * 100 + 50
            result = run_single(
                game_name=game_name,
                condition='control_c_permuted',
                draw=draw,
                seed=seed,
                n_actions=game_n_actions,
                kb_delta=kb_delta,
            ...
            all_results.append(result)
            fname = os.path.join(RESULTS_DIR, f"{game_name}_control_c_permuted_draw{draw}.json")
            with open(fname, 'w') as f:
                json.dump(result, f, indent=2)

    total_elapsed = time.time() - t_global
    print(f"\n{'='*70}")
    print(f"STEP 1280 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # ─── Summary ───
    print("I3 CV Summary (new metric — lower = more uniform):")
    for game in GAMES:
        for cond in ['lpl_pe', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'LPE' if cond == 'lpl_pe' else 'CTL'
            cvs = [r['I3_cv'] for r in runs if r.get('I3_cv') is not None]
            mean_cv = float(np.mean(cvs)) if cvs else None
            print(f"  {game.upper()} {label}: I3_cv={mean_cv:.6f}" if mean_cv is not None else f"  {game.upper()} {label}: I3_cv=null")

    print("\nI3 rho Summary (legacy — ARTIFACT on KB games):")
    for game in GAMES:
        for cond in ['lpl_pe', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'LPE' if cond == 'lpl_pe' else 'CTL'
            rhos = [r['I3_rho'] for r in runs if r.get('I3_rho') is not None]
            passes = sum(1 for r in runs if r.get('I3_rho_pass') == True)
            mean_rho = float(np.mean(rhos)) if rhos else 0.0
            print(f"  {game.upper()} {label}: I3_rho={mean_rho:.3f} ({passes}/{len(runs)})")

    print("\nI3_perm (artifact extent — KB games):")
    for game in KB_GAMES:
        for cond in ['control_c', 'control_c_permuted']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'CTL' if cond == 'control_c' else 'CTLp'
            rhos = [r['I3_rho'] for r in runs if r.get('I3_rho') is not None]
            passes = sum(1 for r in runs if r.get('I3_rho_pass') == True)
            mean_rho = float(np.mean(rhos)) if rhos else 0.0
            print(f"  {game.upper()} {label}: I3_rho={mean_rho:.3f} ({passes}/{len(runs)})")
    ...
    for game in GAMES:
        for cond in ['lpl_pe', 'control_c']:
            ...
            print(f"  {game.upper()} {label}: ARC={np.mean(arcs):.4f} L1={l1s}/{len(runs)}")

    with open(os.path.join(RESULTS_DIR, 'step1280_results.json'), 'w') as f:
