"""
Step 1366 — SSM disconnected with low temperature (T=0.1). Amplify state structure.
Leo mail 3900, 2026-03-30.

Same architecture as 1364 (SSM 2L disconnected, 2K steps). ONE change:
action selection uses T=0.1 instead of T=1.0.

At T=1.0 (default): softmax near-uniform -> no concentration -> no sequences.
At T=0.1: softmax sharper -> top-logit action dominates -> temporal autocorrelation.
As SSM state evolves, the dominant action shifts -> structured exploration without training.

Architecture: IDENTICAL to 1364. Only ACT_TEMP changes.
...
- Prediction head: W_pred @ y_t -> next projected obs (trained via RTRL)
- Action head: W_act @ y_t (fixed random, NEVER updated)
- Action selection: sample(softmax(logits / T)), T=0.1

Protocol:
- 10 draws, seeds 13660-13669
- 2K steps per episode
- Kill: chain_mean < 4.59e-5 (MLP+TP baseline)
- Signal: T=0.1 RHAE > baseline, action_autocorr > 0.5, entropy drops below T=1.0
"""
...
STEP        = 1366
N_DRAWS     = 10
DRAW_SEEDS  = [13660 + i for i in range(N_DRAWS)]
ACT_TEMP    = 0.1   # softmax temperature for action selection (1364 used 1.0)
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
class MambaSSMSubstrate:
    ...
    def __init__(self, n_actions, max_steps=TRY1_STEPS):
        ...
        # Autocorrelation tracking: fraction of steps where a_t == a_{t-1}
    ...
    def process(self, obs_arr):
        ...
        if self._step < WARMUP:
            ...
            logits = (logits - logits.max()) / ACT_TEMP
            probs = np.exp(logits)
            probs /= probs.sum()
            action = int(np.random.choice(self.n_actions, p=probs))

        # Track autocorrelation (step > 0, after warmup)
        if self._step > WARMUP:
            if self._in_try2:
    ...
    def get_stats(self):
        ...
        t1_autocorr = round(self._try1_autocorr_count / self._try1_autocorr_steps, 4) if self._try1_autocorr_steps > 0 else None
        t2_autocorr = round(self._try2_autocorr_count / self._try2_autocorr_steps, 4) if self._try2_autocorr_steps > 0 else None
        return {
            ...
            'try1_autocorr':       t1_autocorr,
            'try2_autocorr':       t2_autocorr,
        }
...
def main():
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: SSM low-temp (T={ACT_TEMP}) — temporal structure via sharper softmax ===")
    ...
    print(f"W_act: fixed random, never updated. ACT_TEMP={ACT_TEMP}.")
    print(f"MLP+TP baseline (1349): {MLP_TP_BASELINE:.2e}  |  1364 (T=1.0): 1.341e-4 (draw variance)")
