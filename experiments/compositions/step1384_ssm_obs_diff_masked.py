"""
Step 1384 — Predict obs DIFF (not obs_next): action effect is the signal.
Leo mail 3952, 2026-03-30.

Steps 1379-1383: obs_prediction objective has stable obs-only fixed point.
Gradient trap: at the attractor, masking action doesn't change prediction error
(ratio≈1.00). Architecture-independent — linear, gated, inverse-dynamics, hard-inject,
selective all confirmed action-blind.

Root cause: predicting obs_next doesn't REQUIRE action information. obs_{t+1} is mostly
predicted from obs_t (inertia). The action-free component explains ~95%+ of variance.

Fix: predict diff = proj_obs_{t+1} - proj_obs_t instead.

Key difference:
  FULL: target = proj_obs_next              (mostly inertia, action irrelevant)
  DIFF: target = proj_obs_next - proj_obs_t (mostly zero when action has no effect,
                                             non-zero when action changes obs)

Hypothesis: diff is SPARSE and action-caused. The model can predict zeros for most steps,
but non-zero diffs require knowing WHICH action was taken. This creates gradient pressure
toward action conditioning that doesn't exist for absolute obs prediction.

Note: diff computed in projected space is equivalent to projecting the raw diff:
  W @ (obs_{t+1} - obs_t) = W @ obs_{t+1} - W @ obs_t
so no architecture change needed — just change the prediction target.

Architecture: identical to step1383 (SelectiveSSM), same N_LAYERS, D, N_STATE.
Mandatory first diagnostic: 3 draws DIFF vs DIFF_MASKED.
Gate: action_blind_ratio = pred_loss(DIFF_MASKED)/pred_loss(DIFF) > 1.05.

Conditions (30 draws, paired):
  DIFF: SelectiveSSM, predict obs_diff. Frozen projection try2.
  FULL: SelectiveSSM, predict obs_next (baseline: same arch, prior objective).
Both: PRNG fix, h reset, weights carry. Same W_fixed per (draw, game).

Seeds: 14110-14139 (fresh).
"""
...
STEP        = 1384
N_DRAWS     = 30
DRAW_SEEDS  = [14110 + i for i in range(N_DRAWS)]
...
# Frozen projection (same as 1378-1383)
TEMPERATURE   = 3.0

# Thresholds
ACTION_BLIND_THRESHOLD = 0.05   # ratio > 1.05 → uses action info

CONDITIONS     = ['DIFF', 'FULL']
DIAG_CONDITION = 'DIFF_MASKED'
...
# ---------------------------------------------------------------------------
# Selective SSM Layer (identical to step1383)
# ---------------------------------------------------------------------------
...
class SSMSubstrate:
    """
    mode: 'diff'         — SelectiveSSM, predict proj_obs_diff. Frozen projection try2.
          'diff_masked'  — same but action token zeroed in SSM input. (diagnostic only)
          'full'         — SelectiveSSM, predict proj_obs_next. (baseline: 1383 objective)

    W_fixed: (n_actions, H_DIM) frozen projection.
    """

    def __init__(self, n_actions, mode='diff', W_fixed=None):
        ...
        self._prev_action     = 0
        self._prev_y          = None
        self._prev_proj_obs   = None   # for diff target: proj_obs_t at time of process()
        self._in_try2         = False
    ...
    def _rtrl_step(self, proj_obs_next, y):
        ...
        For DIFF/DIFF_MASKED: target = proj_obs_next - _prev_proj_obs
        For FULL: target = proj_obs_next
        """
        if self.mode in ('diff', 'diff_masked'):
            if self._prev_proj_obs is None:
                return None   # skip: no previous obs to diff against
    ...
    def process(self, obs_arr):
        ...
        self._prev_y       = y
        self._prev_proj_obs = proj_obs.copy()   # cache for diff target
...
def run_draw(draw_idx, draw_seed, cond_name, max_steps):
    ...
    try2_progress   = {}
    optimal_steps_d = {}
    mode = cond_name.lower()   # 'diff', 'diff_masked', 'full'
    ...
    for game_idx, (game_name, label) in enumerate(zip(games, game_labels.values())):
        ...
        # Frozen projection: same seed formula as 1378-1383
...
def main():
    ...
    sub_t1 = SSMSubstrate(n_actions=na_t1, mode='diff', W_fixed=W_fixed_t1)
    ...
    # ── Mandatory first diagnostic: 3 draws of DIFF vs DIFF_MASKED ────────
    print(f"\n=== MANDATORY DIAGNOSTIC: {N_DIAG_INIT} draws DIFF vs DIFF_MASKED ===")
    diag_diff_rhae  = []; diag_diff_pred  = []; diag_diff_pred_t2  = []
    diag_mask_rhae  = []; diag_mask_pred  = []; diag_mask_pred_t2  = []

    t_diag = time.time()
    for di in range(N_DIAG_INIT):
        ds = DRAW_SEEDS[di]
        rhae_d, pred_d, pred_t2_d, _ = run_draw(di, ds, 'DIFF',        max_steps)
        rhae_m, pred_m, pred_t2_m, _ = run_draw(di, ds, 'DIFF_MASKED', max_steps)
        diag_diff_rhae.append(rhae_d);  diag_diff_pred.append(pred_d);  diag_diff_pred_t2.append(pred_t2_d)
    ...
    diff_preds_valid = [p for p in diag_diff_pred_t2  if p is not None and not math.isnan(p)]
    mask_preds_valid = [p for p in diag_mask_pred_t2  if p is not None and not math.isnan(p)]

    if diff_preds_valid and mask_preds_valid and np.mean(diff_preds_valid) > 0:
        action_blind_ratio = round(float(np.mean(mask_preds_valid)) / float(np.mean(diff_preds_valid)), 4)
    else:
        action_blind_ratio = None

    print(f"\n  DIFF    pred_try2: {diag_diff_pred_t2}")
    print(f"  MASKED  pred_try2: {diag_mask_pred_t2}")
    print(f"  Action-blind ratio (DIFF_MASKED/DIFF): {action_blind_ratio}")
    ...
    if action_blind_ratio is None or action_blind_ratio < 1.0 + ACTION_BLIND_THRESHOLD:
        ...
        print("DIFF objective still action-blind. Objective change did not break the attractor.")
        summary = {
            'step':               STEP,
            'verdict':            verdict,
            'action_blind_ratio': action_blind_ratio,
            'diag_draws':         N_DIAG_INIT,
            'diag_diff_pred_t2':  diag_diff_pred_t2,
            'diag_masked_pred_t2': diag_mask_pred_t2,
            'note': 'Full experiment aborted — diff objective also action-blind.',
        }
    ...
    print(f"  Diff objective is action-conditional! Proceeding to full {N_DRAWS}-draw experiment.")

    # ── Full experiment: DIFF and FULL, 30 draws ──────────────────────────
    print(f"\n=== STEP {STEP}: DIFF vs FULL, {N_DRAWS} draws, {max_steps} steps ===")
    ...
    # Reuse first N_DIAG_INIT draws from diagnostic for DIFF
    for di in range(N_DIAG_INIT):
        rhae_by_cond['DIFF'].append(round(diag_diff_rhae[di], 7))
        pred_by_cond['DIFF'].append(diag_diff_pred[di])
        pred_t2_by_cond['DIFF'].append(diag_diff_pred_t2[di])
        print(f"  [DIFF] Draw {di} (from diagnostic) RHAE={diag_diff_rhae[di]:.6e}")

    # Run remaining DIFF draws
    print(f"\n--- Condition: DIFF (draws {N_DIAG_INIT}-{N_DRAWS-1}) ---")
    t_cond = time.time()
    for di in range(N_DIAG_INIT, N_DRAWS):
        ds = DRAW_SEEDS[di]
        rhae, pred, pred_t2, _ = run_draw(di, ds, 'DIFF', max_steps)
        rhae_by_cond['DIFF'].append(round(rhae, 7))
        pred_by_cond['DIFF'].append(pred)
        pred_t2_by_cond['DIFF'].append(pred_t2)
    print(f"  DIFF done in {time.time()-t_cond:.0f}s")

    # Run FULL (all 30 draws)
    print(f"\n--- Condition: FULL ---")
    t_cond = time.time()
    for di, ds in enumerate(DRAW_SEEDS):
        rhae, pred, pred_t2, _ = run_draw(di, ds, 'FULL', max_steps)
        rhae_by_cond['FULL'].append(round(rhae, 7))
        pred_by_cond['FULL'].append(pred)
        pred_t2_by_cond['FULL'].append(pred_t2)
    print(f"  FULL done in {time.time()-t_cond:.0f}s")
    ...
    # Paired sign test: DIFF vs FULL
    diff_rhae = rhae_by_cond['DIFF']
    full_rhae = rhae_by_cond['FULL']
    wins   = sum(1 for d, f in zip(diff_rhae, full_rhae) if d > f)
    losses = sum(1 for d, f in zip(diff_rhae, full_rhae) if d < f)
    ties   = N_DRAWS - wins - losses
    p_val  = binomial_p_one_sided(wins, wins + losses)

    print(f"\nPaired sign test (DIFF vs FULL):")
    print(f"  Wins={wins}  Losses={losses}  Ties={ties}  p={p_val:.4f}")
    print(f"\nAction-blind ratio (DIFF_MASKED/DIFF, {N_DIAG_INIT} diagnostic draws): {action_blind_ratio}")
    ...
    print(f"DIFF chain_mean:  {summary['conditions']['DIFF']['chain_mean']:.6e}")
    print(f"FULL chain_mean:  {summary['conditions']['FULL']['chain_mean']:.6e}")
