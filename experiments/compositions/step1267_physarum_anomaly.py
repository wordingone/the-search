"""
Step 1267 — Physarum anomaly flow + argmin selection (tube-weighted)

Amendment to 1266: Replace raw flow magnitude with anomaly flow.
Barlow 1961: informativeness ∝ surprise, not magnitude.

  enc = centered_obs               # (ENC_DIM,) — C4 centering
  h = tanh(W_h @ h + W_in @ enc)  # (H_DIM,) — C26 recurrent
  ext = [enc, h]                   # (EXT_DIM=320,)

  # Anomaly flow (after observing enc_after):
  delta = enc_after - enc_before
  delta_ema[last_action] = 0.9 * delta_ema[last_action] + 0.1 * delta
  flow = ||delta - delta_ema[last_action]||   # surprise, not magnitude
  W_action[last_action] += eta * flow * ext
  W_action *= (1.0 - decay)

  # Argmin weighted by tube thickness
  tube_thickness = ||W_action||_row   # (n_actions,) L2 norm of each action row
  effective_count = action_counts / (1.0 + tube_thickness)
  action = argmin(effective_count)
  action_counts[action] += 1

No argmax. Coverage maintained (argmin visits all actions).
Thick tubes = high-anomaly actions = visited sooner by argmin.
1266 raw-flow: VC33=5/5 (coverage fixed), LS20=1/5, SAL=0.829 on LS20.
Anomaly flow selects for variable/state-dependent actions (not just large ones).

Parameters: eta=0.01, decay=0.001, ema_rate=0.9 (frozen).
R3: W_action evolved by flow -> Jacobian diff vs initial random W_action.
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1267')
...
class PhysarumSubstrate:
    ...
    def __init__(self, n_actions: int, seed: int, freeze_mean: bool = False):
        ...
        # Per-action EMA of state change (for anomaly flow)
        self._delta_ema = np.zeros((n_actions, ENC_DIM), np.float32)
    ...
    def update_flow(self, next_obs_raw):
        ...
        # Anomaly flow: surprise relative to per-action EMA (Barlow 1961)
        self._delta_ema[self._last_action] = (
            EMA_RATE * self._delta_ema[self._last_action] + (1.0 - EMA_RATE) * delta
        )
        flow = float(np.linalg.norm(delta - self._delta_ema[self._last_action]))
        # Tube thickens with anomaly flow
...
def main():
    ...
    print(f"Physarum: eta={ETA_FLOW}, decay={DECAY}, ema_rate={EMA_RATE}. No refractory (argmin provides cycling).")
    print()
