"""
Step 1278 — normalized pe selection, full 10-game PRISM chain.

Same as Step 1276 but pe_ema normalized before use in selection:
  score_a = visit_count_a - alpha * (pe_ema_a / (max(pe_ema) + eps))

Normalization bounds signal to [0, alpha] regardless of action space size.
- FT09 (4103 actions, max/mean=55x): normalization amplifies ratio → pe_max action strongly preferred
- LS20 (7 actions, max/mean=1.2x): signal stays proportional → no I3 destruction

Composition: NormPESubstrate (normalized_pe — normalized LPL prediction error)
...
SELECTION_ALPHA = 0.1  # pe_ema influence on action selection
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1278')
...
class NormPESubstrate:
    """Step 1278 — normalized LPL prediction error as action selection signal.

    W_pred (ENC_DIM×ENC_DIM) predicts enc_{t+1} from enc_t.
    pe_a = EMA of ||enc_{t+1} - W_pred @ enc_t|| per action.
    Selection: argmin(visit_count - alpha * (pe_ema / (max(pe_ema) + eps))).
    Normalization bounds signal to [0, alpha] regardless of action space size.
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        self.pe_ema = np.zeros(n_actions, np.float32)  # per-action EMA of prediction error
    ...
    def process(self, obs_raw) -> int:
        ...
        # Normalized pe selection: argmin(visit_count - alpha * (pe_ema / (max + eps)))
        pe_norm = self.pe_ema / (self.pe_ema.max() + 1e-8)
        score = self.action_counts - SELECTION_ALPHA * pe_norm
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    if condition == 'normalized_pe':
        substrate = NormPESubstrate(n_actions=n_actions, seed=seed)
    ...
    r3_snapshot = None
    r3_obs_sample = None
    pe_snapshots = {}  # step → pe_ema snapshot (normalized_pe only)
    PE_SNAPSHOT_STEPS = {100, 1000, 5000, 10000}
    ...
    while steps < MAX_STEPS:
        ...
        # pe_a distribution snapshots (normalized_pe only)
        if steps in PE_SNAPSHOT_STEPS and condition == 'normalized_pe' and hasattr(substrate, 'pe_ema'):
            pe = substrate.pe_ema
            n7 = min(7, n_actions)
            pe_snapshots[steps] = {
    ...
    if condition == 'normalized_pe' and r3_snapshot and r3_obs_sample:
    ...
    label = 'NPE' if condition == 'normalized_pe' else 'CTL'
    def i1_str(r, tag):
...
def main():
    ...
    print("=" * 70)
    print("STEP 1278 — normalized pe selection, full 10-game PRISM chain with dual I1")
    print("NormPESubstrate vs Control C on all 10 solved games.")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Conditions: normalized_pe (normalized LPL pe), control_c (pure argmin)")
    ...
    for game_name in GAMES:
        ...
        for condition in ['normalized_pe', 'control_c']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'normalized_pe' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1278 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1278_results.json'), 'w') as f:
    ...
    for game in GAMES:
        for cond in ['normalized_pe', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'NPE' if cond == 'normalized_pe' else 'CTL'
            for tag, pk, wk, bk in [
