"""
Step 1257 — Combined Attention: pred-error × viability (multiplicative composition)

BOTH C25 (prediction-error attention) AND C_VIA (viability attention) kept.
Combined attention is their element-wise product.

  combined_attention[d] = alpha_pe[d] * via_attention[d]
  attended_enc = enc * combined_attention  [256D, unnormalized]

C25: dimensions with high prediction error → large alpha_pe (0.10–5.0).
C_VIA: dimensions where different actions produce different state changes → large via_attention.
Product: only dimensions that are BOTH experience-detectable AND action-relevant get high weight.

  1254: pred-error attn + GPR → R3=1.00, SAL=0.20
  1255: viability attn + GPR (normalized) → R3=0.00, SAL=0.40
  1256: viability attn + GPR (unnormalized) → R3=0.00, SAL=0.40
  1257: combined (pred-error × viability) + GPR → R3=?, SAL=?

Hypothesis: R3 > 0.05 (C25 provides the detectable range), SAL > 0.3 (viability
provides the action-relevance filter). Both pass simultaneously for the first time.
"""
...
ALPHA_FLOOR = 0.10       # Additive floor for attention (warmup)

# Prediction-error attention hyperparameters (C25, adapted to ENC_DIM)
ETA_PRED = 0.01
ALPHA_EMA_RATE = 0.10
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1257')
...
class ViaWiringA:
    """
    WiringA with COMBINED attention (C25 × C_VIA), argmin replaced by GPR.

    C25 (pred-error, adapted to ENC_DIM): alpha_pe[d] tracks prediction error
      per encoding dimension. Range: ALPHA_LO–ALPHA_HI (0.10–5.00).
    C_VIA (viability): via_attention[d] = max-min spread of per-action EMA of
      |enc_change[d]| + ALPHA_FLOOR.
    Combined: combined_attention[d] = alpha_pe[d] * via_attention[d]
      attended_enc = enc * combined_attention  [256D, unnormalized]

    GPR: salience = W_salience @ ext_enc_mod, argmax if max > mean+std.
    R3 internal repr: ext_enc = [attended_enc, h] (320D).
    """
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        # C25: Prediction-error attention (adapted to ENC_DIM)
        self.alpha_pe = np.ones(ENC_DIM, np.float32)  # initialized to 1.0
        self.pred_err_ema = np.zeros(ENC_DIM, np.float32)
        self.W_pred = np.zeros((ENC_DIM, ENC_DIM + n_actions), np.float32)
        ...
        # History
        self._prev_enc = None   # for viability delta computation
    ...
    def _update_pred_error_alpha(self, enc: np.ndarray):
        """C25 adapted to ENC_DIM: predict next enc from prev_enc * alpha_pe + action."""
        if self._prev_enc is None or self._prev_action is None:
            return
        a_oh = np.zeros(self.n_actions, np.float32)
        a_oh[self._prev_action] = 1.0
        inp = np.concatenate([self._prev_enc * self.alpha_pe, a_oh])  # (ENC_DIM + n_actions,)
        pred = self.W_pred @ inp                                        # (ENC_DIM,)
        error = (enc * self.alpha_pe) - pred                           # (ENC_DIM,)
    ...
    def get_internal_repr_readonly(self, obs_raw, frozen_running_mean,
        ...
        """R3 read-only: combined-attended enc + h = ext_enc (320D)."""
        obs = np.asarray(obs_raw, dtype=np.float32)
        x = _enc_frame(obs)
        ...
        # Combined attention: pred_error × viability (Step 1257)
        spread = np.max(frozen_via_alpha, axis=0) - np.min(frozen_via_alpha, axis=0)
        via_attention = spread + ALPHA_FLOOR
        combined_attention = frozen_alpha_pe * via_attention  # (ENC_DIM,)
        attended_enc = enc * combined_attention               # unnormalized

        # C21/C26: recurrent (frozen h)
        h_new = np.tanh(self.W_h @ frozen_h + self.W_x @ attended_enc)
        ext_enc = np.concatenate([attended_enc, h_new])
        return ext_enc  # 320D
    ...
    def process(self, obs_raw) -> int:
        ...
        # C25: Update prediction-error alpha (uses _prev_enc set at end of last step)
        self._update_pred_error_alpha(enc)

        # C_VIA: Update viability alpha from previous step's delta
        ...
        # Combined attention: pred_error × viability (Step 1257)
        via_attention = self._compute_attention()       # (ENC_DIM,)
        combined_attention = self.alpha_pe * via_attention  # (ENC_DIM,)
        attended_enc = enc * combined_attention         # 256D — unnormalized

        # Attention snapshots (combined)
        if self.step in ATTN_SNAPSHOT_STEPS:
            self._attn_snapshots[self.step] = combined_attention.copy()

        # C21/C26: Recurrent h (reads from attended_enc)
        self.h = np.tanh(self.W_h @ self.h + self.W_x @ attended_enc)
        ext_enc = np.concatenate([attended_enc, self.h])  # 320D

        # C15/C20: Transition detection
        trans_signal = self._transition_signal(obs_hash, self._prev_obs_hash)
        ext_enc_mod = ext_enc * (1 + TRANSITION_SCALE * trans_signal)
        ...
        # C_GPR: Salience-gated selection (same as 1254)
        salience = self.W_salience @ ext_enc_mod  # (n_actions,)
        ...
        # SAL tracking (same as 1254)
        self._sal_sum[action] += float(abs(salience[action]))
        self._sal_count[action] += 1

        # Store attended for I1
        self._prev_attended = ext_enc_mod.copy()

        # Store for next step
        self._prev_enc = enc.copy()  # enc AFTER C22, BEFORE attention (for delta)
...
def compute_r3_jacobian(substrate, obs_sample, frozen_state, n_actions, rng_seed=42):
    ...
    if _is_via:
        ...
        fresh_alpha_pe = np.ones(ENC_DIM, np.float32)  # fresh: alpha_pe=1.0
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    print(f" {l1_str} | {i3_str} | I4={'P' if i4_result['pass'] else 'F'} | {r3_str} | {sal_str} | {conc_str} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1257 — COMBINED ATTENTION: pred-error × viability")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("Comparison: 1256 R3=0.00/SAL=0.40 vs 1257 (pred-error × viability). Expect both R3 and SAL to pass.")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1257 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1257_results.json')
