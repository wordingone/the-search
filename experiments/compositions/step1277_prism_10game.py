"""
Step 1277 — bare LPL+argmin, full 10-game PRISM chain.

Strip all dynamics. LPL Hebbian updates W_action (for R3 measurement).
Action selection: pure argmin(visit_count) — no Physarum, no PE, no tube bias.

Tests: Was the FT09 L1 advantage (4/5 in 1274-1276) from dynamics or from R3 encoding?
- If FT09 L1~4/5 here: dynamics were irrelevant, R3 encoding IS the composition.
- If I3 regressions on LS20/TR87/TU93 disappear vs 1276: dynamics were causing them.

Composition: LPLBareSubstrate (lpl_bare — LPL Hebbian + pure argmin)
Control:     Control C (argmin-alone, no W_action update)
...
Spec: Leo RESEARCH_STATE.md edit, 2026-03-28.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1277')
...
class LPLBareSubstrate:
    """Step 1277 — bare LPL+argmin. Strips all dynamics from action selection.

    W_action updated via Hebbian (for R3 measurement only).
    Action selection: pure argmin(visit_count) — no tube bias, no PE, no dynamics.
    Tests whether R3 encoding modification (not dynamics) explains FT09 L1 advantage.
    """
    ...
    def process(self, obs_raw) -> int:
        ...
        # Pure argmin — no dynamics influence on selection
        action = int(np.argmin(self.action_counts))
        self.action_counts[action] += 1
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    if condition == 'lpl_bare':
        substrate = LPLBareSubstrate(n_actions=n_actions, seed=seed)
    ...
    if condition == 'lpl_bare' and r3_snapshot and r3_obs_sample:
    ...
    label = 'LPB' if condition == 'lpl_bare' else 'CTL'
    def i1_str(r, tag):
...
def main():
    ...
    print("=" * 70)
    print("STEP 1277 — LPL prediction error, full 10-game PRISM chain with dual I1")
    print("LPLBareSubstrate vs Control C on all 10 solved games.")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Conditions: lpl_bare (LPL prediction error), control_c (pure argmin)")
    ...
    for game_name in GAMES:
        ...
        for condition in ['lpl_bare', 'control_c']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'lpl_bare' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1277 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1277_results.json'), 'w') as f:
    ...
    for game in GAMES:
        for cond in ['lpl_bare', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'LPB' if cond == 'lpl_bare' else 'CTL'
            for tag, pk, wk, bk in [
