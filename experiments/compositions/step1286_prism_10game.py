"""
Step 1286 — Causal controls for allosteric claim (paper-quality).

Three ablation conditions test whether W modification (LPL Hebbian learning) is
causally necessary for L1 success and R3 signal.

Condition A — frozen_init:
  W_action and W_pred initialized normally, NEVER updated (eta_h=0, eta_p=0).
  h still evolves via forward pass. Tests: does L1 require W modification?

Condition B — frozen_mid:
  W updates normally (eta_h=0.05) for steps 0-5000, then frozen (step 5001+).
  Tests: is continuous W modification needed, or does early learning suffice?

Condition C — eta_slow:
  eta_h=0.005 (0.1x reduction from baseline 0.05). Everything else identical.
  Tests: dose-response of learning rate on L1/R3.

Control — lpl_pe_ctrl:
  eta_h=0.05 (Step 1282 confirmed composition). Baseline for comparison.

Predictions:
  A: L1=0/5 all games, R3~0 (W unchanged = no Jacobian diff)
  B: FT09 L1=2-4/5 (early learning persists), R3=0 after freeze point
  C: FT09 L1=3-5/5 (slower learning), R3 reduced but positive
  Ctrl: FT09 5/5, LS20 0/5 (reproduced from 1282)

Kill criteria: N/A — all outcomes are informative.

Games:  ft09, ls20, vc33, tr87, sp80, sb26, tu93, cn04, cd82, lp85
Draws:  5 per condition per game = 200 runs total
Budget: 10K steps, one config

Spec: Leo mail 3608, 2026-03-28.
"""
...
ETA_H_CTRL = 0.05     # control / condition B (pre-freeze)
ETA_H_SLOW = 0.005    # condition C (0.1x)
FREEZE_MID_STEP = 5000  # condition B: freeze after this step
...
CONDITIONS = ['frozen_init', 'frozen_mid', 'eta_slow', 'lpl_pe_ctrl']
LABELS = {'frozen_init': 'FRI', 'frozen_mid': 'FRM', 'eta_slow': 'ETS', 'lpl_pe_ctrl': 'CTL'}
...
class LPLPESubstrate:
    """LPL PE substrate with causal ablation controls.

    Supports three ablation modes:
      freeze_at_init=True  — W_action, W_pred frozen at random initialization
      freeze_mid_step=N    — W updates normally until step N, then frozen
      eta_h controls Hebbian update rate (condition C: 0.005 vs baseline 0.05)

    In all cases h evolves via forward pass; only weight updates are controlled.
    """

    def __init__(self, n_actions: int, seed: int, eta_h: float = ETA_H_CTRL,
                 freeze_at_init: bool = False, freeze_mid_step: int = None):
        self.n_actions = n_actions
        self.eta_h = eta_h
        self.freeze_at_init = freeze_at_init
        self.freeze_mid_step = freeze_mid_step
    ...
    def _is_frozen(self) -> bool:
        if self.freeze_at_init:
            return True
        if self.freeze_mid_step is not None and self.step > self.freeze_mid_step:
            return True
        return False
    ...
    def update_flow(self, next_obs_raw):
        ...
        if not frozen:
            ...
            self.W_action[self._last_action] += self.eta_h * flow * self._prev_ext
...
def make_substrate(condition: str, n_actions: int, seed: int) -> LPLPESubstrate:
    if condition == 'frozen_init':
        return LPLPESubstrate(n_actions=n_actions, seed=seed,
                              eta_h=ETA_H_CTRL, freeze_at_init=True)
    elif condition == 'frozen_mid':
        return LPLPESubstrate(n_actions=n_actions, seed=seed,
                              eta_h=ETA_H_CTRL, freeze_mid_step=FREEZE_MID_STEP)
    elif condition == 'eta_slow':
        return LPLPESubstrate(n_actions=n_actions, seed=seed,
                              eta_h=ETA_H_SLOW)
    else:  # lpl_pe_ctrl
        return LPLPESubstrate(n_actions=n_actions, seed=seed,
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    print(f" [{label}] Lmax={max_level} | {i3_str} | {i1_str(i1_enc_result,'I1enc')} | {r3_str} | {arc_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'draw': draw,
        'seed': seed,
        'eta_h': substrate.eta_h,
        'freeze_at_init': substrate.freeze_at_init,
        'freeze_mid_step': substrate.freeze_mid_step,
        'steps_taken': steps,
...
def main():
    ...
    print("=" * 70)
    print("STEP 1286 — Causal controls for allosteric claim")
    print("Conditions: frozen_init / frozen_mid / eta_slow / lpl_pe_ctrl")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1286 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # --- Summary ---
    print("R3 Summary (causal test — key metric):")
    for game in GAMES:
    ...
    print("\nL1 Summary (allosteric claim test):")
    for game in GAMES:
        parts = []
        for cond in CONDITIONS:
            label = LABELS[cond]
    ...
    for game in GAMES:
        ...
        for cond in CONDITIONS:
            ...
            parts.append(f"{label}={mean_cv:.4f}" if mean_cv is not None else f"{label}=null")
        print(f"  {game.upper()}: " + " | ".join(parts))

    with open(os.path.join(RESULTS_DIR, 'step1286_results.json'), 'w') as f:
