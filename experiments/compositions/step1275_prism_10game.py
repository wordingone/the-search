"""
Step 1275 — novelty-gated tube flow, full 10-game PRISM chain.

Same as Step 1274 but with novelty-gated Physarum flow:
  flow_a = ||delta_a - EMA(delta)|| × (1 - cos_sim(delta_a, EMA_a(delta)))

Suppresses repetitive state changes from reinforcing tubes.
Prediction: I3 recovers on ls20 (PHY currently 0/5, CTL 5/5).
ft09 L1=4/5 should be maintained. R3 maintained (7/10 games).

Composition: Physarum+argmin (novelty-gated flow)
...
ETA_FLOW = 0.01
DECAY = 0.001
EMA_ALPHA = 0.05   # novelty gate: EMA decay for global and per-action delta tracking
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1275')
...
class PhysarumSubstrate:
    """Step 1275 config — novelty-gated Physarum tube dynamics + argmin. EXT_DIM=320.

    Flow gate: flow_a = ||delta_a - EMA(delta)|| × (1 - cos_sim(delta_a, EMA_a(delta)))
    Suppresses repetitive state changes from reinforcing tubes.
    """
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        self.step = 0
        # Novelty gate state
        self.ema_delta = np.zeros(ENC_DIM, np.float32)          # global EMA of delta
        self.ema_delta_per_action = np.zeros((n_actions, ENC_DIM), np.float32)  # per-action EMA
    ...
    def update_flow(self, next_obs_raw):
        ...
        delta = enc_after - self._prev_enc_flow  # shape: (ENC_DIM,)

        # Novelty gate: flow_a = ||delta - EMA(delta)|| × (1 - cos_sim(delta, EMA_a(delta)))
        # Compute novelty factors using PRIOR EMA values (before updating)
        delta_novel = delta - self.ema_delta                    # subtract global mean
        flow_base = float(np.linalg.norm(delta_novel))          # ||delta_a - EMA(delta)||

        a = self._last_action
        ema_a = self.ema_delta_per_action[a]
        norm_delta = np.linalg.norm(delta)
        norm_ema_a = np.linalg.norm(ema_a)
        if norm_delta > 1e-8 and norm_ema_a > 1e-8:
            cos_sim_a = float(np.dot(delta, ema_a) / (norm_delta * norm_ema_a))
            novelty_a = max(0.0, 1.0 - cos_sim_a)
        else:
            novelty_a = 1.0  # first visit: maximum novelty

        flow = flow_base * novelty_a

        # Update EMA state (after computing novelty)
        self.ema_delta = (1.0 - EMA_ALPHA) * self.ema_delta + EMA_ALPHA * delta
        self.ema_delta_per_action[a] = (1.0 - EMA_ALPHA) * ema_a + EMA_ALPHA * delta
...
def main():
    ...
    print("=" * 70)
    print("STEP 1275 — full 10-game PRISM chain with dual I1")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1275 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1275_results.json'), 'w') as f:
