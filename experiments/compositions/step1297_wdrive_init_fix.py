"""
Step 1297 — W_drive init scale fix: 0.01 → 0.1, full PRISM chain.

Context: 1296 confirmed activation normalization fix (ro_drift measurable). New bottleneck:
W_inhibit init U(0.5-1.5) >> W_drive init (0.01). Drive/inhibit ratio ≈ 1:50-150.
Competitive network produces win_act ≈ 0 throughout → Oja gate `win_act > 1e-8` never fires
→ W_drive frozen → R3=0.000 structurally.

Fix: W_drive init 0.01 → 0.1 (10x increase).
  self.W_drive = rng_w.randn(NETWORK_SIZE, EXT_DIM).astype(np.float32) * 0.1  # was 0.01

New drive/inhibit ratio ≈ 1:5-15. W_drive can compete. win_act should be non-trivial.
Oja should fire. W_drive should learn. R3 > 0 expected for first time on reflexive network.

Architecture (all conditions):
  N=64, W_drive (64x320) Oja, W_inhibit frozen U(0.5-1.5), W_pred for pe.
  ...
  W_readout update (condition-dependent):
    ...
    FROZEN-RO:   W_readout frozen (1296 FROZEN-RO baseline).
...
One variable vs 1296: W_drive init scale 0.01 → 0.1.

Kill criteria (same as 1296):
...
Risk: Higher activations may destabilize attractors. Diagnostic: if attractor_sil < 0.5 on
all games AND attractor_k = 1 everywhere → attractors collapsed (inhibition was load-bearing).
Then try Option B (reduce W_inhibit scale instead).

Predictions:
  - win_act > 1e-8 routinely → Oja fires → W_drive changes → R3 > 0 (HIGH confidence)
  - ro_drift similar to 1296 (readout normalization unchanged)
  - L1 on any game: LOW confidence (learning mechanism now active, but behavioral consequence unknown)

Direction change note: pre-drafted self-predictive consolidation SUSPENDED until W_drive
actually learns. This fixes the engineering bottleneck first (Leo mail 3648, 2026-03-28).

Budget: 10 ARC × 3 cond × 5 draws = 150 + 1 MBPP × 3 cond × 5 draws = 15 → 165 runs
  ~8s/ARC + ~5s/MBPP → ~1275s ≈ 21 min (within 30-min chain cap).

Spec: Leo mail 3648, 2026-03-28.
"""
...
# Known baselines from step1296 FROZEN-RO (= 1295 NEG-DIAG baseline)
FROZEN_RO_BASELINE = {'ft09': {'lock': 0.771, 'k': 4.0}}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1297')
...
class WDriveFixSubstrate:
    """N=64 NEG-DIAG competitive network with W_drive init 0.1 (was 0.01 in 1296).

    All conditions use NEG-DIAG base (fill_diagonal(W_recur, -0.1) always).

    Key change vs 1296: W_drive init scale 0.01 → 0.1.
      self.W_drive = rng_w.randn(NETWORK_SIZE, EXT_DIM) * 0.1  # was * 0.01

    Motivation: W_inhibit U(0.5-1.5) >> W_drive (0.01) → win_act ≈ 0 → Oja never fires.
    W_drive init 0.1 → drive/inhibit ratio ≈ 1:5-15 → non-trivial win_act expected.

    W_readout conditions (unchanged from 1296):
      learned_ro:  W_readout[action] += W_RO_LR * pe * act_norm_vec  [if pe > threshold]
      frozen_ro:   W_readout frozen (1296 FROZEN-RO baseline)
    ...
    def __init__(self, n_actions, seed, mode='learned_ro'):
        ...
        # Oja fire tracking (new in 1297 — confirms bottleneck fix)
        ...
        # win_act samples for scale diagnostic
        self._win_act_samples = []
        self._win_act_sample_freq = 100  # sample every 100 steps
    ...
    def compute_wdrive_drift(self):
        """Frobenius norm of W_drive - W_drive_init. Key diagnostic for Oja activity."""
        diff = self.W_drive - self.W_drive_init
        return round(float(np.linalg.norm(diff, 'fro')), 4)
    ...
    def process(self, obs_raw, current_level=0):
        ...
        if self.step % self._win_act_sample_freq == 0:
            self._win_act_samples.append(win_act)
        if win_act > 1e-8:
            self.W_drive[winner] += OJA_LR * win_act * (ext - self.W_drive[winner])
            self.oja_fire_count += 1  # track Oja activations
        ...
        # W_readout update (condition-dependent)
        # Activation-normalized update (from 1296 fix, unchanged)
    ...
    def mean_win_act(self):
        """Mean win_act over sampled steps. Near 0 in 1296, should be > 0 in 1297."""
        if not self._win_act_samples:
            return None
        return round(float(np.mean(self._win_act_samples)), 6)
...
def run_single(game_name, condition, draw, seed, n_actions, solver_level_steps):
    ...
    substrate = WDriveFixSubstrate(n_actions=n_actions, seed=seed, mode=condition)
    ...
    wdrive_drift = substrate.compute_wdrive_drift()
    oja_fires = substrate.oja_fire_count
    mean_win_act = substrate.mean_win_act()
    ...
    wd_str = f"wd_drift={wdrive_drift:.4f}(oja={oja_fires})"
    wa_str = f"win_act={mean_win_act:.4f}" if mean_win_act is not None else "win_act=?"
    print(f" [{label}] Lmax={max_level} | {lock_str} | {i3_str} | {r3_str} "
          f"{sil_str} {ro_str} | {wd_str} | {wa_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'wdrive_drift': wdrive_drift,
        'oja_fire_count': oja_fires,
        'mean_win_act': mean_win_act,
...
def main():
    ...
    print("=" * 72)
    print("STEP 1297 — W_drive init fix: 0.01 → 0.1 (10x increase)")
    print("LEARNED-RO:  W_readout[action] += 0.001 * pe * act_norm_vec  [pe-gated]")
    print("FROZEN-RO:   W_readout frozen (1296 FROZEN-RO baseline)")
    print("UNGATED-RO:  W_readout[action] += 0.001 * act_norm_vec  [always]")
    print("ALL conditions: NEG-DIAG base (W_recur diag=-0.1 always)")
    print("KEY CHANGE vs 1296: W_drive init scale 0.01 → 0.1")
    print("New drive/inhibit ratio ≈ 1:5-15 (was 1:50-150)")
    print("=" * 72)
    print(f"ARC games: {ARC_GAMES} | N_DRAWS={N_DRAWS}")
    print(f"MBPP: {MBPP_GAMES} | N_DRAWS_MBPP={N_DRAWS_MBPP}")
    print(f"Known FROZEN-RO (1296): FT09 lock={FROZEN_RO_BASELINE['ft09']['lock']}")
    print(f"Prediction (HIGH confidence): oja_fire_count >> 0, wdrive_drift > 0, R3 > 0")
    print(f"Prediction (LOW confidence): L1 on any game")
    print()
    ...
    print(f"\n{'='*72}")
    print(f"STEP 1297 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*72}\n")

    # W_drive learning diagnostic (PRIMARY — confirms bottleneck fix)
    print("W_drive Learning Diagnostic (PRIMARY — was R3=0 structurally in 1296):")
    for game in ARC_GAMES[:3]:  # show first 3 games as representative
        print(f"\n  {game.upper()}:")
        for cond in CONDITIONS:
            ...
            if not runs:
                continue
            oja_fires = [r.get('oja_fire_count', 0) for r in runs]
            wd_drifts = [r.get('wdrive_drift', 0) for r in runs]
            win_acts = [r.get('mean_win_act') for r in runs if r.get('mean_win_act') is not None]
            r3s = [r.get('R3_jacobian_diff', 0) for r in runs if r.get('R3_jacobian_diff') is not None]
            n = len(runs)
            print(f"    [{label}] oja_fires={sum(oja_fires)/n:.0f} | wdrive_drift={sum(wd_drifts)/n:.4f} | "
                  f"win_act={sum(win_acts)/len(win_acts):.4f} | R3={sum(r3s)/len(r3s):.4f}" if (win_acts and r3s) else
                  f"    [{label}] oja_fires={sum(oja_fires)/n:.0f} | wdrive_drift={sum(wd_drifts)/n:.4f}")

    # L1 summary
    print("\nL1 Results (central question — any L1 from LEARNED-RO?):")
    for game in GAMES:
        print(f"\n  {game.upper()}:")
        for cond in CONDITIONS:
            label = LABELS[cond]
    ...
    # R3 summary
    print("\nR3 Summary (HIGH confidence: R3 > 0 expected for first time):")
    for game in ARC_GAMES:
        ...
        for cond in CONDITIONS:
            ...
            wd_str = f"{sum(wd_drifts)/len(runs):.4f}" if runs else "?"
            print(f"    [{label}] R3={r3_str} | pass={r3_pass}/{len(runs)} | wdrive_drift={wd_str}")

    # ro_drift summary
    print("\nro_drift Summary (carried over from 1296 fix):")
    for game in GAMES:
        print(f"\n  {game.upper()}:")
        for cond in CONDITIONS:
            label = LABELS[cond]
            ...
            print(f"    [{label}] ro_drift={drift_display} (n_updates={updates_display})")
    ...
    # Attractor stability check (risk diagnostic from spec)
    print("\nAttractor stability check (risk: higher W_drive may collapse attractors):")
    all_sils = [r.get('attractor_silhouette') for r in all_results if r.get('attractor_silhouette') is not None]
    all_ks = [r.get('attractor_k') for r in all_results if r.get('attractor_k') is not None]
    if all_sils:
        mean_sil = float(np.mean(all_sils))
        mean_k = float(np.mean(all_ks)) if all_ks else None
        collapsed = mean_sil < 0.5 and mean_k is not None and mean_k < 1.5
        print(f"  Overall: mean_sil={mean_sil:.3f} | mean_k={mean_k:.1f} | "
              f"{'*** COLLAPSED — inhibition was load-bearing ***' if collapsed else 'stable'}")
    else:
    ...
    with open(os.path.join(RESULTS_DIR, 'step1297_results.json'), 'w') as f:
