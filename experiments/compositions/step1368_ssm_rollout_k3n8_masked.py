"""
Step 1368 — ARGMAX rollout K=3 N=8 at 2K steps. Option A scale-down.
Leo mail 3905, 2026-03-30.

Step 1367 (K=10, N=32) showed ARGMAX signal at 2.1x baseline but budget cap hit at 337
steps (7.16ms/step). This reduces rollout cost 13x: K=3, N=8 (24 rollout steps vs 320).

ARGMAX only (ARGMIN dead). Otherwise identical to 1367.
Goal: confirm ARGMAX signal at full 2K steps with 1800 effective post-warmup steps.
"""
...
STEP        = 1368
N_DRAWS     = 10
DRAW_SEEDS  = [13680 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# Conditions
CONDITIONS = ['ROLLOUT-ARGMAX']  # ARGMIN killed in 1367
...
def main():
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: SSM ARGMAX rollout K={K_ROLLOUT} N={N_CANDIDATES} at 2K steps ===")
    print(f"K_ROLLOUT={K_ROLLOUT}, N_CANDIDATES={N_CANDIDATES}, WARMUP={WARMUP}")
    print(f"SSM: D={D}, N_STATE={N_STATE}, N_LAYERS={N_LAYERS}")
    print(f"Condition: ROLLOUT-ARGMAX only (ARGMIN killed in 1367)")
    print(f"MLP+TP baseline (1349): {MLP_TP_BASELINE:.2e}  |  1367-ARGMAX: 9.70e-5")
