"""
Step 1358 — Conditional info-gain type_head: train only for keyboard-only games.
Leo mail 3871, 2026-03-30. Replication of 1357 on 10 fresh draws.
...
N_DRAWS     = 10
DRAW_SEEDS  = [13510 + i for i in range(N_DRAWS)]

RESULTS_DIR       = os.path.join('B:/M/the-search/experiments/compositions', 'results_1358')
...
def main():
    ...
    hier_rhae = load_baseline(HIER_BASELINE_DIR, 1352, 10)  # all 10 draws from 1352
    hier_mean = sum(hier_rhae) / len(hier_rhae)
    hier_nz   = sum(1 for r in hier_rhae if r > 0)

    print(f"Step {STEP} — COND-INFO replication on 10 fresh draws (1357 showed 2.64× HIER).")
    ...
    print(f"HIER baseline (1352 all 10 draws): {hier_rhae}  mean={hier_mean:.7f}  nz={hier_nz}/10")
    print()
    ...
    print()
    print(f"  Baselines:")
    print(f"    HIER (1352 all 10): {hier_rhae}  mean={hier_mean:.7f}  nz={hier_nz}/10")
    print()
