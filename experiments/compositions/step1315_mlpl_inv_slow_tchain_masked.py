"""
Step 1315 — Inverse model W3 with reduced eta (fix cr regression, preserve T_chain)
Leo mail 3720, 2026-03-29.

Step 1314 found: T_chain=5.72 (largest signal), RHAE=1.5e-5 (floor cracked).
Kill trigger: INV cr > BASE cr (0.87 vs 0.82) — from exploration disruption, not broken coupling.

Fix: W3_ETA = eta / 10. Inverse model learns slower → less exploration shock → cr recovers.
W1/W2 keep full eta. Same coupling law, different rate for W3 only.

Kill criteria:
  INV-SLOW cr > BASE cr → still regressing even at lower rate → direction problem → KILL
  INV-SLOW T_chain ≤ BASE T_chain → reduced rate killed the signal → KILL

Predictions (Leo):
  1. INV-SLOW cr ≈ BASE cr (reduced rate = less exploration disruption)
  2. INV-SLOW T_chain > BASE T_chain (transfer signal survives, maybe < 5.72 but > 3.23)
  3. INV-SLOW RHAE ≥ BASE RHAE (floor stays cracked)
"""
...
GAMES, GAME_LABELS = select_games(seed=1315)
...
CONDITIONS = ['inv_slow', 'base']
LABELS     = {'inv_slow': 'INV-SLOW', 'base': 'BASE'}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1315')
...
W_CLIP  = 100.0
T_MIN   = 0.01
W3_ETA  = ETA / 10   # reduced learning rate for inverse model W3
...
class InvSlowSubstrate(BaseSubstrate):
    """Inverse model W3 with reduced learning rate (W3_ETA = eta/10).

    W3 += W3_ETA * outer(e3, h2)
    e3 = action_onehot(a) - softmax(W3 @ h2)

    W1/W2 keep full ETA. Slower W3 learning → less exploration disruption → cr recovers.
    """
    ...
    def process(self, obs_raw):
        ...
        # W1, W2: full eta (unchanged)
        self.W1 += ETA * np.outer(h1, e1)
        self.W2 += ETA * np.outer(h2, e2)
        np.clip(self.W1, -W_CLIP, W_CLIP, out=self.W1)
        np.clip(self.W2, -W_CLIP, W_CLIP, out=self.W2)

        # W3: inverse model, reduced learning rate (eta/10)
        e3 = action_onehot - probs
        self.W3 += W3_ETA * np.outer(e3, h2)
        np.clip(self.W3, -W_CLIP, W_CLIP, out=self.W3)
...
def make_substrate(condition, n_actions, checkpoints=None):
    if condition == 'inv_slow':
        return InvSlowSubstrate(n_actions=n_actions, checkpoints=checkpoints)
...
def kill_assessment(chain_summary, t_chain_inv, t_chain_base):
    ...
    if t_chain_inv is not None and t_chain_base is not None:
        if t_chain_inv <= t_chain_base:
            lines.append(
                f"  >>> KILL: INV-SLOW T_chain={t_chain_inv:.4f} ≤ BASE T_chain={t_chain_base:.4f} "
                f"→ reduced rate killed the signal"
            )
            kill = True
        else:
            lines.append(
                f"  >>> SIGNAL: INV-SLOW T_chain={t_chain_inv:.4f} > BASE T_chain={t_chain_base:.4f} "
                f"→ transfer signal survives at lower rate"
            )
        pred2 = "CONFIRMED" if (t_chain_inv is not None and t_chain_inv > t_chain_base) else "WRONG"
        lines.append(f"  >>> PREDICTION 2 (INV-SLOW T_chain > BASE): {pred2}")

    # cr comparison (Leo prediction 1 — main fix check)
    if cr_inv is not None and cr_base is not None:
        cr_diff = cr_inv - cr_base
        pred1 = "CONFIRMED" if cr_inv <= cr_base else "WRONG"
        lines.append(
            f"  >>> PREDICTION 1 (INV-SLOW cr ≈ BASE cr): {pred1} "
            f"(INV-SLOW={cr_inv:.4f}, BASE={cr_base:.4f}, diff={cr_diff:+.4f})"
        )
        if cr_inv > cr_base:
            lines.append(
                f"  >>> KILL: INV-SLOW cr={cr_inv:.4f} > BASE cr={cr_base:.4f} "
                f"→ still regressing — direction problem not rate problem"
            )
            kill = True

    # RHAE comparison (Leo prediction 3)
    rhae_inv = s_inv.get('mean_RHAE', 0.0)
    rhae_base = s_base.get('mean_RHAE', 0.0)
    pred3 = "CONFIRMED" if rhae_inv >= rhae_base else "WRONG"
    lines.append(
        f"  >>> PREDICTION 3 (INV-SLOW RHAE ≥ BASE RHAE): {pred3} "
        f"(INV-SLOW={rhae_inv:.2e}, BASE={rhae_base:.2e})"
    )

    # Collapse check
    if kl_inv is not None:
        if kl_inv < 0.01:
            lines.append(f"  >>> KILL: INV-SLOW action_KL={kl_inv:.4f} < 0.01 → collapsed")
            kill = True
        else:
            lines.append(f"  >>> OK: INV-SLOW action_KL={kl_inv:.4f} ≥ 0.01")

    if kill:
        lines.append("  >>> KILL TRIGGERED")
    else:
        lines.append("  >>> NO KILL — reduced rate fixes cr regression, transfer preserved")
...
def main():
    ...
    print(f"Step 1315 — Inverse model W3, reduced eta (W3_eta=eta/10), T_chain")
    print(f"Games: {masked_game_list(GAME_LABELS)} (ARC games masked)")
    print(f"Conditions: {CONDITIONS}")
    print(f"INV-SLOW: W3 += (eta/10) * outer(e3, h2), e3 = action_onehot - softmax(W3@h2)")
    print(f"BASE: W3 += eta * outer(action_onehot, h2)  (Hebbian, 1313 baseline)")
    print(f"W3_ETA={W3_ETA} (eta/10={ETA/10}), W1/W2 keep full ETA={ETA}")
    ...
    for game in GAMES:
        ...
        out_path = os.path.join(RESULTS_DIR, label_filename(label, 1315))
    ...
    t_chain_inv = round(float(np.mean(t_data['inv_slow'])), 4) if t_data['inv_slow'] else None
    t_chain_base = round(float(np.mean(t_data['base'])), 4) if t_data['base'] else None

    summary = {
        'chain_summary': chain_summary,
        'T_chain': {'inv_slow': t_chain_inv, 'base': t_chain_base},
    ...
    sep = '=' * 100
    print(f"\n{sep}")
    print(f"STEP 1315 — CHAIN-LEVEL SUMMARY\n")
    for cond in CONDITIONS:
        lbl = LABELS[cond]
    ...
    print(f"  T_chain: INV-SLOW={t_inv_str}  BASE={t_base_str}")
    print()
    ...
    print(f"  1314 reference: INV T_chain=5.7188, BASE T_chain=3.2263, INV cr=0.8661, BASE cr=0.8152")
