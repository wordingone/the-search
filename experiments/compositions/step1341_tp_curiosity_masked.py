"""
Step 1341 — Curiosity-driven action selection: explore where model is most wrong.
Leo mail 3813, 2026-03-29.

Same architecture as 1340 (MLP + TP), different criterion.
1340 used novelty (maximize predicted change). 1341 uses prediction ERROR —
take actions where the model has historically been most wrong.

Why prediction error is better than novelty:
- Novelty: concentrated on visually-responsive but task-irrelevant actions
- Prediction error: concentrated on state boundaries where rules change (task-relevant)

With 92% accurate model, most actions have low error (model predicts outcome well).
High-error actions = rare transitions OR state boundaries = level progression candidates.

Implementation:
- Track surprise_by_action[a] = EMA of prediction error for each action (init=1.0)
- After taking action, compute surprise = ||pred_h3_next - actual_h3_next|| in latent space
- Update EMA: surprise_by_action[a] = (1-EMA_ALPHA)*old + EMA_ALPHA*surprise
- Action selection: sample proportional to surprise_by_action (curiosity = explore where wrong)
- Warmup = 500 steps (random, for model to calibrate before curiosity is meaningful)
- Optimistic init: unvisited actions have surprise=1.0 → encouraged to explore all actions

Constitutional audit:
  R0: Deterministic init, seed-free ✓
  R1: Self-supervised. Action selected by internal prediction error, no external reward ✓
  R2: Prediction error IS the computation driving action selection ✓
  R3: As model calibrates, surprise estimates change, action selection changes ✓
  R4: Implicit in RHAE(try2) ✓
  R5: Game is ground truth ✓
  R6: Deletion test: remove curiosity → entropy baseline ✓

NEW component: curiosity-driven action selection via EMA prediction error.
Extends 1340's deliberation paradigm. First test of error-weighted exploration.

Conditions:
  curiosity:  MLP + TP + error-weighted action sampling (EMA surprise per action)
  mlp_tp:     MLP + TP + entropy (random) selection — 1337/1340 baseline

Protocol: 2K steps per try, seed-free, level-masked. 1 draw × 3 games × 2 conditions.

Kill:     CURIOSITY RHAE(try2) ≤ NOVELTY(1340) RHAE=0 → prediction error doesn't beat novelty. KILL.
Signal:   CURIOSITY progress on ≥1 game → SIGNAL. First curiosity-guided progress ever.
Both 0:   Model-based action selection doesn't help regardless of criterion. KILL paradigm.
"""
...
GAMES, GAME_LABELS = select_games(seed=1341)
...
CONDITIONS  = ['curiosity', 'mlp_tp']
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1341')
...
# Curiosity action selection constants
_CURIOSITY_WARMUP  = 500    # env steps before using curiosity (model calibration time)
_CURIOSITY_EMA     = 0.1    # EMA alpha for surprise tracking


# ---------------------------------------------------------------------------
# Obs encoding (same as 1337-1340)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Curiosity substrate (CURIOSITY: error-weighted sampling)
# ---------------------------------------------------------------------------

class CuriosityMlpTpSubstrate:
    """MLP + TP + curiosity-driven action selection via EMA prediction error.

    Action selection:
      Warmup (steps 0-500): random.
      Phase 2 (steps 500+):
        - maintain surprise_by_action[a] = EMA(prediction error for action a)
        - prediction error = ||pred_h3_next - actual_h3_next|| (latent space L2)
        - actions never tried: surprise = 1.0 (optimistic init → exploration encouraged)
        - sample action proportional to surprise_by_action
        - after observing result: update EMA for chosen action

    Carry from try1: model weights, buffer, surprise_by_action (transfer test).
    """
    ...
    def __init__(self, n_actions):
        ...
        # Curiosity state: EMA surprise per action (optimistic init = 1.0)
        self._surprise_by_action = np.ones(n_actions, dtype=np.float32)
    ...
    def reset_for_try2(self):
        ...
        self._pred_loss_at = {ck: None for ck in LOSS_CHECKPOINTS}
        self._action_log_all = []
        # NOTE: model weights, buffer, and surprise_by_action all carry to try2
    ...
    def process(self, obs_arr):
        ...
        # Warmup or before buffer ready: random
        if self._step <= _CURIOSITY_WARMUP or len(self._buffer) < _BATCH_SIZE:
            action = int(self._rng.randint(self.n_actions))
            self._prev_action = action
            self._action_log_all.append(action)
            return action % self.n_actions

        # Curiosity: sample proportional to surprise_by_action
        surp = self._surprise_by_action
        total = surp.sum()
        probs = surp / total if total > 1e-10 else np.ones(self.n_actions) / self.n_actions
    ...
    def update_after_step(self, obs_next, action, reward):
        ...
        # Update surprise for the action taken (after model has some training)
        if self._step > _CURIOSITY_WARMUP and len(self._buffer) >= _BATCH_SIZE:
            try:
                with torch.no_grad():
                    prev_t = torch.from_numpy(
                        self._prev_enc.astype(np.float32)).unsqueeze(0).to(_DEVICE)
                    next_t = torch.from_numpy(
                        enc_next.astype(np.float32)).unsqueeze(0).to(_DEVICE)
                    h3_prev, _, _, _ = self._model.forward_all_layers(prev_t)
                    h3_next_actual, _, _, _ = self._model.forward_all_layers(next_t)
                    at_t = torch.from_numpy(at_vec).unsqueeze(0).to(_DEVICE)
                    pred_in = torch.cat([h3_prev, at_t], dim=1)
                    pred_h3_next = self._model.pred_head(pred_in)
                    surprise = float((pred_h3_next - h3_next_actual).norm().item())
                a_idx = int(action) % self.n_actions
                self._surprise_by_action[a_idx] = (
                    (1.0 - _CURIOSITY_EMA) * self._surprise_by_action[a_idx]
                    + _CURIOSITY_EMA * surprise
                )
            except Exception:
                pass  # don't break episode on surprise update failure
    ...
    def get_action_entropy(self):
        """Normalized action entropy (0=concentrated, 1=uniform)."""
    ...
    def get_surprise_stats(self):
        """Diagnostic: max/mean surprise ratio (>1 = concentrated exploration)."""
        s = self._surprise_by_action
        mean_s = float(s.mean())
        max_s  = float(s.max())
        if mean_s < 1e-10:
            return None
        return round(max_s / mean_s, 3)


# ---------------------------------------------------------------------------
# MLP-TP control substrate (same as 1337-1340, random action selection)
# ---------------------------------------------------------------------------

class MlpTpSubstrate:
    """MLP + TP control. Random action selection. Identical to step 1340."""
    ...
    def get_surprise_stats(self):
        return None  # control has no surprise tracking
...
def run_episode(env, substrate, n_actions, seed, max_steps):
    ...
    return steps_to_first_progress, progress_count, {
        ...
        'surprise_ratio':          substrate.get_surprise_stats(),
...
def run_game(game_name, label, condition):
    ...
    if condition == 'curiosity':
        substrate = CuriosityMlpTpSubstrate(n_actions=n_actions)
    else:
...
def main():
    ...
    print(f"Step {STEP} — Curiosity-driven action selection (CURIOSITY vs MLP-TP)")
    print(f"Games: {masked_game_list(GAME_LABELS)} (ARC games masked)")
    print(f"Device: {_DEVICE}")
    print(f"Warmup={_CURIOSITY_WARMUP} steps (random), then EMA surprise-weighted sampling")
    print(f"EMA alpha={_CURIOSITY_EMA}, optimistic init=1.0 for unvisited actions")
    ...
    if first_arc:
        ...
        print(f"Tier 1: timing on {first_label} (100 steps, CURIOSITY)...")
        env_t = make_game(first_arc)
        try:
            na_t = int(env_t.n_actions)
        except AttributeError:
            na_t = 4103
        sub_t = CuriosityMlpTpSubstrate(n_actions=na_t)
        t0 = time.time()
    ...
    for condition in CONDITIONS:
        ...
        for game_name, label in zip(GAMES, GAME_LABELS.values()):
            ...
            surp1   = result['try1'].get('surprise_ratio')
            surp2   = result['try2'].get('surprise_ratio')

            try2_progress[label] = p2
            optimal_steps[label] = result['optimal_steps']
            eff_sq_by_game_cond[condition][label] = eff

            surp_str = f'  surp_ratio={surp1}/{surp2}' if surp1 is not None else ''
            print(f"  {label}: speedup={format_speedup(speedup)}  cr={cr}  eff²={eff}"
                  f"  ent1={ent1}  ent2={ent2}{surp_str}  ({elapsed:.1f}s)")
    ...
    # Report
    # -------------------------------------------------------------------------
    novelty_rhae = 0.0  # 1340 result for comparison
    print("=" * 80)
    print(f"STEP {STEP} — RESULT (Curiosity/prediction-error vs MLP-TP entropy)")
    print()
    print(f"  CURIOSITY  RHAE(try2) = {rhae_by_condition['curiosity']:.4f}")
    ...
    for label in GAME_LABELS.values():
        c_val = eff_sq_by_game_cond['curiosity'].get(label, 0.0)
        m_val = eff_sq_by_game_cond['mlp_tp'].get(label, 0.0)
        print(f"  {label}: CUR={c_val:.6f}  ENT={m_val:.6f}")
    print()

    cur_rhae = rhae_by_condition['curiosity']
    ent_rhae = rhae_by_condition['mlp_tp']

    print("KILL/SIGNAL ASSESSMENT:")
    if cur_rhae is not None and cur_rhae > 0:
        print(f"  >>> SIGNAL: CURIOSITY RHAE={cur_rhae:.4f} > 0. First model-based progress!")
        if cur_rhae > novelty_rhae:
            print(f"  >>> CURIOSITY beats NOVELTY. Prediction error > predicted change. LANDMARK.")
    elif cur_rhae == 0.0 and ent_rhae == 0.0:
        print(f"  >>> KILL: CURIOSITY RHAE=0.0000 = NOVELTY RHAE=0.0000.")
        print(f"  >>> Model-based action selection doesn't help regardless of criterion.")
        print(f"  >>> KILL PARADIGM: deliberation via forward model is insufficient.")
    else:
        print(f"  >>> KILL: CURIOSITY RHAE={cur_rhae:.4f} ≤ NOVELTY RHAE={novelty_rhae:.4f}.")
    print("=" * 80)
