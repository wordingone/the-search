"""
sub1057_prosecution_v10.py — Prosecution v10: adaptive cascading strategy (ℓ_π).

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1057 --substrate experiments/sub1057_prosecution_v10.py

FAMILY: attention-gated
R3 HYPOTHESIS: ℓ_π with adaptive strategy allocation via short probes.
  Fixed epochs waste budget on inapplicable strategies (Step 1055: LS20
  regressed because 5000 steps went to click/sequence on a keyboard game).

  Instead: 500-step probes with early exit. Budget goes to what WORKS.
  Self-evaluation gates = the internal restart Jun described.

Step 0-500: KEYBOARD PROBE — all 7 KB actions. If signal → commit 9500 to KB.
Step 500-1000: CLICK PROBE (if KB failed) — random clicks. If signal → commit 9000 to clicks.
Step 1000-2000: SEQUENCE PROBE (if both failed) — evo sequences. If signal → commit 8000.
Step 2000+: EXPLOIT detected game type with full remaining budget.

Attention transfers across probes: "these dims respond to KB" informs
which dims to prioritize (or deprioritize) during click probe.
...
class ProsecutionV10Substrate:
    ...
    def process(self, obs: np.ndarray) -> int:
        ...
        # ── Evaluate KB probe ──
            ...
            avg_kb = self._kb_change_accum / KB_PROBE_END
            if avg_kb > PROBE_SIGNAL_THRESH:
...
CONFIG = {
    ...
    "att_init": ATT_INIT,
    "att_lr": ATT_LR,
    "v10_features": "adaptive cascade — short probes + early exit + attention transfer (l_pi)",
}

SUBSTRATE_CLASS = ProsecutionV10Substrate
