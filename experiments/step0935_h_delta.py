"""
step0935_h_delta.py -- h-delta tracking (reservoir surprise) instead of enc-delta.

R3 hypothesis: Reservoir surprise (||Δh||) captures trajectory novelty that observation
surprise (||Δenc||) misses. h encodes "where I've been" — changes to h reflect how
unexpected the current observation is GIVEN THE TRAJECTORY. A correct click at position 3
produces h-change toward a novel region of h-space. A wrong click (reset) produces
h-change back toward familiar h-space (position 0).

Architecture: Same as 916 (recurrent h + clamped alpha + 800b). ONLY change:
Current (800b): delta[a] EMA tracks ||ext_enc_t - ext_enc_{t-1}|| * alpha  (enc-space change)
935:            delta[a] EMA tracks ||h_t - h_{t-1}||                       (reservoir surprise)
...
class Chain935:
    """916 recurrent h + clamped alpha. 800b tracks h-change instead of enc-change.

    Alpha + running_mean persist across games. W_pred + h + delta reset per game.
    """
    ...
    def reset_seed(self, seed):
        ...
        self.h = np.zeros(H_DIM, dtype=np.float32)
        self.delta_per_action = None
        self._prev_ext = None; self._prev_h = None; self._prev_action = None
    ...
    def process(self, obs):
        prev_h = self.h.copy()   # capture h BEFORE encode updates it
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            # 800b tracking: h-delta instead of enc-delta (THE ONLY CHANGE)
            h_change = float(np.linalg.norm(self.h - self._prev_h))
            a = self._prev_action
    ...
    def delta_summary(self):
        return f"min={self.delta_per_action.min():.3f} max={self.delta_per_action.max():.3f}"
...
print("=" * 70)
print("STEP 935 — h-DELTA TRACKING (reservoir surprise, ||Δh|| not ||Δenc||)")
print("=" * 70)
print("R3: 800b tracks ||h_t - h_{t-1}|| instead of ||ext_enc_t - ext_enc_{t-1}||.")
print("h encodes trajectory context — h-change reflects trajectory surprise, not enc-change.")
t0 = time.time()
...
print(f"\n{'=' * 70}")
print(f"STEP 935 RESULTS (h-delta, PRISM-light, 10K/phase, 5 seeds):")
...
print(f"  916 (enc-delta EMA, 25K): LS20=290.7  FT09=0  VC33=0")
print(f"  914 (enc-delta, 10K):     LS20=248.6  FT09=0  VC33=0")
print(f"  935 (h-delta, 10K):       LS20={np.mean(ls20):.1f}  FT09={np.mean(ft09):.1f}  VC33={np.mean(vc33):.1f}")
