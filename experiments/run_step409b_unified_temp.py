#!/usr/bin/env python3
"""
Step 409b -- Unified temperature. ONE noisy sims for everything.

Single noisy sims used for action selection, winner, spawn, attract.
Temperature self-tunes from spawn rate (target 5%).
Feedback loop works because spawn responds to temperature.

LS20. 64x64. 50K steps.
Script: scripts/run_step409b_unified_temp.py
"""
...
class TempFold:
    ...
    def process(self, x):
        ...
        # Add temperature noise — ONE set of sims for EVERYTHING
        if self.temperature>0:
            si=si+self.temperature*torch.randn_like(si)

        # Action selection: argmin on class votes
        ...
        # Spawn/attract using SAME noisy sims
...
def main():
    t0=time.time()
    print(f"Step 409b -- Unified temperature. 64x64. 50K. LS20.", flush=True)
    ...
    print("STEP 409b SUMMARY", flush=True); print("="*60, flush=True)
