"""
Step 572t — Fix xhp-step DR desync after puq death.

Root cause of 572s L3=0 (wp_idx=4, lhs_ap=0):
  On the step AFTER a death flash, game is in xhp=True state: it just
  does egb.set_visible(False), xhp=False, and returns WITHOUT processing
  the player's action (no movement). But our DR still called _puq_advance_dr()
  and moved 1 step → DR 1 step ahead of actual player after each death.
  With 2 deaths/episode, DR is 2 steps ahead → player misses GIC/KDY sprites
  → tmx/tuv never set correctly → win never fires.

Fix: _puq_xhp_step flag.
  When death flash detected in observe(): set _puq_xhp_step=True.
  In act(): if _puq_xhp_step, clear flag and return any action WITHOUT
  calling _puq_advance_dr(). Player returns dummy action (game ignores
  it during xhp), DR stays at spawn. Next step: both DR and player at
  spawn. ✓ Synchronized.

Chain: 572l→572m→572n→572o→572p→572q→572r→572s→572t
Each step fixed one specific root cause:
...
PUQ_WAYPOINTS = [
    PUQ_GIC_GRID,    # 0: gic touch → tmx=1
    PUQ_KDY_GRID,    # 1: bgt touch → tuv=1
    PUQ_KDY_BOUNCE,  # 2: bounce
    PUQ_KDY_GRID,    # 3: bgt touch → tuv=2
    PUQ_KDY_BOUNCE,  # 4: bounce
    PUQ_KDY_GRID,    # 5: bgt touch → tuv=3
    PUQ_LHS_GRID,    # 6: mae touch → win
]
PUQ_WP_AVOID = {
    0: PUQ_GIC_GRID,
    5: PUQ_KDY_GRID,
