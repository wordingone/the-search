"""
Step 613: LS20 L4 bootstrap fix.

Extends Option C bootstrap to L4 (game_level=3).

Problem: At L3->L4 transition (on_l3), l4_frames=0.
  L4 cluster targeting needs l4_mode built from l4_frames >= WARMUP.
  Without l4_mode clusters, agent has no targets and falls back to argmin.

Fix: Same pattern as L2/L3 bootstrap.
  - During L4 (game_level=3): once l4_frames >= WARMUP AND l3_count >= N_MAP_L4,
    build l4_targets from l4_mode and start targeting
  - L4 uses cluster targeting (same as L0), NOT wall-based BFS nav
  - N_MAP_L4 = 30 (same as other levels)
...
N_MAP_PUQ = 30
N_MAP_L3 = 30
N_MAP_L4 = 20    # Lower threshold for L4: 20 cycles before targeting
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self._puq_lhs_ap = 0
        # Bootstrap flags
        self._puq_bootstrap_done = False   # L3 wall_set built
        self._l4_bootstrap_done = False    # L4 cluster targets first built
    ...
    def _update_bg(self, arr):
        ...
        if self.game_level == 0:
            ...
            if self.l3_frames % MODE_EVERY == 0:
                ...
                # L3 bootstrap: build puq wall_set once enough L3 data accumulated
                if (not self._puq_wall_frozen and not self._puq_bootstrap_done
                    ...
                    print(f"    [L3-bootstrap] puq wall_set built at l3_frames={self.l3_frames} "
            ...
            if self.l4_frames % MODE_EVERY == 0:
                ...
                # L4 bootstrap: build cluster targets once enough L4 data accumulated
                if (not self._l4_bootstrap_done
                        and self.l3_count >= N_MAP_L4 and self.l4_frames >= WARMUP):
                    self.l4_targets = find_isolated_clusters(self.l4_mode)
                    self._l4_bootstrap_done = True
                    print(f"    [L4-bootstrap] l4_targets built: {len(self.l4_targets)} clusters "
                          f"at l4_frames={self.l4_frames} l3_count={self.l3_count}", flush=True)
    ...
    def on_l2(self):
        ...
        self._puq_bfs_path = []; self._puq_avoid_set = set()
        self._puq_death_seen = False; self._puq_xhp_step = False
        # Build puq wall_set if bootstrap condition met
        if (not self._puq_wall_frozen and not self._puq_bootstrap_done
                and self.l2_count >= N_MAP_PUQ and self.l3_frames >= WARMUP):
            self._puq_wall_set = build_wall_set(self.l3_mode)
            self._puq_bootstrap_done = True
            print(f"    [on_l2] puq wall_set built at l2_count={self.l2_count} "
    ...
    def on_l3(self):
        ...
        # Reset l4_targets each time we enter L4 (fresh targeting per episode)
        self.l4_targets = []
    ...
    def act(self):
        # Refresh L0 cluster targets periodically
        ...
        # L4: cluster targeting (bootstrap-dependent)
        if self.game_level == 3:
            if self._l4_bootstrap_done and self._steps_since_l4_detect >= 500:
                self.l4_targets = find_isolated_clusters(self.l4_mode)
...
def main():
    ...
    n_seeds = 2; per_seed_cap = 300
    t_start = time.time()
    print("Step 613: LS20 L4 bootstrap (L3+L4 Option C fix)", flush=True)
    print(f"  L3: puq wall_set from l3_mode (mid-run bootstrap)", flush=True)
    print(f"  L4: cluster targets from l4_mode (mid-run bootstrap)", flush=True)
    ...
    for seed in range(n_seeds):
        ...
        for step in range(1, 1_000_001):
            ...
            if prev_cl == -1:
                if cl == 1:
                    ...
                    if l2_first[0] is None:
                        l2_first[0] = step
                        print(f"  s{seed} L2@{step} l2c={sub.l2_count} "
                              f"boot={sub._puq_bootstrap_done}", flush=True)
                elif cl == 3:
                    sub.on_l3(); l3_steps.append(step)
                    print(f"  s{seed} L3@{step}! l3c={sub.l3_count} "
                          f"l4boot={sub._l4_bootstrap_done}", flush=True)
                elif cl == 4:
            ...
            if time.time() - t_seed > per_seed_cap:
                print(f"  cap @{step} go={go} cyc={sub.l1_cycles} l2c={sub.l2_count} "
                      f"l3c={sub.l3_count} l4c={sub.l4_count} "
                      f"l3boot={sub._puq_bootstrap_done} l4boot={sub._l4_bootstrap_done} "
                      f"deaths={sub._puq_death_count} wp={sub._puq_wp_idx} "
                      f"lhs={sub._puq_lhs_ap}", flush=True)
                break

        R.append({'seed': seed, 'l2c': sub.l2_count, 'l3c': sub.l3_count,
                  'l4c': sub.l4_count, 'l3_steps': l3_steps, 'l4_steps': l4_steps,
                  'puq_wall': sub._puq_wall_frozen, 'l3boot': sub._puq_bootstrap_done,
                  'l4boot': sub._l4_bootstrap_done, 'deaths': sub._puq_death_count,
                  'wp_idx': sub._puq_wp_idx, 'lhs_ap': sub._puq_lhs_ap})

    print(f"\n{'='*60}", flush=True)
    print(f"Step 613: LS20 L4 bootstrap", flush=True)
    ...
    for r in R:
        print(f"  s{r['seed']}: L2={r['l2c']} L3={r['l3c']} L4={r['l4c']} "
              f"l3boot={r['l3boot']} l4boot={r['l4boot']} "
              f"deaths={r['deaths']} wp={r['wp_idx']} lhs={r['lhs_ap']} "
              f"L3@{r['l3_steps'][:3]} L4@{r['l4_steps'][:3]}", flush=True)
    print(f"  total_t={time.time()-t_start:.1f}s", flush=True)
    if l4_wins >= 1:
        print("  SIGNAL: L4 reached! Bootstrap chain works.", flush=True)
    elif l3_wins >= 1:
        print("  L3 reached. L4 needs more bootstrap cycles.", flush=True)
    else:
        print("  L3 not reached in 5 min. Check mgu wall_set activation.", flush=True)
