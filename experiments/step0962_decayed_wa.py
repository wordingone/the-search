"""Step 962: Decayed W_a — bounded Hebbian, tests whether bounding fixes the lock.

FAMILY: Bounded Hebbian (new)

R3 HYPOTHESIS: W_a decay bounds magnitude → scoring depends on direction not
magnitude → lock breaks. State-dependent via h: different states produce
different action rankings because scores can't be dominated by magnitude alone.

ROOT CAUSE (from 961): Positive Hebbian → unbounded W_a[a0] growth →
||W_a[a0]|| >> ||W_a[others]|| → any scorer (dot product, L2) gives consistent
argmax → winner-take-all.

FIX: W_a *= 0.999 per step → leaky accumulator → old updates decay → recent
states dominate → bounded equilibrium magnitude.

One-line change from 948: W_a *= W_A_DECAY before scoring and update.

If WORKS → bounding is the fix → state-dependent action > 800b scalar.
If FAILS → bounded W_a ≈ 800b → no advantage to state-dependent scoring.
...
ETA_ACTION = 0.001
EPSILON = 0.20
W_A_DECAY = 0.999  # per-step decay, half-life ~693 steps
...
class DecayedWa962:
    """948 base + W_a *= 0.999 per step (leaky accumulator, bounded magnitude)."""
    ...
    def process(self, obs):
        ...
        # Decay W_a before scoring and update (bounds magnitude)
        self.W_a *= W_A_DECAY
        ...
        if self._prev_h is not None and self._prev_action is not None:
            ...
            if not np.any(np.isnan(error)):
                ...
                # Hebbian update (same as 948)
                self.W_a[self._prev_action] += ETA_ACTION * delta * self._prev_h

        # Action selection: standard dot product + epsilon (same as 948)
...
def run_game(game_name, n_actions, seeds, n_steps):
    results = []
    for seed in seeds:
        sub = DecayedWa962(n_actions=n_actions, seed=seed)
...
if __name__ == "__main__":
    import os, time
    print("=" * 70)
    print("STEP 962 — DECAYED W_a (W_a *= 0.999 per step, bounded Hebbian)")
    print("=" * 70)
    t0 = time.time()
    ...
    print(f"W_A_DECAY={W_A_DECAY}  ETA_PRED={ETA_PRED}  ETA_ACTION={ETA_ACTION}  EPSILON={EPSILON}")
    print()
