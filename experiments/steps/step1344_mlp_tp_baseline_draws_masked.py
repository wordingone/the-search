"""
Step 1344 — MLP+TP baseline RHAE distribution across 5 draws.
Leo mail 3823, 2026-03-29.

Stop engineering model-based action selection. The MLP+TP baseline (1337 control)
scored RHAE=0.0001 on Game A in step 1343. The baseline IS the signal.

This step runs MLP+TP entropy baseline across 5 independent game draws to measure:
- What fraction of games produce RHAE > 0 with random actions + 92% compression?
- What's the real baseline chain mean? (1 draw was 0.0001. Lucky or typical?)
- What's the variance across draws?

Protocol:
  5 draws × (MBPP + 2 masked ARC) × 2 tries = 30 episodes
  MLP+TP, entropy action selection, 2K steps/try, seed-free
  Each draw uses different game selection (different PRISM seed)
  Report: RHAE(try2) per draw, chain mean, per-game efficiency² distribution

Budget: ~7.5 min (30 episodes × ~15s avg). Within 15-min threshold (Leo, mail 3807).

Constitutional audit: same as 1337. Simplest constitutional substrate.
"""
...
N_DRAWS     = 5
# Different game selections per draw
DRAW_SEEDS  = [STEP * 10 + i for i in range(N_DRAWS)]

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1344')
...
# ---------------------------------------------------------------------------
# Obs encoding (same as 1337-1343)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# MLP-TP substrate (same as 1337 control)
# ---------------------------------------------------------------------------
...
class MlpTpSubstrate:
    """MLP + TP + entropy action selection. Identical to 1337 control."""
...
def main():
    ...
    print(f"Step {STEP} — MLP+TP baseline: {N_DRAWS} draws. RHAE distribution.")
    print(f"Device: {_DEVICE}")
    print(f"Protocol: {N_DRAWS} draws × (MBPP + 2 ARC) × 2 tries = {N_DRAWS*3*2} episodes")
    print(f"MLP+TP entropy actions. 2K steps/try. Draw seeds: {DRAW_SEEDS}")
    print()
    ...
    # Per-game eff² distribution
    all_eff_sq = [r['eff_sq'] for r in all_results]

    print("=" * 80)
    print(f"STEP {STEP} — RESULT (MLP+TP baseline RHAE distribution)")
    print()
    ...
    print("ASSESSMENT:")
    if chain_mean > 1e-5:
        print(f"  >>> SIGNAL: Chain mean RHAE={chain_mean:.6f} > 0 across {N_DRAWS} draws.")
        print(f"  >>> Baseline IS the working substrate. Improve from here.")
    elif nonzero_draws >= 2:
        print(f"  >>> WEAK SIGNAL: {nonzero_draws}/{N_DRAWS} draws non-zero. Baseline occasionally works.")
        print(f"  >>> Chain mean too small ({chain_mean:.6f}) for reliable improvement.")
    elif nonzero_draws == 1:
        print(f"  >>> LUCK: Only 1/{N_DRAWS} draws non-zero. 1343 Game A was likely luck.")
        print(f"  >>> Baseline too weak. Need different approach.")
    else:
        print(f"  >>> ZERO: All draws RHAE=0. 1343 Game A was luck. Baseline is dead.")
    print("=" * 80)
