"""
Step 1261 — Argmin warm-up → GPR transition (conc-gated)

One change from 1260: replace epsilon-random warm-up with argmin warm-up.

  conc = std(via_attention) / (mean(via_attention) + eps)

  if conc < 0.1:
      action = argmin(action_counts)   # systematic coverage, same as Control C
  else:
      action = argmax(|W_salience @ attended_ext_enc|)  # GPR selection

Viability attention accumulates from argmin's systematic exploration.
When conc crosses 0.1, switch to GPR informed by viability spread.

SAL measured ONLY during GPR phase (after conc >= 0.1).
Key: conc trajectory + transition step + VC33 L1 timing.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1261')
...
def main():
    ...
    print("=" * 70)
    print("STEP 1261 — CONCENTRATION-GATED EXPLORATION on 1258 COMPOSITION")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1261 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1261_results.json')
