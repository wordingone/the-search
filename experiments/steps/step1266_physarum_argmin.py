"""
Step 1266 — Physarum flow dynamics + argmin selection (tube-weighted)

Physarum for encoding dynamics (I4). Argmin for coverage (L1).
Tube thickness weights argmin's visit priority.

  enc = centered_obs               # (ENC_DIM,) — C4 centering
  h = tanh(W_h @ h + W_in @ enc)  # (H_DIM,) — C26 recurrent
  ext = [enc, h]                   # (EXT_DIM=320,)

  # Physarum update (after observing enc_after):
...
Thick tubes = high-flow actions = visited sooner by argmin.
Combines 1263 (viability-weighted argmin, SAL=1.00) with 1266 (Physarum I4=0.73).

Parameters: eta=0.01, decay=0.001. No refractory (argmin provides cycling).
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1266')
...
def main():
    ...
    print("=" * 70)
    print("STEP 1266 — PHYSARUM FLOW + ARGMIN SELECTION (TUBE-WEIGHTED)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("tube_thickness = ||W_action||_row. effective_count = action_counts / (1 + tube). action = argmin.")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1266 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1266_results.json')
