"""
Step 1279 — pe as tiebreaker (not additive), full 10-game PRISM chain.

Architectural fix for pe's KB-game bootstrap problem: pe only breaks ties
among minimum-count actions. Coverage DISTRIBUTION is identical to pure argmin.
Only the ORDER of visits within each count level changes.

Selection:
  min_count = visit_count.min()
  candidates = where(visit_count == min_count)
  if len(candidates) == 1: action = candidates[0]
  else: action = candidates[pe_ema[candidates].argmax()]

Property: I3 measures coverage distribution, not visit order → I3 mathematically
preserved. Pe tiebreak can't change which action is selected when there's a unique
minimum.

Composition: PETiebreakerSubstrate (pe_tiebreak — LPL pe as coverage tiebreaker)
...
ETA_FLOW = 0.01      # Hebbian W_action update rate (for R3 measurement)
DECAY = 0.001        # W_action decay (for R3 measurement)
ETA_PRED = 0.01      # W_pred learning rate
PE_EMA_ALPHA = 0.05  # per-action prediction error EMA decay
...
class PETiebreakerSubstrate:
    """Step 1279 — LPL prediction error as tiebreaker for argmin coverage.

    Selection: find minimum-visit-count actions, use pe_ema to break ties
    by preferring the highest-pe action among candidates.
    Coverage distribution is IDENTICAL to pure argmin — only visit order changes.
    ...
    def process(self, obs_raw) -> int:
        ...
        # Pe tiebreaker: always select a minimum-count action,
        # use pe_ema to break ties (prefer highest pe among candidates)
        min_count = self.action_counts.min()
        candidates = np.where(self.action_counts == min_count)[0]
        if len(candidates) == 1:
            action = int(candidates[0])
        else:
            action = int(candidates[self.pe_ema[candidates].argmax()])
        self.action_counts[action] += 1
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    if condition == 'pe_tiebreak':
        substrate = PETiebreakerSubstrate(n_actions=n_actions, seed=seed)
    ...
    while steps < MAX_STEPS:
        ...
        # pe_ema distribution snapshots (pe_tiebreak only)
        if steps in PE_SNAPSHOT_STEPS and condition == 'pe_tiebreak' and hasattr(substrate, 'pe_ema'):
            pe = substrate.pe_ema
            n7 = min(7, n_actions)
            pe_snapshots[steps] = {
    ...
    if condition == 'pe_tiebreak' and r3_snapshot and r3_obs_sample:
    ...
    label = 'PTB' if condition == 'pe_tiebreak' else 'CTL'
    def i1_str(r, tag):
...
def main():
    ...
    print("=" * 70)
    print("STEP 1279 — pe tiebreaker, full 10-game PRISM chain with dual I1")
    print("PETiebreakerSubstrate vs Control C on all 10 solved games.")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Conditions: pe_tiebreak (LPL pe tiebreaker), control_c (pure argmin)")
    ...
    for game_name in GAMES:
        ...
        for condition in ['pe_tiebreak', 'control_c']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'pe_tiebreak' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1279 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1279_results.json'), 'w') as f:
        json.dump({'runs': summary, 'total_elapsed': round(total_elapsed, 1)}, f, indent=2)

    print("I3 + L1 Summary:")
    for game in GAMES:
        for cond in ['pe_tiebreak', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'PTB' if cond == 'pe_tiebreak' else 'CTL'
            i3_rhos = [r['I3_spearman_rho'] for r in runs if r.get('I3_spearman_rho') is not None]
            i3_pass = sum(1 for r in runs if r.get('I3_pass') == True)
            l1s = sum(1 for r in runs if r.get('L1_solved') == True)
            rho_mean = float(np.mean(i3_rhos)) if i3_rhos else 0.0
            print(f"  {game.upper()} {label}: I3={rho_mean:.3f}({i3_pass}/{len(runs)}) L1={l1s}/{len(runs)}")
