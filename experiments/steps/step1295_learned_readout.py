"""
Step 1295 — Phase 2: Learned W_readout on NEG-DIAG Base.

Context: 1294 NEG-DIAG confirmed lock=0.549 < FROZEN baseline 0.551 on FT09.
Active self-suppression (diag=-0.1) forces winner cycling (MI=0.861, k=4.0).
Now W_readout learns to map network states to actions via pe-gated three-factor rule.

Architecture (all conditions):
  N=64, W_drive (64x320) Oja, W_inhibit frozen, W_pred for pe.
  W_recur: pe-gated three-factor + fill_diagonal(W_recur, -0.1)  [NEG-DIAG base, ALL conditions]
  W_readout update (condition-dependent):
    LEARNED-RO:  W_readout[action] += 0.001 * pe * activation  [if pe > pe_threshold]
                 Row normalize after update.
    FROZEN-RO:   W_readout frozen (same as 1294 NEG-DIAG control).
    UNGATED-RO:  W_readout[action] += 0.001 * activation  [always, no pe gate]
                 Row normalize after update.

Central question: Does LEARNED-RO produce L1 on any game?
This is the first R2-compliant configuration with both:
  - Cycling dynamics (NEG-DIAG, MI=0.861, k=4.0)
  - Adaptive readout (W_readout learns state→action mapping)

Kill criteria:
  - LEARNED-RO lock > 0.8 on FT09 → readout re-introduced lock
  - LEARNED-RO I3_cv > 3 × FROZEN-RO I3_cv on 2+ games → catastrophic action concentration
  - UNGATED-RO ≈ LEARNED-RO on all metrics → pe gate unnecessary for readout (direction change)

Known baselines (step1294):
  FT09 NEG-DIAG (=FROZEN-RO): lock=0.549, k=4.0, mi=0.861, corr=-0.008, L1=0

Note: Argmin CTL comparison uses historical data (step1253+) if L1 is achieved.
Budget: 36 ARC runs × ~8s + 3 MBPP × ~5s ≈ 303s. Spec requested 5 draws (360s > cap), using 4.

Spec: Leo mail 3639, 2026-03-28.
"""
...
# Known baselines from step1294 NEG-DIAG (= FROZEN-RO)
FROZEN_RO_BASELINE = {'ft09': {'lock': 0.549, 'k': 4.0, 'mi': 0.861, 'corr': -0.008}}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1295')
...
class LearnedReadoutSubstrate:
    """N=64 NEG-DIAG competitive network with optional W_readout learning.

    All conditions use NEG-DIAG base (fill_diagonal(W_recur, -0.1) always).

    W_readout conditions:
      learned_ro:  W_readout[action] += W_RO_LR * pe * activation  [if pe > pe_threshold]
      frozen_ro:   W_readout frozen (same as 1294 NEG-DIAG)
      ungated_ro:  W_readout[action] += W_RO_LR * activation  [always, no pe gate]
    """
    ...
    def __init__(self, n_actions, seed, mode='learned_ro'):
        ...
        # W_readout: learned or frozen depending on mode
        ...
        # W_readout change tracking (for behavioral R3 signal)
    ...
    def compute_ro_drift(self):
        """Frobenius norm of W_readout - W_readout_init (how much readout has changed)."""
    ...
    def process(self, obs_raw, current_level=0):
        ...
        # W_readout update (condition-dependent)
        act_norm = float(np.linalg.norm(self.activation))
        if act_norm > 1e-8:
            if self.mode == 'learned_ro':
                if pe_scalar > self._pe_threshold and self._pe_threshold > 0.0:
                    self.W_readout[action] += W_RO_LR * pe_scalar * self.activation
                ...
                self.W_readout[action] += W_RO_LR * self.activation
                norms = np.linalg.norm(self.W_readout, axis=1, keepdims=True)
                self.W_readout /= np.maximum(norms, 1.0)
                self.ro_update_count += 1
...
def run_single(game_name, condition, draw, seed, n_actions, solver_level_steps):
    ...
    substrate = LearnedReadoutSubstrate(n_actions=n_actions, seed=seed, mode=condition)
...
def main():
    ...
    print("=" * 72)
    print("STEP 1295 — Phase 2: Learned W_readout on NEG-DIAG Base")
    print("LEARNED-RO:  W_readout[action] += 0.001 * pe * activation  [pe-gated]")
    print("FROZEN-RO:   W_readout frozen (1294 NEG-DIAG baseline)")
    print("UNGATED-RO:  W_readout[action] += 0.001 * activation  [always]")
    ...
    print(f"Known FROZEN-RO (1294 NEG-DIAG): FT09 lock={FROZEN_RO_BASELINE['ft09']['lock']}, "
          f"k={FROZEN_RO_BASELINE['ft09']['k']}, mi={FROZEN_RO_BASELINE['ft09']['mi']}")
    print(f"Central question: Does LEARNED-RO produce L1 on any game?")
    print()
    ...
    print(f"\n{'='*72}")
    print(f"STEP 1295 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*72}\n")
    ...
    # R3 summary (behavioral vs representational)
    print("\nR3 Summary (behavioral R3 expected for LEARNED-RO):")
    ...
    # Lock + MI summary
    print("\nLock + MI Summary (FT09 key metric; FROZEN-RO baseline: lock=0.549):")
    for game in GAMES:
        print(f"\n  {game.upper()}:")
        for cond in CONDITIONS:
            label = LABELS[cond]
            ...
            print(f"    [{label}] lock={lock_display} | mi={mi_display} | sil={sil_display} | ro_drift={drift_display}")
    ...
    # UNGATED-RO vs LEARNED-RO comparison (is pe gate necessary for readout?)
    ...
    for game in ARC_GAMES:
        for metric in ['positive_lock_fraction', 'sequence_mi', 'attractor_silhouette']:
            ...
            if lro_vals and uro_vals:
                ...
                print(f"  {game.upper()} {metric}: LRO={np.mean(lro_vals):.4f} | URO={np.mean(uro_vals):.4f}{eq_flag}")

    with open(os.path.join(RESULTS_DIR, 'step1295_results.json'), 'w') as f:
