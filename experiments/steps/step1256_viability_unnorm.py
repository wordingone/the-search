"""
Step 1256 — Viability Attention Unnormalized (one change from 1255)

One change from Step 1255: remove sum(attention) normalization.

  1255: attended_enc = enc * attention / sum(attention)  [normalized]
  1256: attended_enc = enc * attention                    [unnormalized]

Hypothesis: unnormalized viability attention produces BOTH detectable R3 (>0.05)
AND SAL > 0.3. In 1255, normalization by sum(attention) erased the learned spread
because ALPHA_FLOOR dominated att_sum. Without normalization, the 0.05-0.15 range
per dimension (vs 0.10 floor) creates ~50% Jacobian difference that R3 detects,
while preserving the viability signal that improved SAL from 0.20→0.40.

  1254: pred-error attn + GPR → R3=1.00, SAL=0.20
  1255: viability attn + GPR (normalized) → R3=0.00, SAL=0.40
  1256: viability attn + GPR (unnormalized) → R3=?, SAL=?

If R3 and SAL both pass: first composition where encoding changes are both
measurable (R3) AND action-relevant (SAL).
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1256')
...
class ViaWiringA:
    ...
    def get_internal_repr_readonly(self, obs_raw, frozen_running_mean,
        ...
        attended_enc = enc * attention  # unnormalized (Step 1256)
    ...
    def process(self, obs_raw) -> int:
        ...
        att_sum = attention.sum()
        attended_enc = enc * attention  # 256D — unnormalized (Step 1256)
...
def main():
    ...
    print("=" * 70)
    print("STEP 1256 — VIABILITY ATTENTION UNNORMALIZED (one change from 1255)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("Comparison: 1255 R3=0.00/SAL=0.40 (normalized) vs 1256 (unnormalized). Expect R3 to recover.")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1256 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1256_results.json')
