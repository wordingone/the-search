"""
Step 1268 — Physarum+argmin on SP80/FT09/TR87 with per-level stage instrumentation.

Direction change (Leo/Jun, 2026-03-27): test at L_max, not L1.
Same substrate as 1266 (Physarum+argmin). New games: SP80, FT09, TR87 (working solvers).
Per-level instrumentation: stage metrics computed per level, L_max flagged as design target.

Substrate (same as 1266):
  enc = centered_obs               # (ENC_DIM,) — C4 centering
  h = tanh(W_h @ h + W_in @ enc)  # (H_DIM,) — C26 recurrent
  ext = [enc, h]                   # (EXT_DIM=320,)
  tube_thickness = ||W_action||_row
  effective_count = action_counts / (1 + tube_thickness)
  action = argmin(effective_count)
  Physarum: W_action[last_action] += eta * flow * ext; W_action *= (1-decay)

Per-level instrumentation:
  - action_log segmented by level transitions
  - I3, I4, SAL computed per level window
  - I1 computed using same-level vs different-level repr distances (cross-level natural)
  - R3 snapshot at step 5000 (or end of run)
  - L_max = highest level reached; all stage metrics reported at L_max

Games: SP80 (mixed, 6L), FT09 (click, 6L), TR87 (KB, 6L).
Solver baseline: all 3 have working prescriptions.
"""
...
ETA_FLOW = 0.01       # flow learning rate (tube thickening)
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1268')
...
class PhysarumSubstrate:
    """
    Physarum flow dynamics + argmin selection (tube-weighted).
    Tubes thicken with flow (actual state change), thin over time.
    Argmin weighted by tube thickness preserves coverage.

    Architecture:
      enc (C4) -> h=tanh(W_h@h+W_in@enc) (C26) -> ext=[enc,h]
      tube_thickness = ||W_action||_row  (n_actions,)
      effective_count = action_counts / (1 + tube_thickness)
      action = argmin(effective_count)
      Physarum: W_action[action] += eta * flow * ext; W_action *= (1-decay)

    No argmax. No refractory. Coverage maintained by argmin.
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        # W_action: action = encoding dynamics (Physarum flow-learned)
        ...
        # Argmin visit counts (for tube-weighted argmin)
        self.action_counts = np.zeros(n_actions, np.float32)
        ...
        # Physarum flow history
        self._prev_enc_flow = None   # enc before action, for flow delta
        self._prev_ext = None        # ext = [enc, h] at time of action

        # Last action (for flow update)
    ...
    def process(self, obs_raw) -> int:
        ...
        # Activation (for SAL/repr tracking)
        activation = self.W_action @ ext  # (n_actions,)

        # Tube thickness: L2 norm of each action's row in W_action
        tube_thickness = np.linalg.norm(self.W_action, axis=1)  # (n_actions,)

        # Argmin weighted by tube thickness — coverage preserved, thick tubes visited sooner
        ...
        # Store for Physarum flow update (called externally after env.step)
        self._prev_enc_flow = enc.copy()   # enc before next step's running_mean
        self._prev_ext = ext.copy()        # ext = [enc, h] for flow update
    ...
    def update_flow(self, next_obs_raw):
        """Physarum update: called after env.step with next observation."""
        if self._prev_enc_flow is None or self._last_action is None:
            return
        next_obs = np.asarray(next_obs_raw, dtype=np.float32)
        enc_after = _enc_frame(next_obs) - self.running_mean  # read-only, no mean update
        delta = enc_after - self._prev_enc_flow  # (ENC_DIM,) state change
        flow = float(np.linalg.norm(delta))
        # Tube thickens with flow
        self.W_action[self._last_action] += ETA_FLOW * flow * self._prev_ext
        # All tubes thin over time
...
def compute_r3_jacobian(substrate, obs_sample, frozen_state, n_actions, rng_seed=42):
    ...
    _is_lpl = isinstance(substrate, PhysarumSubstrate)
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    # Per-level tracking
    level_action_logs = {0: []}   # level -> list of actions taken at that level
    level_step_start = {0: 0}     # level -> step when level started
    level_repr_snapshots = {}      # level -> list of repr vectors (sampled)
    current_level_repr = []        # accumulator for current level
    ...
    while steps < MAX_STEPS:
        ...
        # I1 repr log: use activation (Physarum) or enc (CtrlC)
        ...
        level_action_logs.setdefault(level, []).append(action)

        # Per-level repr sampling (every 10 steps within a level)
        step_in_level = steps - level_start_step
        if step_in_level % 10 == 0:
            if hasattr(substrate, '_prev_repr') and substrate._prev_repr is not None:
                current_level_repr.append(substrate._prev_repr.copy())
            elif hasattr(substrate, '_last_enc') and substrate._last_enc is not None:
                current_level_repr.append(substrate._last_enc.copy())

        obs, reward, done, info = env.step(action)
        steps += 1

        # Physarum flow update + state-change tracking for SAL
        if obs is not None and not fresh_episode:
            ...
            if isinstance(substrate, PhysarumSubstrate):
                substrate.update_flow(obs)           # Physarum tube update
                substrate.record_state_change(action, delta)  # SAL tracking
        ...
        if cl > level:
            ...
            # Save repr snapshot for completed level
            level_repr_snapshots[level] = current_level_repr[:]
            current_level_repr = []
            level = cl
            level_start_step = steps
            level_step_start[level] = steps
            level_action_logs.setdefault(level, [])
            substrate.on_level_transition()
    ...
    i4_result = compute_i4(action_log, n_actions)  # global I4

    # Per-level I4 — compute for each level reached
    lmax_level = level
    per_level_i4 = {}
    for lvl, acts in sorted(level_action_logs.items()):
        if len(acts) >= 10:
            li4 = compute_i4(acts, n_actions)
            per_level_i4[int(lvl)] = {
                'reduction_pct': li4['reduction_pct'],
                'pass': li4['pass'],
                'n_actions': len(acts),
            }

    # L_max stage results (use per-level data for L_max level)
    lmax_i4 = per_level_i4.get(lmax_level, {})
    ...
    # Activation stats at end of run (Physarum only)
    act_stats = {}
    if isinstance(substrate, PhysarumSubstrate) and substrate._sal_steps > 0:
    ...
    result = {
        ...
        # Per-level I4 (design target: lmax_level)
        'per_level_i4': per_level_i4,
        'lmax_i4_reduction_pct': lmax_i4.get('reduction_pct'),
        'lmax_i4_pass': lmax_i4.get('pass', False),
        'lmax_i4_n_actions': lmax_i4.get('n_actions'),
    ...
    lmax_str = f"Lmax={lmax_level}"
    lmax_i4_str = f"LmaxI4={lmax_i4.get('reduction_pct', 'null'):.1f}%" if lmax_i4.get('reduction_pct') is not None else "LmaxI4=null"
    ...
    print(f" {lmax_str} | {lmax_i4_str} | {i3_str} | {r3_str} | {sal_str} | {i4_str} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1268 — PHYSARUM+ARGMIN ON SP80/FT09/TR87, PER-LEVEL INSTRUMENTATION")
    print("=" * 70)
    print(f"Games: {GAMES} (working solvers — solver validation step)")
    ...
    print()
    print("DIRECTION CHANGE: Design target = L_max, not L1.")
    print("Per-level I4 instrumentation. lmax_i4 = I4 at highest level reached.")
    print(f"Physarum: eta={ETA_FLOW}, decay={DECAY}. No refractory (argmin provides cycling).")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1268 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    for c in conditions_run:
        print(f"  {'CtrlC' if c == 'control_c' else 'PHY':>8}", end='')
    print()
    print("-" * 35)
    for stage in stages:
        print(f"{stage:<12}", end='')
    ...
    print()
    print("I4 (entropy reduction, global):")
    for game in GAMES:
        for cond in conditions_run:
            runs = [r for r in all_results if r['condition'] == cond and r['game'] == game]
    ...
    for game in GAMES:
        for cond in conditions_run:
            runs = [r for r in all_results if r['condition'] == cond and r['game'] == game]
            levels = [r['lmax_level'] for r in runs]
            label = 'CtrlC' if cond == 'control_c' else 'PHY'
            if levels:
                print(f"  {game.upper()} {label}: Lmax={np.mean(levels):.1f}±{np.std(levels):.1f}  (draws: {levels})")

    print()
    print("L_max I4 (design target):")
    for game in GAMES:
        for cond in conditions_run:
            runs = [r for r in all_results if r['condition'] == cond and r['game'] == game]
            lmax_i4s = [r['lmax_i4_reduction_pct'] for r in runs if r.get('lmax_i4_reduction_pct') is not None]
            passes = [r['lmax_i4_pass'] for r in runs if r.get('lmax_i4_pass') is not None]
            label = 'CtrlC' if cond == 'control_c' else 'PHY'
            if lmax_i4s:
                pass_rate = sum(passes) / len(passes) if passes else 0
                print(f"  {game.upper()} {label}: {np.mean(lmax_i4s):.1f}%±{np.std(lmax_i4s):.1f}%  pass={pass_rate:.2f}")
            else:
                print(f"  {game.upper()} {label}: null")

    out_path = os.path.join(RESULTS_DIR, 'step1268_results.json')
