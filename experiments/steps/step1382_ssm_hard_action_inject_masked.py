"""
Step 1382 — Hard action injection: bypass the gradient entirely.
Leo mail 3946, 2026-03-30.

1379 ACTION_BLIND: SSM ignores action token (ratio=1.00002).
1380 GATE_FAILS: Multiplicative gating degenerates — gradient can't escape action-blind attractor.
1381 INERT: Inverse dynamics fails (ratio=0.999989) — h action-blind fixed point resists all
     gradient interventions. SSM+RTRL+prediction family exhausted.

Fix: bypass gradient entirely. Reserve LAST K=8 dims of h_concat for action identity.
After each SSM state update, OVERWRITE those K dims with a deterministic action encoding.
h_concat = [h_obs (first N_LAYERS*N_STATE - K = 56 dims, RTRL) | h_act (last 8 dims, injected)]

Why this works: action info is in h BY CONSTRUCTION. W_fixed @ h_concat reads both obs features
AND action identity. Obs-action interaction at the readout level, not state transition level.

Action encoding (K=8), using prev_action:
  Keyboard (action < 7):  one_hot in dims 0-6, dim 7=0
  Click (action >= 7):    [1.0, x_norm, y_norm, 0, 0, 0, 0, 0]

Note on RTRL gradient: h_act dims are overwritten post-forward, so gradient through them to B/A
is spurious (h_act is not a function of B). This is a minor inaccuracy — B rows corresponding
to h_act dims receive noisy updates, but since h_act is always overwritten, these rows never
affect the forward pass. Acceptable for this test.

Conditions (paired, 30 draws):
  INJECT:   Hard action injection + frozen projection try2. COUNT try1.
  STANDARD: Normal SSM + frozen projection try2 (= FWD/STANDARD from 1381). COUNT try1.

Diagnostic (10 draws):
  INJECT_MASKED: Same as INJECT but h_act dims zeroed (zeros instead of action encoding).
  Action-blind ratio = pred_loss_try2(INJECT_MASKED) / pred_loss_try2(INJECT).
  By construction should be >> 1.05 if pred head learns to use h_act.

Kill criteria:
  1. action_blind_ratio < 1.05 → injection didn't propagate to pred head (implementation bug).
  2. ratio > 1.05 but INJECT RHAE <= STANDARD RHAE → action info in h doesn't help via W_fixed.
  3. INJECT RHAE > STANDARD (p <= 0.10) → SIGNAL. First genuine feature→action transfer.

Seeds: 14050-14079 (fresh).
"""
...
STEP        = 1382
N_DRAWS     = 30
DRAW_SEEDS  = [14050 + i for i in range(N_DRAWS)]
N_DIAG      = 10
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
...
N_LAYERS      = 2
SSM_LR        = 1e-3
H_DIM         = N_LAYERS * N_STATE   # 64 — full h_concat dim for frozen projection

# Hard action injection
K             = 8         # dims reserved for action encoding in h_concat (last K of last layer's h)

# Frozen projection (same as 1378-1381)
TEMPERATURE   = 3.0

# Thresholds
ACTION_BLIND_THRESHOLD = 0.05   # ratio > 1.05 → pred head uses h_act dims

CONDITIONS     = ['INJECT', 'STANDARD']
DIAG_CONDITION = 'INJECT_MASKED'
...
# ---------------------------------------------------------------------------
# Action injection encoding
# ---------------------------------------------------------------------------

def _encode_action_for_injection(prev_action, n_actions):
    """
    K=8 deterministic encoding using prev_action:
      Keyboard (action < 7):  one_hot in dims 0-6, dim 7=0
      Click (action >= 7):    [is_click=1, x_norm, y_norm, 0, 0, 0, 0, 0]
    For small action spaces (n_actions <= 16): all treated as keyboard types.
    Returns float32 array of shape (K,).
    """
    enc = np.zeros(K, dtype=np.float32)
    if n_actions <= 16:
        # Keyboard-only game: one-hot in dims 0..n_actions-1 (capped at K-1)
        if 0 <= prev_action < K - 1:
            enc[prev_action] = 1.0
    else:
        # Large action space: 0-6 = keyboard types, 7+ = click positions
        if prev_action < 7:
            enc[prev_action] = 1.0        # keyboard one-hot in dims 0-6, dim 7=0
        else:
            click_idx = prev_action - 7
            x_pos = click_idx % 64
            y_pos = click_idx // 64
            enc[0] = 1.0                    # is_click
            enc[1] = float(x_pos) / 63.0   # x_norm
            enc[2] = float(y_pos) / 63.0   # y_norm
            # dims 3-7 = 0
    return enc
...
# ---------------------------------------------------------------------------
# SSM Layer (identical to 1379-1381)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SSM Substrate (with hard action injection)
# ---------------------------------------------------------------------------

class SSMSubstrate:
    """
    mode: 'inject'        — h_concat[-K:] overwritten with encode(prev_action) after SSM forward.
          'standard'      — normal SSM, no injection (= FWD/STANDARD from 1381).
          'inject_masked' — h_concat[-K:] overwritten with zeros (diagnostic baseline).

    The injection overwrites the last K dims of the LAST layer's h after its forward pass,
    then recomputes y = C_last @ h_last (with updated h). RTRL proceeds on the overwritten h:
    C learns to read the injected dims; B/A updates for those dims are spurious but harmless
    since h_act is always overwritten.

    W_fixed: (n_actions, H_DIM=64) frozen projection. Reads full h_concat including h_act dims.
    """
    ...
    def __init__(self, n_actions, mode='standard', W_fixed=None):
        ...
        self._pred_losses      = []
        self._pred_losses_try2 = []   # tracked separately for action-blind ratio
    ...
    def _ssm_forward(self, proj_obs):
        ...
        # Hard action injection: overwrite last K dims of last layer's h, recompute y
        if self.mode in ('inject', 'inject_masked'):
            last_layer = self._layers[-1]
            if self.mode == 'inject':
                act_enc = _encode_action_for_injection(self._prev_action, self.n_actions)
            else:
                act_enc = np.zeros(K, dtype=np.float32)
            last_layer.h[-K:] = act_enc
            y = last_layer.C @ last_layer.h   # recompute y with injected h_act
...
def run_draw(draw_idx, draw_seed, cond_name, max_steps):
    ...
    try2_progress   = {}
    optimal_steps_d = {}
    mode = cond_name.lower()   # 'inject', 'standard', 'inject_masked'
    ...
    for game_idx, (game_name, label) in enumerate(zip(games, game_labels.values())):
        ...
        # Frozen projection: same seed formula as 1378-1381
    ...
    print(f"  [{cond_name}] Draw {draw_idx} RHAE={rhae:.6e}  pred_loss={draw_pred}  pred_try2={draw_pred_t2}")
...
def main():
    ...
    sub_t1 = SSMSubstrate(n_actions=na_t1, mode='inject', W_fixed=W_fixed_t1)
    ...
    # ── Main conditions (INJECT, STANDARD) ───────────────────────────────
    print(f"\n=== STEP {STEP}: INJECT vs STANDARD, {N_DRAWS} draws, {max_steps} steps ===")
    ...
    for cond_name in CONDITIONS:
        ...
        for di, ds in enumerate(DRAW_SEEDS):
            rhae, pred, pred_t2, _ = run_draw(di, ds, cond_name, max_steps)
            rhae_by_cond[cond_name].append(round(rhae, 7))
            pred_by_cond[cond_name].append(pred)
            pred_t2_by_cond[cond_name].append(pred_t2)
        print(f"  Condition done in {time.time()-t_cond:.0f}s")

    # ── Action-blind diagnostic (INJECT_MASKED, 10 draws) ────────────────
    print(f"\n--- Diagnostic: {DIAG_CONDITION} ({N_DIAG} draws, seeds {DRAW_SEEDS[0]}-{DRAW_SEEDS[N_DIAG-1]}) ---")
    diag_rhae = []; diag_pred = []; diag_pred_t2 = []
    t_diag = time.time()
    for di in range(N_DIAG):
        ds = DRAW_SEEDS[di]
        rhae, pred, pred_t2, _ = run_draw(di, ds, DIAG_CONDITION, max_steps)
        diag_rhae.append(round(rhae, 7))
        diag_pred.append(pred)
        diag_pred_t2.append(pred_t2)
    print(f"  Diagnostic done in {time.time()-t_diag:.0f}s")

    # Action-blind ratio: pred_loss_try2(INJECT_MASKED) / pred_loss_try2(INJECT) — first N_DIAG draws
    inject_t2_diag = [p for p in pred_t2_by_cond['INJECT'][:N_DIAG] if p is not None]
    masked_t2_diag = [p for p in diag_pred_t2 if p is not None]
    if inject_t2_diag and masked_t2_diag:
        action_blind_ratio = round(
            float(np.mean(masked_t2_diag)) / float(np.mean(inject_t2_diag)), 4
        )
    else:
    ...
    # Paired sign test: INJECT vs STANDARD
    inject_rhae   = rhae_by_cond['INJECT']
    standard_rhae = rhae_by_cond['STANDARD']
    wins   = sum(1 for i, s in zip(inject_rhae, standard_rhae) if i > s)
    losses = sum(1 for i, s in zip(inject_rhae, standard_rhae) if i < s)
    ...
    print(f"\nAction-blind ratio (INJECT_MASKED/INJECT try2 pred_loss, {N_DIAG} draws): {action_blind_ratio}")

    # Verdict
    if action_blind_ratio is not None and action_blind_ratio < 1.0 + ACTION_BLIND_THRESHOLD:
        verdict = 'INJECTION_FAILED'
    elif wins > losses and p_val <= 0.10:
        verdict = 'SIGNAL'
    elif wins <= losses:
        verdict = 'KILL'
    else:
    ...
    print(f"INJECT   chain_mean: {summary['conditions']['INJECT']['chain_mean']:.6e}")
    print(f"STANDARD chain_mean: {summary['conditions']['STANDARD']['chain_mean']:.6e}")
