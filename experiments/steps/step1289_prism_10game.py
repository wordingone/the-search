"""
Step 1289 — Allosteric softmax selection.

One variable changed from Step 1282: replace argmin(counts - alpha*pe_ema)
with softmax over W_action @ ext (salience).
...
W_pred, flow update: W_action[a] += eta_h * flow * ext; W_action *= (1-DECAY).
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1289')
...
def main():
    ...
    print("=" * 70)
    print("STEP 1289 — Allosteric softmax selection")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1289 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1289_results.json'), 'w') as f:
