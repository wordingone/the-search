"""
Step 1260 — Concentration-gated exploration on 1258 composition

One change from 1258: replace fixed GPR threshold with concentration-gated epsilon.

Bootstrap mechanism:
  Step 0: conc=0 (via_alpha flat). epsilon=1. Pure random exploration.
  Steps 1-N: random exploration builds viability alpha per-action stats.
  Step N+: conc > 0.1. epsilon drops. GPR starts selecting.

  conc = std(via_attention) / (mean(via_attention) + eps)  # coefficient of variation
  epsilon = max(0.1, 1.0 - conc * 10)  # conc=0 -> eps=1 (random), conc=0.1 -> eps=0

Key measurement: conc trajectory at steps 200, 1000, 2000, 5000, 10000.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1260')
...
class ViaWiringA:
    ...
    def process(self, obs_raw) -> int:
        ...
        epsilon = max(0.1, 1.0 - conc * 10.0)

        # C_GPR: Concentration-gated exploration
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    result = {
        ...
        # New (Step 1260): conc trajectory — coefficient of variation at checkpoints
    ...
    # For 1260: show conc trajectory (at_200 → at_10000)
    conc_traj = ""
    ...
    print(f" {l1_str} | {i3_str} | {r3_str} | {sal_str} | {conc_traj} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1260 — CONCENTRATION-GATED EXPLORATION on 1258 COMPOSITION")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1260 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1260_results.json')
