"""
Step 1265 — Physarum flow dynamics substrate: no selector, flow thickens tubes

Same no-selector architecture as 1264 (LPL). One change: replace LPL update
with Physarum flow dynamics. W_action updated by ACTUAL FLOW (what happened),
not temporal prediction (what might follow).
  ...
  # Physarum update on W_action (after observing enc_after):
  delta = enc_after - enc_before   # state change vector (ENC_DIM,)
  flow = ||delta||                 # scalar flow magnitude
  W_action[last_action] += eta * flow * ext     # tube thickens with flow
  W_action *= (1.0 - decay)                     # all tubes thin over time

No LPL. No Hebbian. No prediction. Pure flow reinforcement.
SAL=1.00 (Step 1263) proves per-action state-change correlates with action effects.
Physarum integrates that signal directly into the dynamics.

Parameters: eta=0.01, decay=0.001.
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1265')
...
class PhysarumSubstrate:
    """
    Physarum flow dynamics: W_action IS encoding AND selector.
    ...
    No LPL. No Hebbian. No prediction.
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    while steps < MAX_STEPS:
        ...
        if steps == I3_STEP:
            # For Physarum: action_counts not tracked, use action_log frequency
...
def main():
    ...
    print("=" * 70)
    print("STEP 1265 — PHYSARUM FLOW DYNAMICS: NO SELECTOR, FLOW THICKENS TUBES")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"Physarum: eta={ETA_FLOW}, decay={DECAY}, refractory={REFRACTORY_DECAY}.")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1265 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1265_results.json')
