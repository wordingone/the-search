"""
Step 1386 — Hebbian readout on level transitions (Path B).
Leo mail 3962, 2026-03-30.

Steps 1379-1385: Gradient-based training (RTRL + any prediction objective) converges to
action-blind solutions. The prediction objective route (Path A) is exhausted.

Path B: Bypass gradient for action conditioning. Use reward signal directly.
Mechanism: Hebbian W_read updated when substrate detects level transition (large obs change).
W_read[last_K_actions, :] += alpha * h_concat → associates state patterns with rewarded actions.

Architecture:
- SelectiveSSM (step 1383): 2 layers, D=128, N=32
- Spatial action encoding (step 1385): 9-dim [type_onehot(7), x_norm, y_norm]
- Hard action injection (step 1382): overwrite h_last_layer[-8:] with action encoding after forward
- obs_diff prediction target for RTRL (steps 1384-1385, most action-sensitive)
- W_read (n_actions, 64): Hebbian readout for try2 action selection

Transition detection (self-generated, R1 compliant):
  ||proj_obs_diff|| > running_mean(||proj_obs_diff||) * 10
  Minimum 10 steps of history before triggering.

Hebbian update:
  On transition: W_read[last_K_actions, :] += alpha * h_concat
  K=5 (credit last 5 actions), alpha=0.1
  W_read carries from try1 to try2.

Conditions (30 draws, paired):
  HEBBIAN: SelectiveSSM + injection + spatial + Hebbian W_read readout in try2.
  FROZEN:  SelectiveSSM + injection + spatial + frozen W_fixed readout in try2 (control).
Both: COUNT try1, PRNG fix, h reset, weights carry.
...
STEP        = 1386
N_DRAWS     = 30
DRAW_SEEDS  = [14170 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# SSM dimensions
PROJ_DIM        = 64
SPATIAL_ACT_DIM = 9    # [type_onehot(7), x_norm, y_norm] — for SSM input
INJECT_K        = 8    # hard injection dims in h_last_layer
D_IN            = PROJ_DIM + SPATIAL_ACT_DIM   # 73
D               = 128
N_STATE         = 32
N_LAYERS        = 2
SSM_LR          = 1e-3
H_DIM           = N_LAYERS * N_STATE   # 64 — h_concat for readout

# Frozen projection (FROZEN condition)
TEMPERATURE = 3.0

# Hebbian
HEBBIAN_K          = 5    # credit last K actions on transition
HEBBIAN_ALPHA      = 0.1
TRANSITION_MULTIPLIER = 10.0   # threshold = mean_diff_norm * this
TRANSITION_MIN_STEPS  = 10     # minimum steps before transition detection

CONDITIONS = ['HEBBIAN', 'FROZEN']
...
# ---------------------------------------------------------------------------
# Spatial action encoding (for SSM input)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Hard injection encoding (for h_last_layer[-INJECT_K:])
# ---------------------------------------------------------------------------

def _encode_action_inject(action_id):
    """K=8 encoding for hard injection into state."""
    enc = np.zeros(INJECT_K, dtype=np.float32)
    if action_id < 7:
        enc[action_id] = 1.0
    else:
        click_idx = action_id - 7
...
# ---------------------------------------------------------------------------
# Selective SSM Layer (identical to steps 1383-1385)
# ---------------------------------------------------------------------------
...
class SSMSubstrate:
    """
    mode: 'hebbian' — SelectiveSSM + spatial input + hard injection + Hebbian W_read try2
          'frozen'  — SelectiveSSM + spatial input + hard injection + frozen W_fixed try2
    """

    def __init__(self, n_actions, mode='hebbian', W_fixed=None):
        ...
        # Prediction head (obs_diff target)
        self._W_pred = _det_init(PROJ_DIM, D, scale=0.1)
        self._b_pred = np.zeros(PROJ_DIM, dtype=np.float32)

        # Hebbian readout (HEBBIAN mode only)
        self._W_read = np.zeros((n_actions, H_DIM), dtype=np.float32)

        # Hebbian tracking
        self._last_K_actions  = deque(maxlen=HEBBIAN_K)
        self._diff_norm_sum   = 0.0
        self._diff_norm_count = 0
        self._n_transitions   = 0   # diagnostic: how many transitions triggered
    ...
    def _ssm_forward(self, proj_obs):
        """SSM forward with spatial input encoding + hard injection into last layer."""
        act_emb = _encode_action_spatial(self._prev_action)
        x_in = np.concatenate([proj_obs, act_emb])   # (D_IN=73,)
        ...
        # Hard injection: overwrite last INJECT_K dims of h_last_layer
        last_layer = self._layers[-1]
        inj_enc = _encode_action_inject(self._prev_action)
        last_layer.h[-INJECT_K:] = inj_enc
        # Recompute y with injected h
        y = last_layer.W_out @ (last_layer._C_vec * last_layer.h)
        y = np.nan_to_num(y, nan=0.0, posinf=0.0, neginf=0.0)
        return y

    def _rtrl_step(self, proj_obs_next, y):
        """RTRL with obs_diff target."""
    ...
    def _check_transition_and_update(self, proj_obs_next):
        """Detect obs-magnitude transition; update W_read if HEBBIAN mode."""
        if self._prev_proj_obs is None:
            return
        diff = proj_obs_next - self._prev_proj_obs
        diff_norm = float(np.linalg.norm(diff))

        self._diff_norm_count += 1
        self._diff_norm_sum += diff_norm

        if self.mode != 'hebbian' or self._diff_norm_count < TRANSITION_MIN_STEPS:
            return

        mean_norm = self._diff_norm_sum / self._diff_norm_count
        if mean_norm > 0 and diff_norm > mean_norm * TRANSITION_MULTIPLIER:
            # Transition detected — Hebbian update
            h_concat = np.concatenate([l.h for l in self._layers])
            h_concat = np.nan_to_num(h_concat, nan=0.0)
            for act_idx in self._last_K_actions:
                self._W_read[act_idx] += HEBBIAN_ALPHA * h_concat
            self._n_transitions += 1
            # Reset action tracker (start fresh credit window after transition)
            self._last_K_actions.clear()
    ...
    def process(self, obs_arr):
        ...
        if self._in_try2:
            ...
            logits = W_sel @ h_concat / TEMPERATURE
            logits -= logits.max()
        ...
        self._last_K_actions.append(action)
    ...
    def update_after_step(self, obs_next, action, reward):
        ...
        # Check for transition and potentially update W_read BEFORE RTRL
        self._check_transition_and_update(proj_obs_next)
    ...
    def on_level_transition(self, new_level=None):
        ...
        self._prev_y        = None
        self._prev_proj_obs = None
        # Reset action credit window at level boundary
        self._last_K_actions.clear()
    ...
    def prepare_for_try2(self):
        ...
        self._current_level  = 0
        self._last_K_actions.clear()
        # Reset diff norm stats for try2 (fresh transition detection)
        self._diff_norm_sum   = 0.0
        self._diff_norm_count = 0
        # W_read carries over (Hebbian associations learned in try1)
    ...
    def compute_stage_metrics(self):
        ...
        return {
            ...
            'w_read_norm':    round(float(np.linalg.norm(self._W_read)), 4),
        }
...
def run_draw(draw_idx, draw_seed, cond_name, max_steps):
    ...
    try2_progress   = {}
    optimal_steps_d = {}
    mode = cond_name.lower()   # 'hebbian' or 'frozen'
    ...
    for game_idx, (game_name, label) in enumerate(zip(games, game_labels.values())):
        ...
        # Frozen W_fixed for FROZEN condition (same seed formula)
        ...
        row = {
            ...
            'n_transitions':    stage.get('n_transitions', 0),
            'w_read_norm':      stage.get('w_read_norm', 0.0),
        }
    ...
    n_trans_total = sum(r.get('n_transitions', 0) for r in draw_results)
    w_read_mean   = round(float(np.mean([r.get('w_read_norm', 0.0) for r in draw_results])), 4)

    print(f"  [{cond_name}] Draw {draw_idx} RHAE={rhae:.6e}  pred={draw_pred}  "
          f"transitions={n_trans_total}  w_read_norm={w_read_mean:.4f}")
    return round(rhae, 7), draw_pred, draw_results, n_trans_total
...
def main():
    ...
    sub_t1 = SSMSubstrate(n_actions=na_t1, mode='hebbian', W_fixed=W_fixed_t1)
    ...
    # ── Full experiment: HEBBIAN vs FROZEN, 30 draws ──────────────────────
    print(f"\n=== STEP {STEP}: HEBBIAN vs FROZEN, {N_DRAWS} draws, {max_steps} steps ===")
    print(f"Seeds: {DRAW_SEEDS[0]}-{DRAW_SEEDS[-1]}")

    rhae_by_cond   = {c: [] for c in CONDITIONS}
    pred_by_cond   = {c: [] for c in CONDITIONS}
    trans_by_cond  = {c: [] for c in CONDITIONS}

    for cond_name in CONDITIONS:
        print(f"\n--- Condition: {cond_name} ---")
        t_cond = time.time()
        for di, ds in enumerate(DRAW_SEEDS):
            rhae, pred, _, n_trans = run_draw(di, ds, cond_name, max_steps)
            rhae_by_cond[cond_name].append(round(rhae, 7))
            pred_by_cond[cond_name].append(pred)
            trans_by_cond[cond_name].append(n_trans)
        print(f"  {cond_name} done in {time.time()-t_cond:.0f}s  "
              f"total_transitions={sum(trans_by_cond[cond_name])}")
    ...
    for cond_name in CONDITIONS:
        ...
        n_trans    = sum(trans_by_cond[cond_name])

        print(f"\n{cond_name}: chain_mean={chain_mean:.6e}  nz={nz}/{N_DRAWS}  "
              f"pred_loss={pred_mean}  total_transitions={n_trans}")
    ...
    # Paired sign test: HEBBIAN vs FROZEN
    heb_rhae = rhae_by_cond['HEBBIAN']
    frz_rhae = rhae_by_cond['FROZEN']
    wins   = sum(1 for h, f in zip(heb_rhae, frz_rhae) if h > f)
    losses = sum(1 for h, f in zip(heb_rhae, frz_rhae) if h < f)
    ...
    print(f"HEBBIAN chain_mean: {summary['conditions']['HEBBIAN']['chain_mean']:.6e}")
    print(f"FROZEN  chain_mean: {summary['conditions']['FROZEN']['chain_mean']:.6e}")
