"""
Step 1331 — CNN + Target Propagation + pixel-level mode map (generalization, not memorization)
Leo mail 3770, 2026-03-29.

Step 1330: TP anti-speedup (0.62) on sp80. Substrate memorizes try1's specific pixel layout
and is WORSE on try2's different episode. Same as CNN+Adam anti-speedup (1324: 3/4 games).
Compression without generalization = overfitting to pixel positions.

Hypothesis: mode map discovers WHERE to click from pixel change frequency. Region-based action
selection generalizes across episodes — same game mechanics → same spatial regions →
same active click centroids, regardless of specific pixel positions.

Architecture:
  TP (same as 1329): learned g_i inverses in avg-pooled space + local Adam per layer.
  + pixel-level mode map: track pixel_change_freq on raw obs[0] diffs (no encoding).
    Every 200 steps: connected components on active_mask (change_freq / steps > 5%).
    Action selection: CNN logits MASKED to active region centroids only.
    Mode map PERSISTS across try1→try2 (intentional: regions discovered in try1 pre-seed try2).

Mode map on RAW PIXELS (not encoding). Step 1320 proved encoding-level fails.
Mode map is self-computed from observation diffs (R1 PASS).
No backward mechanism added (R2 PASS).
MBPP / n_actions ≤ 128: mode map inactive (random fallback, same as 1329).

Constitutional audit:
R0: Deterministic init (CNN random init + mode map starts empty, reproducible).
R1: PASS — pixel change freq from obs diffs, self-computed.
R2: PASS — TP local targets + mode map from observation changes, no global backward.
R3: TP weights modify + mode map accumulates. Both update from ongoing observation.
R4: second_exposure_speedup — mode map pre-seeds try2 with regions from try1.
R5: PASS.
R6: Remove mode map → fall back to full action space (step 1330 TP baseline).

Kill:   speedup ≤ 1.0 → regions don't generalize → KILL
Signal: speedup > 1.0 (finite) → substrate generalizes through spatial regions → SIGNAL
"""
...
GAMES, GAME_LABELS = select_games(seed=1331)
...
CONDITIONS  = ['tp_mode']
LABELS      = {'tp_mode': 'TP+ModeMap'}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1331')
...
_MODE_MAP_FREQ   = 200    # recompute connected components every N steps
_CHANGE_THRESH   = 0.05   # pixel must change in >5% of steps to be "active"


# ---------------------------------------------------------------------------
# CNN model (same as steps 1323/1326/1329)
# ---------------------------------------------------------------------------
...
class TpCnnSubstrate:
    ...
    def _tp_train_step(self):
        ...
        # 2. Per-layer g_i reconstruction updates
        recon_pairs = [('g4', avg4, avg3), ('g3', avg3, avg2), ('g2', avg2, avg1)]
    ...
    def get_mode_map_stats(self):
        return {'n_regions': 0, 'n_active_clicks': 0, 'mode_map_step': 0}


# ---------------------------------------------------------------------------
# TP + Mode Map substrate
# ---------------------------------------------------------------------------

class TpModeMapSubstrate(TpCnnSubstrate):
    """TP + pixel-level mode map for action space reduction.

    Mode map tracks pixel change frequency on raw obs[0] (64×64 first channel).
    Every 200 steps: connected components on change_freq / total_steps > 5% mask.
    Action selection: CNN logits masked to active region centroids only.

    IMPORTANT: mode map persists across try1→try2 (reset_loss_tracking does NOT reset it).
    This is intentional: regions discovered in try1 pre-seed try2's exploration.
    If regions generalize across episodes, try2 starts with better-targeted actions.
    """

    def __init__(self, n_actions):
        super().__init__(n_actions)
        self._pixel_change_freq = np.zeros((64, 64), dtype=np.float32)
        self._prev_raw_obs      = None
        self._active_sg_indices = []
        self._mode_map_step     = 0
        self._mode_map_n_regions = 0

    def reset_loss_tracking(self):
        # Reset loss tracking ONLY — mode map intentionally persists to try2
        self._step = 0
        self._recent_losses.clear()
        self._pred_loss_at = {ck: None for ck in LOSS_CHECKPOINTS}

    def on_level_transition(self):
        # Reset obs pointers — mode map accumulation persists across level transitions
        self._prev_one_hot = None
        self._prev_sg_idx  = None
        self._prev_raw_obs = None
    ...
    def process(self, obs_arr):
        ...
        # Update pixel change frequency from raw obs first channel
        raw_ch0 = obs_arr[0]  # (64, 64) integer values 0–15
        if self._prev_raw_obs is not None:
            delta = np.abs(raw_ch0 - self._prev_raw_obs)
            self._pixel_change_freq += (delta > 0).astype(np.float32)
        self._prev_raw_obs = raw_ch0.copy()

        # Recompute active regions every MODE_MAP_FREQ steps
        if (self._mode_map_step % _MODE_MAP_FREQ == 0
                and self._mode_map_step >= _MODE_MAP_FREQ):
        ...
        if self._active_sg_indices:
            # Mask: only allow click positions that are active region centroids
            cp_masked = torch.zeros_like(cp)
            for sg in self._active_sg_indices:
                p = sg - 5
                if 0 <= p < 4096:
                    cp_masked[p] = cp[p]
            combined = torch.cat([ap, cp_masked])
        else:
            # No regions yet: full action space (same as base TP)
            combined = torch.cat([ap, cp])

        total = combined.sum().item()
        combined = (combined / (total + 1e-12) if total > 1e-12
                    else torch.ones(_SG_OUT_DIM) / _SG_OUT_DIM)
        sg_idx = int(torch.multinomial(combined, 1).item())
        self._prev_sg_idx = sg_idx
        return self._sg_to_prism(sg_idx)

    def _update_active_regions(self):
        """Recompute active click centroids from accumulated pixel change frequency."""
        try:
            from scipy.ndimage import label as nd_label, center_of_mass
        except ImportError:
            return

        freq_rate   = self._pixel_change_freq / max(float(self._mode_map_step), 1.0)
        active_mask = (freq_rate > _CHANGE_THRESH).astype(np.uint8)
        ...
        labeled, n_regions = nd_label(active_mask)
        if n_regions == 0:
            self._active_sg_indices  = []
            self._mode_map_n_regions = 0
            return

        raw_centroids = center_of_mass(active_mask, labeled,
                                        list(range(1, n_regions + 1)))
        # center_of_mass returns a tuple (not a list) when index has 1 element
        if isinstance(raw_centroids, tuple) and not isinstance(raw_centroids[0], tuple):
            raw_centroids = [raw_centroids]

        active_sg = []
        for c in raw_centroids:
            row = int(round(float(c[0])))
            col = int(round(float(c[1])))
        ...
        self._active_sg_indices  = active_sg
        self._mode_map_n_regions = n_regions

    def get_mode_map_stats(self):
        return {
            'n_regions':       self._mode_map_n_regions,
            'n_active_clicks': len(self._active_sg_indices),
            'mode_map_step':   self._mode_map_step,
        }
...
def run_episode(env, substrate, n_actions, solver_level_steps, seed):
    ...
    mm_stats   = substrate.get_mode_map_stats()
    ...
    return {
        ...
        'mode_map_n_regions': mm_stats['n_regions'],
        'mode_map_n_clicks':  mm_stats['n_active_clicks'],
    }
...
def run_game(game_name, condition, solver_level_steps):
    ...
    substrate = TpModeMapSubstrate(n_actions=n_actions)

    result_try1 = run_episode(env, substrate, n_actions, solver_level_steps, seed=SEED_A)
    substrate.reset_loss_tracking()  # mode map persists
...
def main():
    ...
    print(f"Step {STEP} — CNN + Target Propagation + pixel-level mode map")
    print(f"Games: {masked_game_list(GAME_LABELS)} (ARC games masked)")
    print(f"Device: {_DEVICE}")
    print(f"TP: learned g_i inverses in avg-pooled space + local Adam")
    print(f"ModeMap: pixel change freq on obs[0] → connected components every {_MODE_MAP_FREQ} steps")
    print(f"         threshold={_CHANGE_THRESH}. Mode map persists across try1→try2.")
    print(f"Kill:   speedup ≤ 1.0  (regions don't generalize)")
    print(f"Signal: speedup > 1.0  (finite — substrate generalizes through spatial regions)")
    print(f"3 runs (3 games × 1 condition), try1+try2 ({MAX_STEPS} steps each).")
    print()
    ...
    if first_arc:
        ...
        try:
            ...
            na_t = 4103
        sub_t = TpModeMapSubstrate(n_actions=na_t)
        ...
        t_100 = time.time() - t0
        est_2k    = t_100 * 20
        est_total = est_2k * 2 * len(GAMES) * len(CONDITIONS)
    ...
    for game in GAMES:
        ...
        for condition in CONDITIONS:
            ...
            print(f"  try1: mode_map={mm1['mode_map_n_regions']} regions "
                  f"({mm1['mode_map_n_clicks']} clicks)")
            print(f"  try2: mode_map={mm2['mode_map_n_regions']} regions "
                  f"({mm2['mode_map_n_clicks']} clicks)")
    ...
    sep = '=' * 100
    print(f"\n{sep}")
    print(f"STEP {STEP} — RESULT (TP + pixel mode map)\n")
    ...
    for r in all_results:
        ...
        mm1   = f"regions={r['try1']['mode_map_n_regions']},clicks={r['try1']['mode_map_n_clicks']}"
        print(f"  {label}: cr={cr}  {mm1}  traj={traj}")
    print()

    print("Mode map discovery per game:")
    for r in all_results:
        label = GAME_LABELS.get(r['game'], '?')
        sp    = r['second_exposure_speedup']
        mm1   = r['try1']['mode_map_n_regions']
        mm2   = r['try2']['mode_map_n_regions']
        n1    = r['try1']['mode_map_n_clicks']
        n2    = r['try2']['mode_map_n_clicks']
        print(f"  {label}: speedup={format_speedup(sp)} | "
              f"try1 {mm1}R/{n1}C → try2 {mm2}R/{n2}C")
    print()

    # Kill / Signal assessment
    tp_speedup = final_speedup.get('tp_mode')
    all_speedups = speedup_by_condition.get('tp_mode', [])

    print("KILL/SIGNAL ASSESSMENT:")
    if tp_speedup is None and all(s is None or s == float('inf') for s in all_speedups):
        print(f"  >>> INCONCLUSIVE: no finite speedup in any game (no game reached L1 in both tries).")
    elif tp_speedup == float('inf') or tp_speedup is None:
        inf_games = [GAME_LABELS.get(r['game'], '?') for r in all_results
                     if r['second_exposure_speedup'] == float('inf')]
        print(f"  >>> INCONCLUSIVE: speedup=inf ({inf_games}) — try1 failed, try2 succeeded.")
        print(f"  >>> Cannot confirm generalization: try1 must succeed for speedup to be meaningful.")
    elif tp_speedup > 1.0:
        print(f"  >>> SIGNAL: speedup={tp_speedup:.4f} > 1.0")
        print(f"  >>> Spatial region generalization confirmed. Mode map reduces anti-speedup.")
        print(f"  >>> Next: full 10-game validation, confirm signal is not stochastic.")
    elif tp_speedup <= 1.0:
        zero_games = [GAME_LABELS.get(r['game'], '?') for r in all_results
                      if r['second_exposure_speedup'] == 0.0]
        anti_games = [GAME_LABELS.get(r['game'], '?') for r in all_results
                      if r['second_exposure_speedup'] is not None
                      and 0 < r['second_exposure_speedup'] < 1.0]
        print(f"  >>> KILL: speedup={tp_speedup} ≤ 1.0")
        print(f"  >>> Regions don't generalize: anti-speedup persists with mode map.")
        if anti_games:
            print(f"  >>> Anti-speedup games: {anti_games}")
        if zero_games:
            print(f"  >>> Zero-speedup games (try1 L1, try2 fail): {zero_games}")
    print(sep)
