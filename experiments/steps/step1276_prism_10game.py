"""
Step 1276 — LPL prediction error as action selection signal, full 10-game PRISM chain.

W_pred (256×256) predicts enc_{t+1} from enc_t.
pe_a = EMA of ||enc_{t+1} - W_pred @ enc_t|| per action.
Selection: argmin(visit_count - alpha * pe_ema), alpha=0.1.

Loop actions: become predicted → pe drops → suppressed.
Novel-destination actions: stay unpredicted → pe stays high → prioritized.
W_action maintained via Hebbian for R3 measurement.

Composition: LPLPESubstrate (lpl_pe — LPL prediction error)
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1276')
...
class LPLPESubstrate:
    """Step 1276 — LPL prediction error as action selection signal.
    ...
    def process(self, obs_raw) -> int:
        ...
        # LPL pe selection: argmin(visit_count - alpha * pe_ema)
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    r3_snapshot = None
    r3_obs_sample = None
    pe_snapshots = {}  # step → pe_ema snapshot (lpl_pe only)
    ...
    while steps < MAX_STEPS:
        ...
        # pe_a distribution snapshots (lpl_pe only)
    ...
    label = 'LPE' if condition == 'lpl_pe' else 'CTL'
    def i1_str(r, tag):
...
def main():
    ...
    print("=" * 70)
    print("STEP 1276 — LPL prediction error, full 10-game PRISM chain with dual I1")
    print("LPLPESubstrate vs Control C on all 10 solved games.")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Conditions: lpl_pe (LPL prediction error), control_c (pure argmin)")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1276 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1276_results.json'), 'w') as f:
