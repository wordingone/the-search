"""
Step 1288 — Encoding-change momentum.

One variable changed from Step 1282: add momentum rule to action selection.

Mechanism:
  enc_change = ||enc_t - enc_{t-1}||
  threshold = running_median(enc_change, window=100)

  if enc_change > threshold AND prev_enc available:
      a_t = a_{t-1}   # repeat last action (momentum)
  else:
      a_t = argmin(counts - alpha * pe_ema)   # normal selection

Everything else unchanged: LPL (eta_h=0.05), pe_ema (alpha=0.1), centered encoding C4.

Motivation: I4=0 everywhere (no temporal structure). Actions selected independently.
Momentum creates temporal blocks of repeated effective actions → non-random temporal structure.
Adaptive threshold (running median) scales universally across game types.

Stage predictions:
  I4: IMPROVE (temporal blocks)
  R3: MAINTAIN (LPL unchanged)
  I3: POSSIBLE DEGRADATION (less coverage during momentum blocks)
  I1: MAINTAIN (encoding modification unchanged)

Kill criteria:
  R3 regression on >2 games → KILL
  I3_cv regression >20% on any game with no I4 improvement → KILL
...
MOMENTUM_WINDOW = 100   # running median window for enc_change threshold
MOMENTUM_MIN_SAMPLES = 10  # require at least this many samples before enabling momentum
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1288')
...
CONDITIONS = ['momentum', 'lpl_pe_ctrl']
LABELS = {'momentum': 'MOM', 'lpl_pe_ctrl': 'CTL'}
...
class LPLPESubstrate:
    """LPL PE substrate with optional encoding-change momentum.

    Momentum condition: repeat last action when enc_change > running_median(window=100).
    Control condition: standard argmin(counts - alpha * pe_ema).
    """

    def __init__(self, n_actions: int, seed: int, use_momentum: bool = False):
        ...
        # Momentum state
        self._prev_enc_momentum = None  # enc from previous step (for change detection)
        self._enc_change_window = deque(maxlen=MOMENTUM_WINDOW)  # running window for median
        ...
        # Momentum tracking stats
        self.momentum_activations = 0
        self.momentum_total_checks = 0
    ...
    def process(self, obs_raw) -> int:
        ...
        if self.use_momentum and self._prev_enc_momentum is not None and self._last_action is not None:
            enc_change = float(np.linalg.norm(enc - self._prev_enc_momentum))
            self._enc_change_window.append(enc_change)
            self.momentum_total_checks += 1

            if len(self._enc_change_window) >= MOMENTUM_MIN_SAMPLES:
                threshold = float(np.median(self._enc_change_window))
                if enc_change > threshold:
                    action = self._last_action  # momentum: repeat last action
                    self.momentum_activations += 1
                else:
                    score = self.action_counts - SELECTION_ALPHA * self.pe_ema
                    action = int(np.argmin(score))
            else:
                # Not enough samples yet — use normal selection
                score = self.action_counts - SELECTION_ALPHA * self.pe_ema
                action = int(np.argmin(score))
        else:
            score = self.action_counts - SELECTION_ALPHA * self.pe_ema
        ...
        if self.use_momentum:
            self._prev_enc_momentum = enc.copy()
    ...
    def on_level_transition(self):
        self._prev_enc_flow = None
        self._prev_ext = None
        self._prev_enc_momentum = None  # reset momentum context on transition
...
# --- Instrumentation (identical to step1282) ---
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_mom = (condition == 'momentum')
    substrate = LPLPESubstrate(n_actions=n_actions, seed=seed, use_momentum=use_mom)
    env = make_game(game_name)
    obs = env.reset(seed=seed)
    ...
    momentum_rate = round(substrate.momentum_activations / max(substrate.momentum_total_checks, 1), 4) if use_mom else None
    ...
    mom_str = f" mom={momentum_rate:.3f}" if momentum_rate is not None else ""
    arc_str = f"ARC={arc_score:.4f}" if arc_score > 0 else "ARC=0"
    print(f" [{label}] Lmax={max_level} | {i3_str} | {i4_str} | {r3_str} |{mom_str} {arc_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'momentum_activations': substrate.momentum_activations if use_mom else 0,
        'momentum_total_checks': substrate.momentum_total_checks if use_mom else 0,
...
def main():
    ...
    print("=" * 70)
    print("STEP 1288 — Encoding-change momentum")
    print("Mechanism: repeat action when enc_change > running_median(window=100)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1288 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    print("\nMomentum activation rates (MOM condition only):")
    for game in GAMES:
        runs = [r for r in all_results if r['game'] == game and r['condition'] == 'momentum']
        rates = [r['momentum_rate'] for r in runs if r.get('momentum_rate') is not None]
        mean_rate = float(np.mean(rates)) if rates else None
        print(f"  {game.upper()}: {mean_rate:.3f}" if mean_rate is not None else f"  {game.upper()}: null")

    with open(os.path.join(RESULTS_DIR, 'step1288_results.json'), 'w') as f:
