"""
Step 1292 — Winnerless competition (Lotka-Volterra dynamics).

New architecture: W_action repurposed as W_drive (input coupling from encoding
to action activation). Action = argmax of Lotka-Volterra dynamical state.
No pe_ema, no argmin, no refractory, no visit counts.

Mechanism (LV condition):
  drive = W_action @ ext                                  # input drive per action
  total_act = activation.sum()
  inhibit = gamma * (total_act - activation)              # O(n) global inhibition
  d_act = activation * (drive - inhibit)                  # Lotka-Volterra
  activation = max(activation + DT * d_act, 0)            # non-negative
  action = argmax(activation)                             # instantaneous winner
  # Hebbian learning on winning action's drive row:
  W_action[action] += ETA_LV * activation[action] * ext
  W_action[action] -= ETA_LV * mean(W_action[action])    # anti-Hebbian decorr

W_action is the drive coupling (W_drive in Leo's spec); field name kept for R3
continuity with steps 1289-1291.

Implementation note: Leo's spec uses full (n_actions x n_actions) W_inhibit
initialized as (1-I)*U(0.5,1.5), but this is O(n^2) = 67MB and ~170s/run for
n=4103. Using O(n) global inhibition: gamma ~ U(0.5,1.5)/n_actions preserves
correct scale (inhibition ~ mean(activation) regardless of n_actions).
Key properties preserved: competitive suppression, no self-inhibition,
scale-invariant with n_actions. Heterogeneous W_inhibit deferred to later step
if cycling is confirmed.

Kill criteria (per Leo spec):
  I3_cv > CTL x 2.0 on 5+ games → dynamics not cycling (collapsed to fixed point)
  Zero L1 on ALL games AND no I4 improvement → dynamics inert
  W_action max > 1000 → add row normalization
...
PE_EMA_ALPHA = 0.05
SELECTION_ALPHA = 0.1
ETA_H = 0.05  # kept for CTL W_action LPL update
DT = 0.1       # LV integration step
ETA_LV = 0.01  # Hebbian learning rate on W_drive (W_action in LV condition)
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1292')
...
CONDITIONS = ['lv', 'lpl_pe_ctrl']
LABELS = {'lv': 'LV', 'lpl_pe_ctrl': 'CTL'}
...
class LPLLVSubstrate:
    """LPL substrate with Lotka-Volterra competition (LV) or argmin+pe_ema (CTL).

    LV condition: action = argmax(activation), where activation evolves via
                  Lotka-Volterra: d_act = activation * (W_action@ext - gamma*(sum-activation)).
                  W_action updated Hebbianly on winner with anti-Hebbian decorr.
    CTL condition: action = argmin(counts - alpha * pe_ema), W_action via LPL.
    """

    def __init__(self, n_actions: int, seed: int, use_lv: bool = False):
        ...
        # LV-only fields
        # activation: LV dynamical state, initialized small uniform
        self.activation = self._rng.uniform(0, 0.1, n_actions).astype(np.float32)
        # gamma: global inhibition strength per competitor, normalized by n_actions
        # Equivalent to W_inhibit ~ (1-I)*U(0.5,1.5) but O(n) computation
        self.gamma = float(rng_w.uniform(0.5, 1.5)) / float(n_actions)
        ...
        # LV cycle stats (LV only)
        self._lv_stats = []        # [(step, n_distinct_winners, max_act, mean_act)] every 100 steps
        self._winners_window = []  # rolling 100-step window for cycle detection
    ...
    def process(self, obs_raw) -> int:
        ...
        if self.use_lv:
            # Lotka-Volterra competitive dynamics (O(n), no self-inhibition)
            drive = salience  # W_action @ ext already computed
            total_act = float(self.activation.sum())
            inhibit = self.gamma * (total_act - self.activation)
            d_act = self.activation * (drive - inhibit)
            self.activation = np.maximum(self.activation + DT * d_act, 0.0)
            action = int(np.argmax(self.activation))

            # Hebbian learning on winning action (anti-Hebbian decorrelation)
            win_act = float(self.activation[action])
            self.W_action[action] += ETA_LV * win_act * ext
            self.W_action[action] -= ETA_LV * float(np.mean(self.W_action[action]))

            # Cycle stats (check for W explosion and cycling)
            self._winners_window.append(action)
            if len(self._winners_window) > 100:
                self._winners_window.pop(0)
            if self.step % 100 == 0:
                n_distinct = len(set(self._winners_window))
                max_act = float(np.max(self.activation))
                mean_act = float(np.mean(self.activation))
                max_w = float(np.max(np.abs(self.W_action)))
                self._lv_stats.append((self.step, n_distinct, round(max_act, 6),
                                       round(mean_act, 6), round(max_w, 4)))
        else:
    ...
    def update_flow(self, next_obs_raw):
        ...
        if not self.use_lv:
            # CTL only: LPL Hebbian update. LV updates W_action in process() instead.
    ...
    def mean_cycle_stats(self):
        """Return (mean_n_distinct_winners, mean_max_activation) across logged steps."""
        if not self._lv_stats:
            return None, None
        n_distinct = float(np.mean([s[1] for s in self._lv_stats]))
        max_act = float(np.mean([s[2] for s in self._lv_stats]))
        return n_distinct, max_act
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_lv = (condition == 'lv')
    substrate = LPLLVSubstrate(n_actions=n_actions, seed=seed, use_lv=use_lv)
    env = make_game(game_name)
    obs = env.reset(seed=seed)
    ...
    mean_cycle_distinct, mean_max_act = substrate.mean_cycle_stats()
    ...
    cyc_str = f" cyc={mean_cycle_distinct:.1f}" if mean_cycle_distinct is not None else ""
    arc_str = f"ARC={arc_score:.4f}" if arc_score > 0 else "ARC=0"
    print(f" [{label}] Lmax={max_level} | {i3_str} | {i4_str} | {r3_str} |{cyc_str} {arc_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'seed': seed,
        'use_lv': use_lv,
        'mean_cycle_distinct': round(mean_cycle_distinct, 2) if mean_cycle_distinct is not None else None,
        'mean_max_activation': round(mean_max_act, 6) if mean_max_act is not None else None,
        'lv_stats': substrate._lv_stats if hasattr(substrate, '_lv_stats') else None,
        'steps_taken': steps,
...
def main():
    ...
    print("=" * 70)
    print("STEP 1292 — Winnerless competition (Lotka-Volterra dynamics)")
    print("Mechanism: d_act = activation * (W_action@ext - gamma*(sum-act)); action = argmax(act)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1292 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    print("\nMean cycling (LV condition only):")
    for game in GAMES:
        runs = [r for r in all_results if r['game'] == game and r['condition'] == 'lv']
        cycs = [r['mean_cycle_distinct'] for r in runs if r.get('mean_cycle_distinct') is not None]
        mean_cyc = float(np.mean(cycs)) if cycs else None
        max_acts = [r['mean_max_activation'] for r in runs if r.get('mean_max_activation') is not None]
        mean_mact = float(np.mean(max_acts)) if max_acts else None
        print(f"  {game.upper()}: cyc={mean_cyc:.1f} max_act={mean_mact:.4f}" if mean_cyc is not None else f"  {game.upper()}: null")

    with open(os.path.join(RESULTS_DIR, 'step1292_results.json'), 'w') as f:
