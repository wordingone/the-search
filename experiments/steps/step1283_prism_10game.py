"""
Step 1283 — temporal prediction error: W_pred on [enc;h] (320 dims) vs enc only (256 dims).

Recurrent state (C21/C26) is already in the substrate since Step 1276. W_pred currently
predicts enc_{t+1} from enc_t only (256×256). This is temporally blind: after A→B vs D→E→C,
the prediction for next action is identical regardless of recent history.

Temporal PE: expand W_pred input to [enc_t; h_t] (320 dims). Now prediction depends on
recent action history encoded in h_t. On LS20 after A→B, action C produces unexpected
encoding change → high pe → prioritized. After D→E, C produces expected change → low pe.

Conditions:
  temporal_pe — W_pred (256×320) input: [enc_t; h_t]
  lpl_pe      — W_pred (256×256) input: enc_t only (Step 1282 eta_h=0.05 baseline)

Both conditions: eta_h=0.05, alpha=0.1, frozen W_h/W_in.
...
PE_EMA_ALPHA = 0.05
SELECTION_ALPHA = 0.1
ETA_H = 0.05  # Fixed from Step 1282 — both conditions use eta_h=0.05
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1283')
...
class LPLPESubstrate:
    """LPL PE substrate with optional temporal prediction error.

    baseline (lpl_pe): W_pred (256×256) predicts enc_{t+1} from enc_t.
    temporal_pe:       W_pred (256×320) predicts enc_{t+1} from [enc_t; h_t].

    Temporal PE makes prediction error sensitive to recent action history:
    same action produces different PE depending on preceding context in h_t.
    """

    def __init__(self, n_actions: int, seed: int, use_temporal_pe: bool = False):
        ...
        self._prev_enc = None
        self.step = 0
        # W_pred input dim depends on condition
        pred_in_dim = EXT_DIM if use_temporal_pe else ENC_DIM  # 320 or 256
        self.W_pred = rng_w.randn(ENC_DIM, pred_in_dim).astype(np.float32) * 0.01
    ...
    def update_flow(self, next_obs_raw):
        ...
        # Temporal PE: predict from [enc;h]. Baseline: predict from enc only.
        pred_input = self._prev_ext if self.use_temporal_pe else self._prev_enc_flow
        pred_enc = self.W_pred @ pred_input
        pe = float(np.linalg.norm(enc_after - pred_enc))
        pred_error = enc_after - pred_enc
        self.W_pred += ETA_PRED * np.outer(pred_error, pred_input)
        a = self._last_action
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_temporal = (condition == 'temporal_pe')
    substrate = LPLPESubstrate(n_actions=n_actions, seed=seed, use_temporal_pe=use_temporal)
    ...
    label = 'TPE' if condition == 'temporal_pe' else 'LPE'
...
def main():
    ...
    print("=" * 70)
    print("STEP 1283 — temporal PE (W_pred 256×320) vs baseline (W_pred 256×256)")
    print("Temporal PE: prediction from [enc_t; h_t] — context-sensitive PE signal")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Draws: {N_DRAWS} per condition per game = {N_DRAWS * 2 * len(GAMES)} total runs")
    print(f"eta_h={ETA_H} (both conditions), alpha={SELECTION_ALPHA}")
    print()
    ...
    for game_name in GAMES:
        ...
        for condition in ['temporal_pe', 'lpl_pe']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'temporal_pe' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1283 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # --- Summary ---
    print("R3 Summary:")
    for game in GAMES:
        for cond in ['temporal_pe', 'lpl_pe']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'TPE' if cond == 'temporal_pe' else 'LPE'
    ...
    for game in GAMES:
        for cond in ['temporal_pe', 'lpl_pe']:
            ...
            print(f"  {game.upper()} {label}: I4={mean_i4:.2f}% ({passes}/5)" if mean_i4 is not None else f"  {game.upper()} {label}: I4=null")
    ...
    with open(os.path.join(RESULTS_DIR, 'step1283_results.json'), 'w') as f:
