"""
Step 1263 — Viability-weighted argmin (learned projection)

Drop GPR and concentration gating entirely. Return to pure argmin.
One change: argmin is biased by LEARNED viability alpha (not frozen random).

  salience = via_alpha @ attended_mod[:ENC_DIM]  # (n_actions,) — learned
  effective_count = action_counts / (1.0 + |salience|)
  action = argmin(effective_count)

via_alpha[a, d] = EMA of |enc_change[d]| when action a was taken.
It IS the learned projection from encoding space to action space.
High-salience actions get lower effective count -> visited sooner.
Coverage preserved (argmin still visits all actions eventually).

Not 938e: 938e used frozen random R. This uses via_alpha (learned from
game responses). SAL=0.36 on LS20 proves via_alpha correlates with action
effects — the bias is informed, not noise.

Everything else from 1258: combined attention (C25 * viability)
post-recurrent on ext_enc (320D). No GPR. No transition.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1263')
...
class ViaWiringA:
    ...
    def process(self, obs_raw) -> int:
        ...
        # Viability-weighted argmin: argmin biased by learned via_alpha projection
        # via_alpha[a, d] = EMA(|enc_change[d]| when action a taken) — (n_actions, ENC_DIM)
        # attended_mod[:ENC_DIM] is the enc slice of the 320D attended representation
        salience = self.via_alpha @ attended_mod[:ENC_DIM]  # (n_actions,) — learned
        effective_count = self.action_counts / (1.0 + np.abs(salience))
        action = int(np.argmin(effective_count))

        # Argmin counterfactual (how often does salience bias change the choice?)
        ...
        # SAL tracking — all steps (salience is always computed)
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    dis_str = f"dis={substrate._argmin_disagree/substrate._argmin_total:.2f}" if isinstance(substrate, ViaWiringA) and substrate._argmin_total > 0 else ""
    print(f" {l1_str} | {i3_str} | {r3_str} | {sal_str} | {dis_str} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1263 — VIABILITY-WEIGHTED ARGMIN (LEARNED) on 1258 COMPOSITION")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("No GPR. No transition. No conc gating. Argmin biased by LEARNED via_alpha.")
    print("salience = via_alpha @ attended_mod[:ENC_DIM]")
    print("effective_count = action_counts / (1 + |salience|). action = argmin(effective_count).")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1263 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    print()
    print("SAL (salience-weighted argmin — all steps):")
    for game in GAMES:
        ...
        dis_str = f"dis={np.mean(dis):.3f}±{np.std(dis):.3f}" if dis else "dis=null"
        conc_str = f"conc={np.mean(concs):.3f}" if concs else ""
    ...
    # Argmin disagree rate (how often salience changes argmin choice)
    print()
    print("Argmin disagree rate (salience bias impact):")
    via_all = [r for r in all_results if r['condition'] == 'via_wiring_a']
    for game in GAMES:
        dis_vals = [r['argmin_disagree_rate'] for r in via_all
                    if r['game'] == game and r.get('argmin_disagree_rate') is not None]
        if dis_vals:
            print(f"  {game.upper()}: dis={np.mean(dis_vals):.3f}±{np.std(dis_vals):.3f}")
        else:
            print(f"  {game.upper()}: null")

    out_path = os.path.join(RESULTS_DIR, 'step1263_results.json')
