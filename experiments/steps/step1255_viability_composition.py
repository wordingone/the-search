"""
Step 1255 — Viability Attention on 1251 Composition

R3 hypothesis: C25 (prediction-error attention) doesn't correlate salience with
action usefulness (SAL=0.20, Step 1254). Viability attention tracks which encoding
DIMENSIONS are discriminated by different actions — attention[d] = max_a(alpha[d,a]) -
min_a(alpha[d,a]). Dimensions where actions produce different state changes get
high weight. This is the action-effect learning signal.

One component swap from Step 1254:
  C25 (prediction-error attention) → C_VIA (viability attention)
  GPR selector unchanged (W_salience @ ext_enc, frozen random)

Isolated variable: attention mechanism
  1254: pred-error attention + GPR → SAL = 0.20
  1255: viability attention + GPR → SAL = ?

NEW COMPONENT (C_VIA): Viability attention.
  alpha[a, d] = EMA(|enc_d - enc_d_prev| when action a was taken)
  attention[d] = max_a(alpha[:, d]) - min_a(alpha[:, d]) + ALPHA_FLOOR
  attended_enc = enc * attention / sum(attention)

  ALPHA_FLOOR ensures initial signal before alpha warms up.
  Initialized alpha = 0. After warmup, viability dominates.

Additional logging: attention weight snapshots at steps 1000 and 5000.
"""
...
# Viability attention hyperparameters
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1255')
...
class ViaWiringA:
    """
    WiringA with C25 (pred-error attention) replaced by C_VIA (viability attention),
    and argmin replaced by GPR (same as Step 1254).

    C_VIA: per-action, per-dimension EMA of encoding change magnitude.
      alpha[a, d] = EMA(|enc[d] - enc_prev[d]| when action a was taken)
      attention[d] = max_a(alpha[:, d]) - min_a(alpha[:, d]) + ALPHA_FLOOR
      attended_enc = enc * attention / sum(attention)  [256D]

    GPR (same as 1254): salience = W_salience @ ext_enc, argmax if max > mean+std.

    R3 internal repr: ext_enc = [attended_enc, h] (320D) — same shape as WiringA.
    """
    ...
    def get_internal_repr_readonly(self, obs_raw, frozen_running_mean,
                                   frozen_h, frozen_via_alpha, frozen_action_counts) -> np.ndarray:
        """R3 read-only: viability-attended enc + h = ext_enc (320D)."""
        obs = np.asarray(obs_raw, dtype=np.float32)
        x = _enc_frame(obs)
    ...
    def process(self, obs_raw) -> int:
        ...
        # C_VIA: Compute attention and attended encoding
        attention = self._compute_attention()
        att_sum = attention.sum()
        attended_enc = enc * attention / (att_sum + 1e-8)  # 256D

        # Attention snapshots
        if self.step in ATTN_SNAPSHOT_STEPS:
            self._attn_snapshots[self.step] = attention.copy()
...
def compute_r3_jacobian(substrate, obs_sample, frozen_state, n_actions, rng_seed=42):
    ...
    if _is_via:
        ...
        frozen_h = fresh_h = frozen_via_alpha = fresh_via_alpha = frozen_ac = fresh_ac = None
    ...
    for obs_raw in obs_sample[:R3_N_OBS]:
        ...
        if _is_via:
            baseline_exp = substrate.get_internal_repr_readonly(
                obs_flat, frozen_rm, frozen_h, frozen_via_alpha, frozen_ac)
        else:
        ...
        for _ in range(R3_N_DIRS):
            ...
            if _is_via:
                pert = substrate.get_internal_repr_readonly(
                    perturbed, frozen_rm, frozen_h, frozen_via_alpha, frozen_ac)
            else:
        ...
        if _is_via:
            baseline_fresh = substrate.get_internal_repr_readonly(
                obs_flat, fresh_running_mean, fresh_h, fresh_via_alpha, fresh_ac)
        else:
        ...
        for _ in range(R3_N_DIRS):
            ...
            if _is_via:
                pert = substrate.get_internal_repr_readonly(
                    perturbed, fresh_running_mean, fresh_h, fresh_via_alpha, fresh_ac)
            else:
...
def main():
    ...
    print("=" * 70)
    print("STEP 1255 — VIABILITY ATTENTION ON 1251 COMPOSITION")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("Comparison: 1254 SAL=0.20 (pred-error attn + GPR) vs 1255 (via attn + GPR).")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1255 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1255_results.json')
