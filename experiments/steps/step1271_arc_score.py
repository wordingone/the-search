"""
Step 1271 — ARC score vs solver baseline. Physarum+argmin (Step 1266) on SP80/FT09/TR87.

Confirmed spec (Leo mail 3510, 2026-03-28):
  Composition: Physarum+argmin (Step 1266 config)
  Control: Control C (argmin-alone)
  Games: SP80, FT09, TR87
  Draws: 5 per condition per game = 30 runs
  Budget: 10K steps, 300s per run

ARC score: for each level the substrate completes,
  arc_score_L = min(1.0, (solver_steps_to_L / substrate_steps_to_L)^2)
  averaged across levels completed by both solver and substrate.

Solver baselines (from validate_solvers.py):
  SP80: 107 total actions, 6 levels
  FT09: 75 total actions, 6 levels
  TR87: 123 total actions, 6 levels

Per-level solver step counts computed by replaying prescriptions at seed=1.

All 5 stages measured simultaneously (I3/I1/I4/I5/R3) same as Step 1268.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1271')
...
# ─── Solver per-level step computation ───
...
def compute_solver_level_steps(game_name: str, seed: int = 1) -> dict:
    """Replay prescription, return {level: first_step_count}."""
...
def compute_arc_score(level_first_steps: dict, solver_level_steps: dict) -> float:
    """ARC score = mean((solver_step / substrate_step)^2 across matched levels)."""
    scores = []
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    max_level = 0
    level_start_step = 0
    level_first_step = {}   # level -> total step when first reached
    level_actions = {}       # level -> actions taken within that level to complete it
    t_start = time.time()
    fresh_episode = True
    ...
    lv_str = f"Lmax={max_level}"
    print(f" {lv_str} | {arc_str} | {i3_str} | {r3_str} | {sal_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'level_actions_within': {int(k): int(v) for k, v in level_actions.items()},
...
def main():
    ...
    print("=" * 70)
    print("STEP 1271 — ARC SCORE VS SOLVER BASELINE")
    print("Physarum+argmin (1266) + Control C on SP80/FT09/TR87")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Conditions: physarum, control_c")
    print(f"Draws: {N_DRAWS} per condition per game = 30 runs")
    print(f"Budget: {MAX_STEPS} steps / {MAX_SECONDS}s per run")
    print(f"ARC score: (solver_steps_to_L / substrate_steps_to_L)^2, avg across levels")
    print()

    # Pre-compute solver per-level step counts
    ...
    for game_name in GAMES:
        try:
            ...
            total = SOLVER_TOTAL_ACTIONS[game_name]
            print(f"  {game_name.upper()}: {slv} (total={total})")
        except Exception as e:
            print(f"  {game_name.upper()}: SOLVER ERROR — {e}")
    ...
    for game_name in GAMES:
        ...
        print(f"\n{'─'*60}")
        print(f"GAME: {game_name.upper()} | n_actions={game_n_actions} | solver_levels={sorted(solver_level_steps.keys())}")
        print(f"{'─'*60}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1271 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    for result in all_results:
        summary.append({k: result[k] for k in [
            ...
            'L1_solved', 'arc_score', 'arc_per_level',
            'I3_pass', 'I1_pass', 'I4_pass', 'I5_pass', 'R3_pass', 'SAL_pass',
            'R3_jacobian_diff', 'I3_spearman_rho', 'SAL_rho', 'I4_reduction_pct',
        ]})
    with open(os.path.join(RESULTS_DIR, 'step1271_results.json'), 'w') as f:
        json.dump({'runs': summary, 'total_elapsed': round(total_elapsed, 1)}, f, indent=2)

    print("ARC Score Summary:")
    for game in GAMES:
        for cond in ['physarum', 'control_c']:
            ...
            label = 'PHY' if cond == 'physarum' else 'CtrlC'
            arc_mean = np.mean(arcs) if arcs else 0
            l1_rate = sum(l1s) / len(l1s) if l1s else 0
            print(f"  {game.upper()} {label}: ARC={arc_mean:.4f}  L1={l1_rate:.2f}  ({arcs})")

    print("\nStage Summary (pass rates):")
    for stage in ['I3', 'I1', 'I4', 'R3']:
        for game in GAMES:
            for cond in ['physarum', 'control_c']:
                runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
                passes = [r[f'{stage}_pass'] for r in runs if r.get(f'{stage}_pass') is not None]
                label = 'PHY' if cond == 'physarum' else 'CtrlC'
                rate = sum(passes) / len(passes) if passes else 0
                print(f"  {game.upper()} {label} {stage}: {rate:.2f} ({sum(passes)}/{len(passes)})")
