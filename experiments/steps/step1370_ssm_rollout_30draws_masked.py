"""
Step 1370 — 30-draw paired monotonicity test.
Leo mail 3908, 2026-03-30.

Same architecture as 1369: ROLLOUT-ARGMAX (K=3, N=8) vs SSM-DISCONNECTED.
Same draws (seeds 13700-13729) for BOTH conditions — paired comparison.

Kill criteria:
  - Rollout losses > 0 → not monotonic. KILL.
  - Rollout wins < 3/30 → effect too weak. KILL.
  - Rollout wins ≥ 6 AND losses = 0 → CONFIRMED monotonic. New baseline.
"""
...
STEP        = 1370
N_DRAWS     = 30
DRAW_SEEDS  = [13700 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# Rollout config
K_ROLLOUT    = 3     # dream steps per candidate
N_CANDIDATES = 8     # candidate actions sampled per real step
...
class MambaSSMSubstrate:
    ...
    def process(self, obs_arr):
        ...
        if self._step < WARMUP:
            ...
            # Autocorrelation tracking (rollout modes only)
            if self._in_try2:
...
def main():
    # ── Tier 1: timing check for ROLLOUT (bottleneck condition) ──────────
    print("=== TIER 1: timing check (ROLLOUT — bottleneck condition) ===")
    ...
    # Pre-warmup: 50 random steps so game is initialized
    run_episode(env_t1, sub_t1, na_t1, seed=0, max_steps=50)
    # Force past warmup so rollout activates in timing run
    sub_t1._step = WARMUP
    t_tier1 = time.time()
    ...
    # DISCONNECTED is ~0.09ms/step (no rollout overhead)
    # Estimate: 15% of rollout time as conservative upper bound
    ms_per_step_disc = ms_per_step_rollout * 0.15

    n_eps_per_cond = N_DRAWS * 3 * 2  # draws × games × tries
    ...
    print(f"  {TIER1_STEPS} steps (ROLLOUT): {tier1_elapsed:.2f}s  ({ms_per_step_rollout:.2f}ms/step)")
    ...
    if est_total_s > MAX_SECONDS:
        # Cap based on rollout budget only (disc is negligible)
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: 30-draw paired monotonicity test ===")
    print(f"Confirming: ROLLOUT monotonically ≥ DISCONNECTED on same draws.")
    print(f"K_ROLLOUT={K_ROLLOUT}, N_CANDIDATES={N_CANDIDATES}, WARMUP={WARMUP}, N_DRAWS={N_DRAWS}")
    print(f"SSM: D={D}, N_STATE={N_STATE}, N_LAYERS={N_LAYERS}")
    print(f"MLP+TP baseline (1349): {MLP_TP_BASELINE:.2e}")
    print(f"Kill: losses > 0. Confirm: wins >= 6 AND losses = 0.")
    ...
    if 'ROLLOUT-ARGMAX' in rhae_by_cond and 'SSM-DISCONNECTED' in rhae_by_cond:
        ...
        if disc_wins > 0:
            print(f"  *** NOT MONOTONIC — DISCONNECTED beat ROLLOUT on {disc_wins} draws ***")
        elif rollout_wins < 3:
            print(f"  *** EFFECT TOO WEAK — only {rollout_wins} wins out of {N_DRAWS} draws ***")
        elif rollout_wins >= 6:
        ...
        summary['monotonic_verdict'] = (
            'NOT_MONOTONIC' if disc_wins > 0 else
            'TOO_WEAK' if rollout_wins < 3 else
            'CONFIRMED' if rollout_wins >= 6 else
            'BORDERLINE'
        )
