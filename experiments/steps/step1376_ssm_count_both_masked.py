"""
Step 1376 — COUNT try2 vs RAND try2 (both with COUNT try1).
Leo mail 3926, 2026-03-30.

Step 1375 showed: COUNT try1 builds better weights (7.4× vs RAND try1).
This tests: given good weights from COUNT try1, does COUNT try2 also help?

Two conditions:
  COUNT-COUNT: try1=COUNT, try2=COUNT. h reset. Weights carry. Visit counts reset at try2 start.
  COUNT-RAND:  try1=COUNT, try2=RANDOM. h reset. Weights carry.

Kill criteria: paired sign test p > 0.10 (COUNT try2 doesn't help beyond try1 weights).

Seeds: 13800-13829 (fresh range, avoids 1374: 13740-13769, 1375: 13770-13799).
"""
...
STEP        = 1376
N_DRAWS     = 30
DRAW_SEEDS  = [13800 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# SSM config (SSM-DIAG-RTRL, same as 1360-1375)
PROJ_DIM      = 64
ACT_EMBED_DIM = 16
D             = 128
N_STATE       = 32
...
CONDITIONS = ['COUNT-COUNT', 'COUNT-RAND']
...
class MambaSSMSubstrate:
    """
    try1_mode: 'count' — action selection during try1 (always COUNT for this experiment)
    try2_mode: 'count' or 'random' — action selection during try2
    h always reset at try boundary (persist_h=False, h persistence killed in 1374/1375).
    Visit counts reset at try2 start (independent COUNT).
    """

    def __init__(self, n_actions, try2_mode='random'):
    ...
    def prepare_for_try2(self):
        """Compute R3, reset state (h always reset — persistence killed)."""
        ...
        self._prev_action    = 0
        self._in_try2        = True
        self._visit_count[:] = 0   # Independent COUNT from zero for try2
        self._current_level  = 0
...
def run_draw(draw_idx, draw_seed, cond_name, try2_mode, max_steps):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        substrate = MambaSSMSubstrate(n_actions=n_actions, try2_mode=try2_mode)
        ...
        row = {
            ...
            'try2_h_norm_1999': stats['try2_h_norm_at'].get(1999),
...
def main():
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, try2_mode='count')
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: COUNT-COUNT vs COUNT-RAND, 30 draws ===")
    print(f"Seeds: {DRAW_SEEDS[0]}-{DRAW_SEEDS[-1]}")
    print(f"Both conditions: try1=COUNT (builds good weights), h reset at try2 start.")
    print(f"COUNT-COUNT: try2=COUNT (independent from zero). COUNT-RAND: try2=RANDOM.")
    ...
    for cond_name in CONDITIONS:
        try2_mode = cond_params[cond_name]
    ...
    # Paired comparison
    cc_list = rhae_by_cond['COUNT-COUNT']
    cr_list = rhae_by_cond['COUNT-RAND']
    cc_wins = sum(1 for a, b in zip(cc_list, cr_list) if a > b)
    cr_wins = sum(1 for a, b in zip(cc_list, cr_list) if b > a)
    ties    = sum(1 for a, b in zip(cc_list, cr_list) if a == b)
    n_dec   = cc_wins + cr_wins
    p_val   = binomial_p_one_sided(cc_wins, n_dec)
    verdict = 'SIGNAL' if p_val <= 0.10 else 'NOT_SIGNIFICANT'

    print(f"\n=== PAIRED COMPARISON ===")
    print(f"  COUNT-COUNT wins {cc_wins}/{N_DRAWS}, COUNT-RAND wins {cr_wins}/{N_DRAWS}, "
          f"ties {ties}/{N_DRAWS}  p={p_val:.4f}")
    if verdict == 'SIGNAL':
        print(f"  *** SIGNAL: COUNT try2 helps beyond try1 weights (p={p_val:.4f}) ***")
    else:
        print(f"  NOT_SIGNIFICANT: COUNT try2 doesn't add beyond try1 weights (p={p_val:.4f})")

    summary['paired_cc_vs_cr'] = {
        'cc_wins': cc_wins, 'cr_wins': cr_wins, 'ties': ties,
