"""
Step 1332 — TP + mode map calibration fix (threshold 1% + K=8 per region + min 20 clicks)
Leo mail 3773, 2026-03-29.

Step 1331 INCONCLUSIVE: mode map found only 1 centroid on Game A (over-restricted), 0 regions
on Game B (threshold 5% too high). Two calibration issues identified:
  1. 5% threshold too high: some games have diffuse pixel changes
  2. Single centroid per region too few: game needs multiple click positions

Fixes applied (same TP architecture, only mode map calibration changes):
  1. Threshold: 5% → 1% (catch weaker pixel change signals)
  2. K=8 random points sampled from within each region's bounding box (not just centroid)
  3. Minimum coverage: if mode_map gives < 20 distinct clicks, supplement with random clicks

Everything else identical to step 1331.

Kill:   speedup ≤ 1.0 AND mode_map_n_regions > 0 → regions found but don't help → KILL
Signal: speedup > 1.0 (finite) → SIGNAL
Inconclusive: speedup null → game draw issue (neither try reached L1)
"""
...
GAMES, GAME_LABELS = select_games(seed=1332)
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1332')
...
_MODE_MAP_FREQ   = 200    # recompute regions every N steps
_CHANGE_THRESH   = 0.01   # 1% threshold (was 5% in step 1331)
_MM_K            = 8      # random points per region bounding box
_MM_MIN_CLICKS   = 20     # minimum distinct click positions; supplement if fewer


# ---------------------------------------------------------------------------
# CNN model (same as steps 1323/1326/1329/1331)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TP substrate (same as 1329/1331)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TP + Mode Map substrate (calibration v2)
# ---------------------------------------------------------------------------

class TpModeMapSubstrate(TpCnnSubstrate):
    """TP + pixel-level mode map, calibration v2.

    Changes from step 1331:
    - Threshold: 1% (was 5%)
    - K=8 random points per region bounding box (was 1 centroid)
    - Minimum 20 distinct click positions (supplement with random if fewer)
    - Separate _mm_rng (seed=43) for mode map sampling — does not affect action RNG.

    Mode map persists across try1→try2 (intentional: test region generalization).
    MBPP / n_actions ≤ 128: mode map inactive (random fallback).
    """
    ...
    def __init__(self, n_actions):
        ...
        self._mm_rng             = np.random.RandomState(43)  # dedicated MM rng

    def reset_loss_tracking(self):
        # Loss tracking only — mode map intentionally persists to try2
        self._step = 0
        self._recent_losses.clear()
        self._pred_loss_at = {ck: None for ck in LOSS_CHECKPOINTS}
    ...
    def _update_active_regions(self):
        """Recompute active click positions: K random points per region bbox + min coverage."""
        try:
        ...
        # Get bounding box slices for each region
        region_slices = find_objects(labeled)
        ...
        for s in region_slices:
            if s is None:
                continue
            row_min = s[0].start
            row_max = s[0].stop - 1  # inclusive
            col_min = s[1].start
            col_max = s[1].stop - 1  # inclusive

            # K random points within bounding box
            for _ in range(_MM_K):
                row = int(self._mm_rng.randint(row_min, row_max + 1))
                col = int(self._mm_rng.randint(col_min, col_max + 1))
        ...
        # Minimum coverage: supplement with random clicks if too few
        active_sg_set = set(active_sg)
        if len(active_sg_set) < _MM_MIN_CLICKS:
            needed = _MM_MIN_CLICKS - len(active_sg_set)
            pool = [sg for sg in range(5, _SG_OUT_DIM) if sg not in active_sg_set]
            if pool:
                extra_idx = self._mm_rng.choice(len(pool),
                                                 min(needed, len(pool)),
                                                 replace=False)
                for i in extra_idx:
                    active_sg_set.add(pool[i])

        self._active_sg_indices  = list(active_sg_set)
...
def main():
    ...
    print(f"Step {STEP} — TP + mode map calibration v2")
    print(f"Games: {masked_game_list(GAME_LABELS)} (ARC games masked)")
    print(f"Device: {_DEVICE}")
    print(f"TP: learned g_i inverses + local Adam (same as 1329)")
    print(f"ModeMap v2: threshold={_CHANGE_THRESH*100:.0f}%, K={_MM_K} pts/bbox, "
          f"min_clicks={_MM_MIN_CLICKS}. Persists try1→try2.")
    print(f"Kill:   speedup ≤ 1.0 AND regions > 0 → mode map direction CLOSED")
    print(f"Signal: speedup > 1.0 (finite) → spatial generalization SIGNAL")
    print(f"3 runs × 2 tries = 6 episodes ({MAX_STEPS} steps each).")
    print()
    ...
    if first_arc:
        ...
        if est_total > 280:
            print(f"  WARNING: estimated {est_total:.0f}s > 5 min cap.")
        print()
        del sub_t, env_t
    ...
    for game in GAMES:
        ...
        for condition in CONDITIONS:
            ...
            print(f"  try1: {mm1['mode_map_n_regions']}R/{mm1['mode_map_n_clicks']}C  "
                  f"try2: {mm2['mode_map_n_regions']}R/{mm2['mode_map_n_clicks']}C")
    ...
    sep = '=' * 100
    print(f"\n{sep}")
    print(f"STEP {STEP} — RESULT (TP + mode map calibration v2)\n")
    ...
    for r in all_results:
        ...
        mm1   = f"{r['try1']['mode_map_n_regions']}R/{r['try1']['mode_map_n_clicks']}C"
        mm2   = f"{r['try2']['mode_map_n_regions']}R/{r['try2']['mode_map_n_clicks']}C"
        print(f"  {label}: speedup={format_speedup(sp)} cr={cr} | try1 {mm1} → try2 {mm2}")
    print()

    # Kill / Signal
    tp_speedup   = final_speedup.get('tp_mode')
    all_speedups = speedup_by_condition.get('tp_mode', [])
    any_regions  = any(r['try1']['mode_map_n_regions'] > 0 or r['try2']['mode_map_n_regions'] > 0
                       for r in all_results if r['condition'] == 'tp_mode')

    print("KILL/SIGNAL ASSESSMENT:")
    if tp_speedup is None and all(s is None for s in all_speedups):
        if any_regions:
            print(f"  >>> INCONCLUSIVE: regions found but no game reached L1 in both tries.")
        else:
            print(f"  >>> INCONCLUSIVE: no regions found AND no L1 in any game.")
    elif tp_speedup == float('inf') or tp_speedup is None:
        print(f"  >>> INCONCLUSIVE: only inf or null speedups.")
    elif tp_speedup > 1.0:
        print(f"  >>> SIGNAL: speedup={tp_speedup:.4f} > 1.0")
        print(f"  >>> Spatial region generalization confirmed.")
    elif tp_speedup <= 1.0:
        if any_regions:
            print(f"  >>> KILL: speedup={tp_speedup} ≤ 1.0 with regions found.")
            print(f"  >>> Regions don't generalize across episodes. Mode map direction CLOSED.")
        else:
            print(f"  >>> INCONCLUSIVE: speedup ≤ 1.0 but no regions found (threshold issue).")
    print(sep)
