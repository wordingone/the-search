"""
Step 1375 — COUNT try1 + persistent h try2.
Leo mail 3923, 2026-03-30.

Step 1374 showed: random-action try1 builds incoherent h (noise). Persistence worthless.
This tests: does structured (COUNT) exploration in try1 build h worth carrying?

Three conditions:
  COUNT-PERSIST: try1 uses COUNT action selection, try2 starts with h0=h_final (weights carry)
  COUNT-RESET:   try1 uses COUNT action selection, try2 starts with h0=0   (weights carry)
  RAND-RESET:    try1 random, try2 random, h0=0                             (control — replicates 1374 RESET on new seeds)

All conditions: try2 uses random action selection (disconnected).

Kill criteria:
  1. COUNT-PERSIST sign test vs COUNT-RESET: p > 0.10 → h persistence doesn't help even with structured try1
  2. COUNT-RESET vs RAND-RESET: p > 0.10 → COUNT try1 doesn't produce better weights than RAND try1

Seeds: 13770-13799 (fresh range, avoids overlap with 1374's 13740-13769 and 1370-1373's 13700-13729).
"""
...
STEP        = 1375
N_DRAWS     = 30
DRAW_SEEDS  = [13770 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# SSM config (SSM-DIAG-RTRL, same as 1360-1374)
PROJ_DIM      = 64
ACT_EMBED_DIM = 16
D             = 128
N_STATE       = 32
...
CONDITIONS = ['COUNT-PERSIST', 'COUNT-RESET', 'RAND-RESET']
...
def binomial_p_one_sided(wins, n):
    """P(X >= wins) under H0: p=0.5, Binomial(n, 0.5)."""
    if n <= 0:
        return 1.0
...
class SSMLayer:
    def __init__(self, d, n_state, lr):
        ...
        self._init_B = self.B.copy()  # R3 reference
...
class MambaSSMSubstrate:
    """
    try1_mode: 'count' or 'random' — action selection during try1
    persist_h: True/False — whether to carry h from try1 into try2
    try2 always uses random actions (disconnected).
    """

    def __init__(self, n_actions, try1_mode='random', persist_h=False):
    ...
    def process(self, obs_arr):
        ...
        # Action selection
        if self._in_try2:
            # try2 always random (disconnected)
            action = int(np.random.randint(self.n_actions))
        elif self.try1_mode == 'count':
            # COUNT: argmin(visit_count), ties broken randomly
        ...
        # I3: snapshot at I3_STEP in try2
    ...
    def prepare_for_try2(self):
        """Compute R3, optionally save h, reset for try2."""
    ...
    def get_stats(self):
        ...
        return {
            ...
            'try2_h_norm_at':  self._try2_h_norm_at,
...
def run_draw(draw_idx, draw_seed, cond_name, try1_mode, persist_h, max_steps):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        substrate = MambaSSMSubstrate(n_actions=n_actions, try1_mode=try1_mode, persist_h=persist_h)
        ...
        row = {
            ...
            't1': t1, 't2': t2,
            'try1_h_norm_100':  stats['try1_h_norm_at'].get(100),
            'try1_h_norm_500':  stats['try1_h_norm_at'].get(500),
            'try1_h_norm_1000': stats['try1_h_norm_at'].get(1000),
        ...
        print(f"    [{cond_name}] {label}: p1={p1} p2={p2} eff_sq={eff_sq:.6f} "
              f"h1_norm_1999={stats['try1_h_norm_at'].get(1999)} "
...
# ---------------------------------------------------------------------------
# Paired sign test helper
# ---------------------------------------------------------------------------

def paired_test(list_a, list_b, label_a, label_b):
    wins_a = sum(1 for a, b in zip(list_a, list_b) if a > b)
    wins_b = sum(1 for a, b in zip(list_a, list_b) if b > a)
    ties   = sum(1 for a, b in zip(list_a, list_b) if a == b)
    n_dec  = wins_a + wins_b
    p      = binomial_p_one_sided(wins_a, n_dec)
    print(f"  Paired {label_a} vs {label_b}: {label_a} wins {wins_a}/{N_DRAWS}, "
          f"{label_b} wins {wins_b}/{N_DRAWS}, ties {ties}/{N_DRAWS}  p={p:.4f}")
    verdict = 'SIGNAL' if p <= 0.10 else 'NOT_SIGNIFICANT'
    return wins_a, wins_b, ties, p, verdict
...
def main():
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, try1_mode='count', persist_h=False)
    ...
    # 3 conditions: COUNT-PERSIST and COUNT-RESET use ~same speed as RAND-RESET
    est_total_s  = ms_per_step / 1000 * TRY1_STEPS * n_eps_total * 3
    ...
    if est_total_s > MAX_SECONDS:
        max_steps = max(200, int(
            (MAX_SECONDS * 0.9) / (ms_per_step / 1000 * n_eps_total * 3)
        ))
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: COUNT-PERSIST vs COUNT-RESET vs RAND-RESET, 30 draws ===")
    print(f"Seeds: {DRAW_SEEDS[0]}-{DRAW_SEEDS[-1]}")

    os.makedirs(RESULTS_DIR, exist_ok=True)

    # (try1_mode, persist_h) for each condition
    cond_params = {
        'COUNT-PERSIST': ('count',  True),
        'COUNT-RESET':   ('count',  False),
        'RAND-RESET':    ('random', False),
    }
    ...
    for cond_name in CONDITIONS:
        try1_mode, persist_h = cond_params[cond_name]
        print(f"\n--- Condition: {cond_name} (try1_mode={try1_mode}, persist_h={persist_h}) ---")
        for di, ds in enumerate(DRAW_SEEDS):
            rhae, draw_res = run_draw(di, ds, cond_name, try1_mode, persist_h, max_steps)
    ...
    # Paired comparisons
    print(f"\n=== PAIRED COMPARISONS ===")
    cp_wins, cr_wins, cp_cr_ties, p1, v1 = paired_test(
        rhae_by_cond['COUNT-PERSIST'], rhae_by_cond['COUNT-RESET'],
        'COUNT-PERSIST', 'COUNT-RESET'
    )
    cr_wins2, rr_wins, cr_rr_ties, p2, v2 = paired_test(
        rhae_by_cond['COUNT-RESET'], rhae_by_cond['RAND-RESET'],
        'COUNT-RESET', 'RAND-RESET'
    )

    if v1 == 'SIGNAL':
        print(f"  *** COUNT-PERSIST > COUNT-RESET: h persistence HELPS when try1 is structured ***")
    else:
        print(f"  NOT_SIGNIFICANT: h persistence doesn't help even with structured try1")

    if v2 == 'SIGNAL':
        print(f"  *** COUNT-RESET > RAND-RESET: COUNT try1 builds better weights ***")
    else:
        print(f"  NOT_SIGNIFICANT: COUNT vs RAND try1 doesn't matter for try2 RHAE")

    summary['paired_count_persist_vs_count_reset'] = {
        'cp_wins': cp_wins, 'cr_wins': cr_wins, 'ties': cp_cr_ties,
        'p_value': p1, 'verdict': v1,
    }
    summary['paired_count_reset_vs_rand_reset'] = {
        'cr_wins': cr_wins2, 'rr_wins': rr_wins, 'ties': cr_rr_ties,
        'p_value': p2, 'verdict': v2,
    }
