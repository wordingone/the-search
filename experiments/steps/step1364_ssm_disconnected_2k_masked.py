"""
Step 1364 — SSM disconnected at full 2K steps. The missing control.
Leo mail 3893, 2026-03-30.

Step 1360 ran SSM disconnected but only 395 steps (Tier 1 overestimated timing).
Steps 1361-1363 ran 2K but WITH action training (all hurt).
This tests: SSM disconnected action head at TRUE 2K steps.

Architecture: IDENTICAL to step 1360. No changes.
...
Question: SSM disconnected 2K steps vs MLP+TP 2K steps (4.59e-5)?
- SSM 395 steps: 3.26e-5 (1360)
- SSM 2K steps: ??? (this step)

Protocol:
- 10 draws, seeds 13600-13609 (same as 1360)
- 2K steps per episode
- Kill criterion: chain_mean > 4.59e-5 (MLP+TP baseline)
"""
...
def run_draw(draw_idx, draw_seed, max_steps):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        row = {
            ...
            'try1_act_ent_1999': stats['try1_act_ent'].get(2000),
            ...
            'try2_act_ent_1999': stats['try2_act_ent'].get(2000),
            'try1_state_norm_100':  stats['try1_state_norm'].get(100),
            'try1_state_norm_1999': stats['try1_state_norm'].get(2000),
            'try2_state_norm_100':  stats['try2_state_norm'].get(100),
            'try2_state_norm_1999': stats['try2_state_norm'].get(2000),
...
def main():
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: SSM disconnected — full 2K steps ===")
    ...
    print(f"W_act: fixed random, never updated. Entropy action selection.")
    print(f"MLP+TP baseline (1349): {MLP_TP_BASELINE:.2e}  |  1360 (395 steps): 3.26e-5")
