"""
Step 1290 — Allosteric softmax, DECAY removed from W_action.

One change from Step 1289: remove W_action *= (1 - DECAY) from update_flow().
W_action now accumulates pure LPL signal without decay.

Mechanism (SAL condition):
  salience = W_action @ ext          # shape: (n_actions,)
  T = max(1.0 / (std(salience) + eps), 0.1)   # adaptive temperature, min 0.1
  probs = softmax(salience / T)
  action = sample(probs)

Early (random W): std(salience) low → T high → probs ≈ uniform → exploration.
Late (structured W): std(salience) high → T low → biased toward high-salience actions.

REMOVED from SAL: action_counts, pe_ema (bypassed by allosteric selection).
Everything else UNCHANGED: LPL (eta_h=0.05), centered encoding C4, recurrent h,
W_pred, flow update: W_action[a] += eta_h * flow * ext. NO DECAY on W_action.

Motivation: 1286 showed R3 is weight-driven but behaviorally INERT — argmin
ignores W entirely. Allosteric softmax reads salience = W_action @ ext directly,
making R3 behaviorally active for the first time.

Stage predictions:
  R3: BEHAVIORAL ACTIVATION (first time R3 affects selection)
  I4: IMPROVE (high-salience actions cluster in time)
  I3: POSSIBLE DEGRADATION (softmax less systematic than argmin)
  I1: MAINTAIN

Kill criteria:
  FT09 L1 < 1/5 AND no new L1 on any failing game → KILL
  I3_cv > 300% of CTL on >3 games → KILL
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1290')
...
CONDITIONS = ['softmax', 'lpl_pe_ctrl']
LABELS = {'softmax': 'SAL', 'lpl_pe_ctrl': 'CTL'}
...
class LPLSALSubstrate:
    """LPL substrate with allosteric softmax selection (SAL) or argmin+pe_ema (CTL).

    SAL condition: action = sample(softmax(W_action @ ext / T)), T = max(1/std(salience), T_MIN).
    CTL condition: action = argmin(counts - alpha * pe_ema).
    """

    def __init__(self, n_actions: int, seed: int, use_softmax: bool = False):
        ...
        # Temperature tracking (SAL only)
        self._T_sum = 0.0
        self._T_count = 0
        self._T_trajectory = []  # [(step, T, std_salience)] every 100 steps
    ...
    def process(self, obs_raw) -> int:
        ...
        if self.use_softmax:
            std_sal = float(np.std(salience))
            T = max(1.0 / (std_sal + 1e-8), T_MIN)
            self._T_sum += T
            self._T_count += 1
            if self.step % 100 == 0:
                self._T_trajectory.append((self.step, round(T, 2), round(std_sal, 6)))
            logits = salience / T
    ...
    def mean_temperature(self):
        if self._T_count == 0:
            return None
        return self._T_sum / self._T_count
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_softmax = (condition == 'softmax')
    substrate = LPLSALSubstrate(n_actions=n_actions, seed=seed, use_softmax=use_softmax)
    env = make_game(game_name)
    obs = env.reset(seed=seed)
    ...
    mean_T = substrate.mean_temperature()
    ...
    T_str = f" T={mean_T:.2f}" if mean_T is not None else ""
    arc_str = f"ARC={arc_score:.4f}" if arc_score > 0 else "ARC=0"
    print(f" [{label}] Lmax={max_level} | {i3_str} | {i4_str} | {r3_str} |{T_str} {arc_str} | {elapsed:.1f}s")
    ...
    return {
        ...
        'mean_temperature': round(mean_T, 4) if mean_T is not None else None,
        'T_trajectory': substrate._T_trajectory if hasattr(substrate, '_T_trajectory') else None,
        'steps_taken': steps,
...
def main():
    ...
    print("=" * 70)
    print("STEP 1290 — Allosteric softmax, DECAY removed from W_action")
    print("Mechanism: action = sample(softmax(W_action @ ext / T)), T=max(1/std, 0.1)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1290 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    print("\nMean temperature (SAL condition only):")
    for game in GAMES:
        runs = [r for r in all_results if r['game'] == game and r['condition'] == 'softmax']
        temps = [r['mean_temperature'] for r in runs if r.get('mean_temperature') is not None]
        mean_temp = float(np.mean(temps)) if temps else None
        print(f"  {game.upper()}: {mean_temp:.3f}" if mean_temp is not None else f"  {game.upper()}: null")

    with open(os.path.join(RESULTS_DIR, 'step1290_results.json'), 'w') as f:
