"""
Step 1282 — address R3 borderline fail on KB games via higher Hebbian learning rate.

LS20/TR87/TU93 show R3=0.044-0.049 (just below 0.05 threshold) on 1276/1280.
Hypothesis: W changes are small per step on 7-action games (each action visited
~1400x, effects well-estimated, Hebbian updates converge). Higher eta_h = larger
W changes per step = Jacobian diff crosses 0.05 threshold.

Conditions:
  lpl_pe_eta05 — eta_h=0.05 (5x increase, target: R3 pass on KB games)
  lpl_pe_eta01 — eta_h=0.01 (1276 confirmed composition, control)

Kill criterion: FT09 L1 < 3/5 (eta_h destabilizing) OR R3 no improvement on KB games.
...
ETA_PRED = 0.01
PE_EMA_ALPHA = 0.05
SELECTION_ALPHA = 0.1  # unchanged from 1276

ETA_H_DEFAULT = 0.01   # 1276 confirmed composition
ETA_H_NEW = 0.05       # 5x — target: R3 pass on KB games
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1282')
...
class LPLPESubstrate:
    """LPL PE substrate with configurable Hebbian learning rate (eta_h).

    1276 composition: LPL encoding + pe_ema action selection (unnormalized).
    eta_h controls how fast W_action learns from flow updates.
    Higher eta_h = larger Jacobian diff = stronger R3 signal.
    """

    def __init__(self, n_actions: int, seed: int, eta_h: float = ETA_H_DEFAULT):
    ...
    def update_flow(self, next_obs_raw):
        ...
        # eta_h controls Hebbian update magnitude
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    eta_h = ETA_H_NEW if condition == 'lpl_pe_eta05' else ETA_H_DEFAULT
    substrate = LPLPESubstrate(n_actions=n_actions, seed=seed, eta_h=eta_h)
    ...
    label = 'E05' if condition == 'lpl_pe_eta05' else 'E01'
...
def main():
    ...
    print("=" * 70)
    print("STEP 1282 — eta_h=0.05 vs eta_h=0.01 (1276 composition)")
    print("Target: R3 pass on LS20/TR87/TU93 (currently 0.044-0.049 vs 0.05 threshold)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    print(f"Draws: {N_DRAWS} per condition per game = {N_DRAWS * 2 * len(GAMES)} total runs")
    print(f"eta_h_new={ETA_H_NEW}, eta_h_ctrl={ETA_H_DEFAULT}")
    print()
    ...
    for game_name in GAMES:
        ...
        for condition in ['lpl_pe_eta05', 'lpl_pe_eta01']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'lpl_pe_eta05' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1282 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # --- Summary ---
    print("R3 Summary (key metric):")
    for game in GAMES:
        for cond in ['lpl_pe_eta05', 'lpl_pe_eta01']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'E05' if cond == 'lpl_pe_eta05' else 'E01'
    ...
    with open(os.path.join(RESULTS_DIR, 'step1282_results.json'), 'w') as f:
