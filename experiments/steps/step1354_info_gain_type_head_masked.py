"""
Step 1354 — Type_head trained via information gain (prediction improvement per type).
Leo mail 3859, 2026-03-29.

1353 proved architecture works (entropy drops 0.16) but change-magnitude target was wrong
(favored keyboard actions that produce visual noise, not task-relevant change).

Fix: type_target = argmax of per-type EMA prediction error.
- Per-type EMA of ||pred_h3_next - actual_h3_next|| for transitions of each action type
- Types that produce novel, hard-to-predict transitions → high info gain → selected more
- For click games: clicking interactive objects → unexpected state → high pred error → selected
- For keyboard games: relevant keys → novel state changes → selected over irrelevant keys
- R1-compliant: self-supervised from own prediction quality, no external reward

Conditions:
  INFO-HIER: Hierarchical + info-gain-trained type_head. Seeds 13500-13504.
  CHANGE-HIER: 1353 draws 0-4 (change-magnitude target). Same seeds.
  HIER: 1352 draws 0-4 (untrained). Same seeds.

Kill criteria:
  INFO type_entropy < CHANGE type_entropy → info-gain concentrates more. Better target.
  INFO RHAE > HIER RHAE → info-gain over untrained. SIGNAL.
  INFO RHAE > 0 Game B → LANDMARK. First click game from learned selection.
"""
...
RESULTS_DIR       = os.path.join('B:/M/the-search/experiments/compositions', 'results_1354')
HIER_BASELINE_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1352')
CHANGE_BASELINE_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1353')
...
# ---------------------------------------------------------------------------
# Info-gain hierarchical substrate
# ---------------------------------------------------------------------------

class InfoGainHierSubstrate:
    """
    MLP+TP with hierarchical action selection. type_head trained via
    information gain: per-type EMA of ||pred_h3_next - actual_h3_next||.
    Types producing harder-to-predict (more informative) transitions get selected more.
    """
    ...
    def get_info_gain_stats(self):
        """Return per-type info gain for diagnostics."""
        return {i: round(float(self._type_info_gain[i]), 4) for i in range(N_TYPES)}
    ...
    def reset_for_try2(self):
        ...
        self._click_count       = 0
        self._type_entropy_at   = {ck: None for ck in ENT_CHECKPOINTS}
        # NOTE: _type_info_gain persists across tries (accumulates experience)
    ...
    def update_after_step(self, obs_next, action, reward):
        ...
        # Determine 8-type classification for buffer
    ...
    def _type_train_step(self, h3_batch):
        """
        Update type_head using information gain: argmax of per-type EMA prediction error.
        Types producing harder-to-predict transitions get selected more.
        """
        # Need at least one sample per type before training
        if self._type_info_count.min() == 0:
            return
        ig = self._type_info_gain
        if ig.max() < 1e-8:
            return
    ...
    def _tp_train_step(self):
        ...
        # Update per-type info gain EMA from this batch
        if self._is_hier:
...
def run_draw(draw_idx, draw_seed):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        substrate = InfoGainHierSubstrate(n_actions=n_actions)
        ...
        info_gain_stats = substrate.get_info_gain_stats() if substrate._is_hier else {}
        ...
        result = {
            ...
            'info_gain_stats': info_gain_stats,
            't1': t1, 't2': t2,
        }
        ...
        if substrate._is_hier:
            ...
            if info_gain_stats:
                best_type = max(info_gain_stats, key=info_gain_stats.get)
                print(f"      info_gain argmax: type={best_type} gain={info_gain_stats[best_type]} (click type 7: {info_gain_stats.get(7)})")
...
def main():
    ...
    change_rhae = load_baseline(CHANGE_BASELINE_DIR, 1353, 5)
    hier_mean   = sum(hier_rhae) / len(hier_rhae)
    change_mean = sum(change_rhae) / len(change_rhae)
    hier_nz     = sum(1 for r in hier_rhae if r > 0)
    change_nz   = sum(1 for r in change_rhae if r > 0)

    print(f"Step {STEP} — Info-gain type_head training.")
    print(f"Device: {_DEVICE}")
    print(f"Protocol: {N_DRAWS} draws × 3 games × 2 tries = {N_DRAWS*3*2} episodes")
    print(f"INFO-HIER: type_target = argmax(per-type EMA prediction error)")
    print(f"Max type entropy: {MAX_TYPE_ENTROPY}  Draw seeds: {DRAW_SEEDS}")
    print(f"HIER baseline (1352 d0-4): {hier_rhae}  mean={hier_mean:.7f}  nz={hier_nz}/5")
    print(f"CHANGE baseline (1353 d0-4): {change_rhae}  mean={change_mean:.7f}  nz={change_nz}/5")
    print()
    ...
    if first_arc:
        ...
        try:
            ...
            na_t = 4103
        sub_t = InfoGainHierSubstrate(n_actions=na_t)
        t0 = time.time()
    ...
    print("=" * 80)
    print(f"STEP {STEP} — RESULT (Info-gain type_head training)")
    print()
    print(f"  INFO-HIER RHAE per draw: {[f'{r:.6f}' for r in rhae_per_draw]}")
    ...
    print(f"    Drop H100→H2000: {ent_drop}  (1353 CHANGE drop was 0.1593)")
    print(f"  Mean click_frac: {mean_click_frac}  (1353 CHANGE: 0.0939, 1352 HIER: 0.1232)")
    print()
    ...
    for r in all_results:
        if r['eff_sq'] > 0:
            ...
            if r.get('info_gain_stats'):
                ig = r['info_gain_stats']
                best = max(ig, key=ig.get)
                print(f"      info_gain argmax=type{best}({ig[best]}), click(7)={ig.get(7)}")
    print()

    print(f"  Baselines:")
    print(f"    HIER (1352 d0-4): {hier_rhae}  mean={hier_mean:.7f}  nz={hier_nz}/5")
    print(f"    CHANGE (1353 d0-4): {change_rhae}  mean={change_mean:.7f}  nz={change_nz}/5")
    print()
    ...
    if ent_drop > 0.05:
        print(f"  >>> type_entropy drops {ent_drop} — INFO-GAIN target concentrates type_head.")
        if ent_drop > 0.1593:
            print(f"  >>> INFO entropy drop ({ent_drop}) > CHANGE drop (0.1593). Better target.")
    else:
        print(f"  >>> type_entropy flat (drop={ent_drop}). Info-gain not concentrating type_head.")
    if hier_mean > 0 and chain_mean > hier_mean:
        ratio = chain_mean / hier_mean
        print(f"  >>> INFO RHAE={chain_mean:.7f} > HIER {hier_mean:.7f} ({ratio:.1f}×). Learning helps.")
    elif chain_mean == 0:
        print(f"  >>> KILL: INFO RHAE=0. Info-gain target still wrong for task progress.")
    else:
        print(f"  >>> INFO RHAE={chain_mean:.7f} vs HIER {hier_mean:.7f}. Marginal.")
    print("=" * 80)
    ...
    summary = {
        ...
        'change_baseline_rhae': change_rhae, 'change_baseline_mean': change_mean,
    }
