"""
Step 1287 — Pairwise transition count argmin.

One variable change from Step 1282: count[a] → count[a_{t-1}, a].

Current (1282): score = count[a] - alpha * pe_ema[a]
New: score = count[a_{t-1}, a] - alpha * pe_ema[a]

Why this produces sequences:
- On 7-action KB games: 49 pairs, each visited ~200x in 10K steps → full sequential coverage
- SP80 (L1 = same key × 2): pair (key, key) with count=0 → selected immediately → chains naturally
- On 4103-action click games: nearly all pairs unvisited (count=0) → same behavior as single-action argmin
  → FT09 risk minimal (LPE predictions: maintained)

Everything else identical: LPL, eta_h=0.05, pe_ema, alpha=0.1, full PRISM.
Control = current composition (single-action count, Step 1282 baseline).

Kill criteria: FT09 L1 < 3/5 OR no new L1 on any failing game.
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1287')
...
CONDITIONS = ['pair_argmin', 'lpl_pe_ctrl']
LABELS = {'pair_argmin': 'PAR', 'lpl_pe_ctrl': 'CTL'}


# --- Utilities (identical to step1282) ---
...
class LPLPESubstrate:
    ...
    Condition 'pair_argmin': action selection uses count[a_{t-1}, a] instead of count[a].
    Condition 'lpl_pe_ctrl': original single-action count (Step 1282 baseline).
    """

    def __init__(self, n_actions: int, seed: int, use_pair_counts: bool = False):
        self.n_actions = n_actions
        self.use_pair_counts = use_pair_counts
        ...
        # Single-action counts (used in ctrl; also tracked in pair condition for I3)
        self.action_counts = np.zeros(n_actions, np.float32)

        # Pair counts: dict mapping prev_action → count vector of length n_actions
        # Lazily allocated — only for prev_actions that have been seen
        if use_pair_counts:
            self._pair_counts = {}  # int → np.ndarray(n_actions, float32)
            self._prev_action_pair = None  # previous action for pair tracking
    ...
    def process(self, obs_raw) -> int:
        ...
        if self.use_pair_counts:
            # Pairwise: score = count[prev_action, a] - alpha * pe_ema[a]
            if self._prev_action_pair is not None and self._prev_action_pair in self._pair_counts:
                pair_row = self._pair_counts[self._prev_action_pair]
            else:
                pair_row = np.zeros(self.n_actions, np.float32)
            score = pair_row - SELECTION_ALPHA * self.pe_ema
        else:
            # Control: score = count[a] - alpha * pe_ema[a]
        ...
        if self.use_pair_counts:
            prev = self._prev_action_pair
            if prev is not None:
                if prev not in self._pair_counts:
                    self._pair_counts[prev] = np.zeros(self.n_actions, np.float32)
                self._pair_counts[prev][action] += 1
            self._prev_action_pair = action
    ...
    def on_level_transition(self):
        ...
        self._prev_ext = None
        if self.use_pair_counts:
            self._prev_action_pair = None  # reset pair context on transition
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_pair = (condition == 'pair_argmin')
    substrate = LPLPESubstrate(n_actions=n_actions, seed=seed, use_pair_counts=use_pair)
    env = make_game(game_name)
    obs = env.reset(seed=seed)
...
def main():
    ...
    print("=" * 70)
    print("STEP 1287 — Pairwise transition count argmin")
    print("Key change: score = count[a_{t-1}, a] - alpha * pe_ema[a]")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1287 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    print("L1 Summary (sequence discovery test):")
    for game in GAMES:
        parts = []
    ...
    with open(os.path.join(RESULTS_DIR, 'step1287_results.json'), 'w') as f:
