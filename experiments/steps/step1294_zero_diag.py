"""
Step 1294 — Zero-Diagonal Recurrence: Does removing self-excitation fix inference-time lock?

Context: Step 1293 showed THREE-FACTOR recurrence INCREASES lock vs FROZEN baseline
(FROZEN=0.551 < 3F=0.748 on FT09). Root cause: W_recur[i,i] > 0 → winner self-excites
through 10 sub-steps → same winner locks. Structural, not learning-related.

Architecture (all conditions): Same as 1293 THREE-FACTOR base.
  N=64, W_drive (64x320) Oja, W_inhibit frozen, W_readout frozen, W_pred for pe.
  THREE-FACTOR: W_recur with pe-gated learning, no diagonal constraint (1293 control).
  ZERO-DIAG:   THREE-FACTOR + np.fill_diagonal(W_recur, 0.0) after each update.
  NEG-DIAG:    THREE-FACTOR + np.fill_diagonal(W_recur, -0.1) after each update.

Key metric: ZERO-DIAG lock < FROZEN baseline (0.551 on FT09) → recurrence now HELPS.

Kill criteria:
  - ZERO-DIAG lock > 3F lock (0.748) on FT09 → zero-diagonal makes it WORSE
  - ZERO-DIAG attractor k ≤ 1.5 on FT09 → removing self-excitation collapsed attractors

Known baselines from step 1293:
  FT09: 3F lock=0.748, FROZEN lock=0.551 | LS20: 3F≈1.0 | SP80: 3F≈0.49

MBPP (game 4): 1 draw per condition — 0% baseline confirmation, no prescription needed.
Budget: 3 ARC games × 3 cond × 4 draws = 36 runs (~288s)
        1 MBPP game × 3 cond × 1 draw  =  3 runs (~15s)
        Total: ~303s ≈ 5 min cap.
Note: Spec requested 5 draws (45 runs = 360s > cap). Reduced to 4 draws per budget rule.

Spec: Leo mail 3637, 2026-03-28.
"""
...
# --- Config ---
ARC_GAMES = ['ls20', 'ft09', 'sp80']
MBPP_GAMES = ['mbpp_0']
GAMES = ARC_GAMES + MBPP_GAMES

N_DRAWS = 4       # ARC games
N_DRAWS_MBPP = 1  # MBPP: always 0%, 1 draw sufficient
...
CONDITIONS = ['three_factor', 'zero_diag', 'neg_diag']
LABELS = {'three_factor': '3F', 'zero_diag': 'ZD', 'neg_diag': 'ND'}

# Known baselines from step1293 (for kill criteria comparison)
FROZEN_LOCK_BASELINE = {'ft09': 0.551, 'ls20': 1.0, 'sp80': 0.49}
TF_LOCK_1293 = {'ft09': 0.748, 'ls20': 1.0, 'sp80': 0.49}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1294')
...
def compute_solver_level_steps(game_name, seed=1):
    """Return {level: step_count} for solver oracle."""
    if game_name.lower().startswith('mbpp'):
    ...
    # ARC games: walk through prescription
...
class ZeroDiagSubstrate:
    """N=64 competitive network with pe-gated three-factor plasticity.

    Conditions:
      three_factor: same as step1293 THREE-FACTOR (control, no diagonal constraint)
      zero_diag:    THREE-FACTOR + np.fill_diagonal(W_recur, 0.0) after each update
      neg_diag:     THREE-FACTOR + np.fill_diagonal(W_recur, -0.1) after each update
    """
    ...
    def __init__(self, n_actions, seed, mode='three_factor'):
        ...
        # Recurrent weights: all three modes start with randn*0.01
        self.W_recur = rng_w.randn(NETWORK_SIZE, NETWORK_SIZE).astype(np.float32) * 0.01
        # Apply initial diagonal constraint
        if mode == 'zero_diag':
            np.fill_diagonal(self.W_recur, 0.0)
        elif mode == 'neg_diag':
            np.fill_diagonal(self.W_recur, -0.1)
    ...
    def process(self, obs_raw, current_level=0):
        ...
        # Network dynamics with recurrent input from prev_activation
        ...
        # W_recur update: pe-gated three-factor rule for all active modes
        if pe_scalar > self._pe_threshold and self._pe_threshold > 0.0 and win_act > 1e-8:
            ...
            # Diagonal constraint (applied after update for zero_diag and neg_diag)
            if self.mode == 'zero_diag':
                np.fill_diagonal(self.W_recur, 0.0)
            elif self.mode == 'neg_diag':
                np.fill_diagonal(self.W_recur, -0.1)
    ...
    def compute_diag_mean(self):
        """Mean absolute diagonal value (should be ~0 for zero_diag, ~0.1 for neg_diag)."""
        return round(float(np.mean(np.abs(np.diag(self.W_recur)))), 4)
...
def run_single(game_name, condition, draw, seed, n_actions, solver_level_steps):
    ...
    substrate = ZeroDiagSubstrate(n_actions=n_actions, seed=seed, mode=condition)
    ...
    # MBPP uses 2000-step episodes; outer loop still runs MAX_STEPS
    # Random actions won't form valid Python, so exec() never called (step_limit only)
    ...
    diag_mean = substrate.compute_diag_mean()
    ...
    diag_str = f"diag={diag_mean:.3f}"
    print(f" [{label}] Lmax={max_level} | {lock_str} | {i3_str} | {r3_str} "
          f"{sil_str} {diag_str} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 72)
    print("STEP 1294 — Zero-Diagonal Recurrence")
    print("THREE-FACTOR: pe-gated W_recur, no diagonal constraint (1293 control)")
    print("ZERO-DIAG:    THREE-FACTOR + fill_diagonal(W_recur, 0.0)")
    print("NEG-DIAG:     THREE-FACTOR + fill_diagonal(W_recur, -0.1)")
    print("=" * 72)
    print(f"ARC games: {ARC_GAMES} | N_DRAWS={N_DRAWS}")
    print(f"MBPP games: {MBPP_GAMES} | N_DRAWS_MBPP={N_DRAWS_MBPP} (0% baseline)")
    print(f"Known FROZEN lock baseline (step1293): {FROZEN_LOCK_BASELINE}")
    print(f"Key test: ZERO-DIAG lock < {FROZEN_LOCK_BASELINE['ft09']} on FT09 → recurrence HELPS")
    print()
    ...
    print(f"\n{'='*72}")
    print(f"STEP 1294 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*72}\n")

    # Summary
    print("Attractor + Lock Summary:")
    for game in GAMES:
        print(f"\n  {game.upper()}:")
        frz_baseline = FROZEN_LOCK_BASELINE.get(game.lower())
        for cond in CONDITIONS:
            ...
            diags = [r['diag_mean_abs'] for r in runs if r.get('diag_mean_abs') is not None]
            ...
            mean_diag = float(np.mean(diags)) if diags else None

            lock_flag = ""
            if mean_lock is not None and frz_baseline is not None:
                if mean_lock < frz_baseline:
                    lock_flag = " *** BELOW FROZEN BASELINE ***"
                elif cond == 'zero_diag' and mean_lock > TF_LOCK_1293.get(game.lower(), 1.0):
                    lock_flag = " [KILL: worse than 3F]"

            print(f"    [{label}] L1={l1s}/{len(runs)} | "
                  f"sil={mean_sil:.3f}(k={mean_k:.1f})" if mean_sil is not None else
                  f"    [{label}] L1={l1s}/{len(runs)} | sil=?", end='')
            print(f" | lock={mean_lock:.3f}{lock_flag}" if mean_lock is not None else " | lock=?", end='')
            print(f" | mi={mean_mi:.4f}" if mean_mi is not None else " | mi=?", end='')
            print(f" | corr={mean_corr:.3f}" if mean_corr is not None else " | corr=?", end='')
            print(f" | diag_abs={mean_diag:.3f}" if mean_diag is not None else " | diag=?")

    # Kill criteria check
    print("\nKill criteria (FT09 primary):")
    ft09_results = {c: [r for r in all_results if r['game'] == 'ft09' and r['condition'] == c]
                    for c in CONDITIONS}

    for cond in CONDITIONS:
        runs = ft09_results[cond]
        if not runs:
            continue
        locks = [r['positive_lock_fraction'] for r in runs if r.get('positive_lock_fraction') is not None]
        ks = [r['attractor_k'] for r in runs if r.get('attractor_k') is not None]
        ...
        print(f"  FT09 [{label}]: lock={lock_display}", end='')
        if mean_lock is not None:
            frz = FROZEN_LOCK_BASELINE['ft09']
            tf = TF_LOCK_1293['ft09']
            if cond == 'zero_diag':
                if mean_lock > tf:
                    print(f" KILL: lock {mean_lock:.3f} > 3F {tf:.3f} (worse)", end='')
                elif mean_lock < frz:
                    print(f" PASS: lock {mean_lock:.3f} < FROZEN {frz:.3f} (prediction 1 confirmed)", end='')
                else:
                    print(f" PARTIAL: lock {mean_lock:.3f} between FROZEN {frz:.3f} and 3F {tf:.3f}", end='')
        if mean_k is not None and cond == 'zero_diag' and mean_k <= 1.5:
            print(f" KILL: k={mean_k:.1f} (attractors collapsed)", end='')
        print()

    # FT09 lock comparison table (key result)
    print("\nFT09 lock comparison (key metric):")
    print(f"  FROZEN baseline (step1293):        lock={FROZEN_LOCK_BASELINE['ft09']:.3f}")
    print(f"  THREE-FACTOR baseline (step1293):  lock={TF_LOCK_1293['ft09']:.3f}")
    for cond in CONDITIONS:
        runs = ft09_results[cond]
        locks = [r['positive_lock_fraction'] for r in runs if r.get('positive_lock_fraction') is not None]
        mean_lock = float(np.mean(locks)) if locks else None
        label = LABELS[cond]
        if mean_lock is not None:
            vs_frozen = "BELOW" if mean_lock < FROZEN_LOCK_BASELINE['ft09'] else "ABOVE"
            print(f"  {label} (step1294):  lock={mean_lock:.3f}  [{vs_frozen} FROZEN baseline]")

    # MBPP summary
    mbpp_runs = [r for r in all_results if r['game'].startswith('mbpp')]
    if mbpp_runs:
        print("\nMBPP Baseline (0% expected):")
        for cond in CONDITIONS:
            runs = [r for r in mbpp_runs if r['condition'] == cond]
            if runs:
                l1s = sum(1 for r in runs if r.get('L1_solved'))
                arcs = [r['arc_score'] for r in runs]
                print(f"  [{LABELS[cond]}] L1={l1s}/{len(runs)} | arc={np.mean(arcs):.4f}")

    with open(os.path.join(RESULTS_DIR, 'step1294_results.json'), 'w') as f:
        json.dump({'total_elapsed': round(total_elapsed, 1), 'n_runs': len(all_results),
                   'conditions': CONDITIONS, 'games': GAMES,
                   'frozen_baseline': FROZEN_LOCK_BASELINE,
                   'tf_baseline_1293': TF_LOCK_1293}, f)
