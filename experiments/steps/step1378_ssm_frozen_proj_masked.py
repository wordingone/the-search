"""
Step 1378 — Frozen random projection: first genuine feature→action channel.
Leo mail 3935, 2026-03-30.

1377 closed COUNT direction: try1 mode has ZERO effect on try2 after PRNG fix.
Root cause: try2=random = no channel from features to actions. Null experiment.

This step tests: frozen random projection W_fixed connects h to actions.
action = softmax(W_fixed @ h_concat / T) with T=3.0 (allosteric coupling).

Two conditions (paired, 30 draws):
  FROZEN_T3:    try1=COUNT, try2=softmax(W_fixed @ h_concat / 3.0)
  DISCONNECTED: try1=COUNT, try2=random (control = 1377 RAND baseline)

Both: h reset between tries, weights carry, PRNG fix applied.
W_fixed: (n_actions, H_DIM) random matrix, same per (draw, game) for both conditions.
h_concat: concat of all SSM layer hidden states, shape (N_STATE * N_LAYERS,) = (64,).

Kill criterion: paired sign test p > 0.10.

Seeds: 13930-13959 (fresh, no overlap with 1374: 13740-13769, 1375: 13770-13799,
       1376: 13800-13829, 1377: 13830-13929).
"""
...
STEP        = 1378
N_DRAWS     = 30
DRAW_SEEDS  = [13930 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# SSM config (SSM-DIAG-RTRL, same as 1360-1377)
PROJ_DIM      = 64
ACT_EMBED_DIM = 16
D             = 128
N_STATE       = 32
...
# Frozen projection config
TEMPERATURE = 3.0
H_DIM       = N_STATE * N_LAYERS   # 64 — shape of h_concat

CONDITIONS = ['FROZEN_T3', 'DISCONNECTED']
...
class MambaSSMSubstrate:
    """
    try1_mode: 'count' (both conditions use COUNT).
    try2_mode: 'frozen' (softmax(W_fixed @ h / T)) or 'random'.
    W_fixed: (n_actions, H_DIM) frozen random projection. Same per (draw, game) across conditions.
    """

    def __init__(self, n_actions, try1_mode='count', try2_mode='random', W_fixed=None):
        self.n_actions  = n_actions
        self.try1_mode  = try1_mode
        self.try2_mode  = try2_mode
        self._W_fixed   = W_fixed       # (n_actions, H_DIM), frozen
    ...
    def process(self, obs_arr):
        ...
        if self._in_try2:
            if self.try2_mode == 'frozen' and self._W_fixed is not None:
                h_concat = np.concatenate([l.h for l in self._layers])  # (H_DIM,)
                logits = self._W_fixed @ h_concat / self._temperature   # (n_actions,)
...
def run_draw(draw_idx, draw_seed, cond_name, try2_mode, max_steps):
    ...
    for game_idx, (game_name, label) in enumerate(zip(games, game_labels.values())):
        ...
        # W_fixed: deterministic per (draw_seed, game_idx) — same for both conditions
        w_rng   = np.random.RandomState(draw_seed * 100 + game_idx)
        W_fixed = w_rng.randn(n_actions, H_DIM).astype(np.float32)

        substrate = MambaSSMSubstrate(
            n_actions=n_actions, try1_mode='count',
            try2_mode=try2_mode, W_fixed=W_fixed,
        )
        ...
        # Normalized entropy: fraction of max entropy log(n_actions)
        h_max = float(np.log(n_actions)) if n_actions > 1 else 1.0
        i4_h_early_norm = round(stage['i4_h_early'] / h_max, 4) if stage['i4_h_early'] else None
        i4_h_late_norm  = round(stage['i4_h_late']  / h_max, 4) if stage['i4_h_late']  else None
        ...
        row = {
            ...
            'i4_h_early_norm':    i4_h_early_norm,   # fraction of max entropy
            'i4_h_late_norm':     i4_h_late_norm,    # fraction of max entropy
...
def main():
    ...
    # Minimal W_fixed for timing substrate
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, try1_mode='count',
                                try2_mode='frozen', W_fixed=W_fixed_t1)
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: FROZEN_T3 vs DISCONNECTED, {N_DRAWS} draws ===")
    print(f"Seeds: {DRAW_SEEDS[0]}-{DRAW_SEEDS[-1]}")
    print(f"T={TEMPERATURE}, H_DIM={H_DIM}. Kill criterion: p > 0.10.")
    print(f"Both conditions: COUNT try1, h reset at try2, PRNG fix applied.")

    os.makedirs(RESULTS_DIR, exist_ok=True)

    rhae_by_cond   = {c: [] for c in CONDITIONS}
    # Track entropy diagnostics for FROZEN (to check if near-max)
    entropy_early_by_draw = []   # FROZEN_T3 only, normalized
    entropy_late_by_draw  = []   # FROZEN_T3 only, normalized

    for cond_name in CONDITIONS:
        try2_mode = 'frozen' if cond_name == 'FROZEN_T3' else 'random'
        print(f"\n--- Condition: {cond_name} (try2_mode={try2_mode}) ---")
        t_cond_start = time.time()
        for di, ds in enumerate(DRAW_SEEDS):
            rhae, draw_res = run_draw(di, ds, cond_name, try2_mode, max_steps)
            rhae_by_cond[cond_name].append(round(rhae, 7))
            if cond_name == 'FROZEN_T3':
                # Aggregate normalized entropy across games in this draw
                e_early_vals = [r.get('i4_h_early_norm') for r in draw_res if r.get('i4_h_early_norm') is not None]
                e_late_vals  = [r.get('i4_h_late_norm')  for r in draw_res if r.get('i4_h_late_norm')  is not None]
                entropy_early_by_draw.append(round(float(np.mean(e_early_vals)), 4) if e_early_vals else None)
                entropy_late_by_draw.append( round(float(np.mean(e_late_vals)),  4) if e_late_vals  else None)
    ...
    # Entropy diagnostic for FROZEN
    valid_early = [e for e in entropy_early_by_draw if e is not None]
    valid_late  = [e for e in entropy_late_by_draw  if e is not None]
    mean_h_early_norm = round(float(np.mean(valid_early)), 4) if valid_early else None
    mean_h_late_norm  = round(float(np.mean(valid_late)),  4) if valid_late  else None
    near_max_count = sum(1 for e in valid_early if e > 0.95)
    summary['frozen_entropy_diagnostic'] = {
        'mean_h_early_norm':   mean_h_early_norm,
        'mean_h_late_norm':    mean_h_late_norm,
        'draws_near_max_early': near_max_count,
        'total_draws':         len(valid_early),
    }
    print(f"\n  FROZEN entropy (normalized): early={mean_h_early_norm}, late={mean_h_late_norm}")
    print(f"  Near-max (>0.95) in early phase: {near_max_count}/{len(valid_early)} draws")
    if mean_h_early_norm is not None:
        if mean_h_early_norm > 0.95:
            print(f"  ** h has no structure (near-uniform). Coupling effectively disconnected. **")
        elif mean_h_early_norm > 0.80:
            print(f"  ** h has weak structure. Moderate coupling. **")
        else:
            print(f"  ** h has structure (H_norm < 0.80). Coupling is active. **")

    # ── Paired comparison ─────────────────────────────────────────────────
    frozen_list = rhae_by_cond['FROZEN_T3']
    disc_list   = rhae_by_cond['DISCONNECTED']
    frozen_wins = sum(1 for a, b in zip(frozen_list, disc_list) if a > b)
    disc_wins   = sum(1 for a, b in zip(frozen_list, disc_list) if b > a)
    ties        = sum(1 for a, b in zip(frozen_list, disc_list) if a == b)
    n_dec       = frozen_wins + disc_wins
    p_val       = binomial_p_one_sided(frozen_wins, n_dec)
    verdict     = 'SIGNAL' if p_val <= 0.10 else 'NOT_SIGNIFICANT'

    print(f"\n=== PAIRED COMPARISON ===")
    print(f"  FROZEN_T3 wins {frozen_wins}/{N_DRAWS}, DISCONNECTED wins {disc_wins}/{N_DRAWS}, "
          f"ties {ties}/{N_DRAWS}  p={p_val:.4f}")
    if verdict == 'SIGNAL':
        print(f"  *** SIGNAL (p≤0.10): Frozen projection helps. Feature→action channel works. ***")
    else:
        print(f"  NOT_SIGNIFICANT (p>0.10): Frozen projection doesn't help at T={TEMPERATURE}.")

    summary['paired_frozen_vs_disc'] = {
        'frozen_wins': frozen_wins, 'disc_wins': disc_wins, 'ties': ties,
