"""
Step 1259 — ARI embedded in 1258 composition (Option B)

Same architecture as 1258. Additional instrument: Adjusted Rand Index (ARI) between
unsupervised clusters of avgpool4 encodings and game-reported state labels (info['level']).

Question: can a representation distinguish game states without being told what a state is?

ARI computation (embedded in via_wiring_a and control_c runs):
  1. Record (enc_256D, level_label) at every step during run.
  2. At end of run, cluster enc vectors with:
     a. k-means, k = n_distinct_levels_seen  (uses game knowledge)
     b. k-means, k = 2  (minimal separation test)
     c. cosine threshold clustering, threshold = 0.5
  3. Compute ARI(cluster_assignment, level_labels) for each method.
  4. Controls:
     - random: replace enc with Gaussian noise → ARI ≈ 0
     - identity: use level_label as cluster → ARI = 1.0

ARI > 0 with p < 0.05 = constructive proof that the encoding distinguishes states.

Same games (ls20, vc33, sp80), same 5 draws, same stage instrumentation as 1258.
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1259')
...
class ViaWiringA:
    ...
    def process(self, obs_raw) -> int:
        ...
        # C_GPR: Salience-gated selection on attended_mod (EXT_DIM)
...
def cosine_threshold_cluster(vecs, threshold=0.5):
    """Greedy cosine threshold clustering. Returns integer label array."""
    n = len(vecs)
    if n == 0:
        return np.array([], dtype=np.int32)
    norms = np.linalg.norm(vecs, axis=1, keepdims=True)
    safe = norms > 1e-8
    normed = np.where(safe, vecs / (norms + 1e-8), 0.0)
    centroids = [normed[0]]
    labels = np.zeros(n, dtype=np.int32)
    for i in range(1, n):
        v = normed[i]
        best_dist = min(float(1.0 - np.dot(v, c)) for c in centroids)
        if best_dist > threshold:
            labels[i] = len(centroids)
            centroids.append(v)
        else:
            labels[i] = int(np.argmin([float(1.0 - np.dot(v, c)) for c in centroids]))
    return labels


def compute_ari(enc_log, level_log, rng_seed=42):
    """
    ARI between unsupervised clusters of enc_log (256D) and level_log labels.
    Three clustering methods + 2 controls.
    enc_log: list of 256D arrays (raw enc, before attention)
    level_log: list of int level labels (same length)
    """
    if len(enc_log) < 10:
        return {'ari_k_known': None, 'ari_k2': None, 'ari_cosine': None,
                'ari_random': None, 'ari_identity': None,
                'n_samples': len(enc_log), 'n_distinct_levels': 0}

    X = np.array(enc_log, dtype=np.float32)
    y = np.array(level_log, dtype=np.int32)
    n_samples = len(X)
    distinct_levels = sorted(set(y.tolist()))
    ...
    def safe_ari(labels_true, labels_pred):
        try:
            return round(float(adjusted_rand_score(labels_true, labels_pred)), 4)
        except Exception:
            return None

    # Method A: k-means with k = n_distinct_levels
    if n_levels >= 2:
        km_a = KMeans(n_clusters=n_levels, random_state=rng_seed, n_init=5).fit(X)
        ari_k_known = safe_ari(y, km_a.labels_)
    else:
        ari_k_known = None

    # Method B: k-means with k = 2
    km_b = KMeans(n_clusters=2, random_state=rng_seed, n_init=5).fit(X)
    ari_k2 = safe_ari(y, km_b.labels_)

    # Method C: cosine threshold clustering (subsample to 2000 for speed)
    sub_idx = rng.choice(n_samples, min(n_samples, 2000), replace=False)
    cos_labels = cosine_threshold_cluster(X[sub_idx], threshold=0.5)
    ari_cosine = safe_ari(y[sub_idx], cos_labels)

    # Control 1: random Gaussian vectors
    X_rand = rng.randn(*X.shape).astype(np.float32)
    if n_levels >= 2:
        km_rand = KMeans(n_clusters=n_levels, random_state=rng_seed, n_init=5).fit(X_rand)
        ari_random = safe_ari(y, km_rand.labels_)
    else:
        ari_random = None

    # Control 2: identity (level_label as cluster) → should be 1.0
    ari_identity = safe_ari(y, y)

    return {
        'ari_k_known': ari_k_known,
        'ari_k2': ari_k2,
        'ari_cosine': ari_cosine,
        'ari_random': ari_random,
        'ari_identity': ari_identity,
        'n_samples': n_samples,
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta):
    ...
    obs_store = []
    level_actions = {}
    enc_ari_log = []    # (256D enc) per step for ARI
    level_ari_log = []  # level label per step for ARI
    ...
    while steps < MAX_STEPS:
        ...
        # ARI: collect enc after process() + current level label
        enc_src = getattr(substrate, '_prev_enc', None)
        if enc_src is None:
            enc_src = getattr(substrate, '_last_enc', None)
        if enc_src is not None:
            enc_ari_log.append(enc_src.copy())
            level_ari_log.append(level)
    ...
    ari_result = compute_ari(enc_ari_log, level_ari_log)
    ...
    result = {
        ...
        # ARI: unsupervised encoding distinguishes states?
        'ARI_k_known': ari_result['ari_k_known'],
        'ARI_k2': ari_result['ari_k2'],
        'ARI_cosine': ari_result['ari_cosine'],
        'ARI_random': ari_result['ari_random'],
        'ARI_identity': ari_result['ari_identity'],
        'ARI_n_levels': ari_result['n_distinct_levels'],
    }
    ...
    ari_str = f"ARI={ari_result['ari_k_known']:.3f}" if ari_result['ari_k_known'] is not None else "ARI=null"
    conc_str = f"conc={attn_stats.get('concentration', 0):.2f}" if attn_stats else ""
    print(f" {l1_str} | {i3_str} | {r3_str} | {sal_str} | {ari_str} | {conc_str} | {elapsed:.1f}s")
...
def main():
    ...
    print("=" * 70)
    print("STEP 1259 — ARI EMBEDDED IN 1258 COMPOSITION (Option B)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print()
    print("Same architecture as 1258. Additional instrument: ARI(unsup clusters, level labels).")
    print()
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1259 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    # ARI summary
    print()
    print("ARI (encoding cluster vs level label):")
    for method in ['ARI_k_known', 'ARI_k2', 'ARI_cosine']:
        vals = [r[method] for r in all_results
                if r['condition'] == 'via_wiring_a' and r.get(method) is not None]
        ctrl_rand = [r['ARI_random'] for r in all_results
                     if r['condition'] == 'via_wiring_a' and r.get('ARI_random') is not None]
        if vals:
            print(f"  {method}: mean={np.mean(vals):.4f}±{np.std(vals):.4f}  (random baseline={np.mean(ctrl_rand):.4f})")
        else:
            print(f"  {method}: null")

    out_path = os.path.join(RESULTS_DIR, 'step1259_results.json')
