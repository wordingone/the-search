"""
Step 1373 — Count-based exploration: argmin(visit_count).
Leo mail 3915, 2026-03-30.

Qualitatively different from prediction-based rollout. No SSM state used for selection.
COUNT condition: always choose least-visited action (tie-broken randomly).
DISC condition: entropy softmax on fixed random W_act.

Same 30 seeds 13700-13729 (paired with 1370/1371/1372).

Kill criteria:
  - COUNT wins > DISC wins → systematic exploration helps. SIGNAL.
  - COUNT ≈ DISC → count-based = random on this game pool. KILL.
"""
...
STEP        = 1373
N_DRAWS     = 30
DRAW_SEEDS  = [13700 + i for i in range(N_DRAWS)]  # Same as 1370-1372
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
CONDITIONS = ['COUNT', 'SSM-DISCONNECTED']
...
class MambaSSMSubstrate:
    ...
    def __init__(self, n_actions, mode='count'):
        ...
        # Count diversity: how many unique actions chosen per checkpoint
        self._try1_unique_actions = set()
        self._try2_unique_actions = set()
        self._try1_unique_at = {}
        self._try2_unique_at = {}
    ...
    def process(self, obs_arr):
        ...
        if self._step < WARMUP:
            ...
            # Count-based: argmin(visit_count), tie-broken randomly
        ...
        if self._step >= WARMUP:
            if self._in_try2:
                ...
                self._try2_unique_actions.add(action)
            else:
                self._try1_autocorr_steps += 1
                if action == self._prev_action:
                    self._try1_autocorr_count += 1
                self._try1_unique_actions.add(action)

        # Checkpoints
        if self._step in ENT_CHECKPOINTS:
            n_unique = len(self._try2_unique_actions if self._in_try2 else self._try1_unique_actions)
            if self._in_try2:
                self._try2_ent[self._step] = n_unique
            else:
                self._try1_ent[self._step] = n_unique
    ...
    def on_level_transition(self):
        for layer in self._layers:
            layer.reset_state()
        self._prev_y = None
        # Note: visit counts NOT reset on level transition — persistent across levels
    ...
    def reset_for_try2(self):
        ...
        self._prev_action = 0
        self._in_try2 = True
        self._visit_count[:] = 0  # Reset counts for try2
        self._try2_unique_actions = set()
        self._try2_autocorr_count = 0
        self._try2_autocorr_steps = 0
    ...
    def get_stats(self):
        ...
        return {
            'try1_unique_actions': self._try1_ent,  # n_unique at each checkpoint
            'try2_unique_actions': self._try2_ent,
...
def run_draw(draw_idx, draw_seed, cond_name, mode, max_steps):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        substrate = MambaSSMSubstrate(n_actions=n_actions, mode=mode)
        ...
        row = {
            ...
            't1': t1, 't2': t2,
            'try1_unique_1999': stats['try1_unique_actions'].get(1999),
            'try2_unique_1999': stats['try2_unique_actions'].get(1999),
        ...
        print(f"    [{cond_name}] {label}: p1={p1} p2={p2} eff_sq={eff_sq:.6f} pred_loss={stats['pred_loss_final']} unique@1999={stats['try2_unique_actions'].get(1999)} ({elapsed_total}s)")
...
def main():
    # ── Tier 1: timing check ──────────────────────────────────────────────
    print("=== TIER 1: timing check (COUNT mode) ===")
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, mode='count')
    ...
    est_total_s = ms_per_step / 1000 * TRY1_STEPS * n_eps_per_cond * 2  # both conditions ~same speed
    print(f"  {TIER1_STEPS} steps (COUNT): {tier1_elapsed:.2f}s  ({ms_per_step:.2f}ms/step)")
    print(f"  Estimated total: {est_total_s:.0f}s ({est_total_s/60:.1f} min)")
    print(f"  Note: COUNT is ~same speed as DISC baseline (no rollout)")

    if est_total_s > MAX_SECONDS:
        max_steps = max(WARMUP + 50, int((MAX_SECONDS * 0.9) / (ms_per_step / 1000 * n_eps_per_cond * 2)))
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: count-based vs disconnected, 30 draws ===")
    print(f"COUNT: argmin(visit_count), tie-broken randomly. No prediction used for selection.")
    print(f"Same seeds 13700-13729 as 1370-1372 for cross-experiment comparison.")
    print(f"N_ACTIONS up to {MAX_N_ACTIONS} — for large spaces, COUNT ≈ random (all ties at 0).")
    ...
    for cond_name in CONDITIONS:
        mode = 'entropy' if 'DISCONNECTED' in cond_name else 'count'
        print(f"\n--- Condition: {cond_name} (mode={mode}) ---")
        for di, ds in enumerate(DRAW_SEEDS):
            rhae, draw_res = run_draw(di, ds, cond_name, mode, max_steps)
    ...
    # Paired comparison
    if 'COUNT' in rhae_by_cond and 'SSM-DISCONNECTED' in rhae_by_cond:
        count = rhae_by_cond['COUNT']
        disc = rhae_by_cond['SSM-DISCONNECTED']
        count_wins = sum(1 for r, d in zip(count, disc) if r > d)
        disc_wins = sum(1 for r, d in zip(count, disc) if d > r)
        ties = sum(1 for r, d in zip(count, disc) if r == d)
        print(f"\n  Paired: COUNT wins {count_wins}/{N_DRAWS}, DISCONNECTED wins {disc_wins}/{N_DRAWS}, ties {ties}/{N_DRAWS}")
        if count_wins > disc_wins:
            print(f"  *** COUNT > DISC — systematic exploration helps ***")
        elif disc_wins > count_wins:
            print(f"  *** DISC > COUNT — entropy > count-based ***")
        else:
            print(f"  *** COUNT ≈ DISC — tied ***")
        summary['paired_count_wins'] = count_wins
        summary['paired_disc_wins'] = disc_wins
        summary['paired_ties'] = ties
        summary['verdict'] = 'COUNT_BETTER' if count_wins > disc_wins else ('DISC_BETTER' if disc_wins > count_wins else 'TIED')
