"""
Step 1361 — SSM+RTRL with act_head in RTRL graph.
Leo mail 3876 + amendment 3880, 2026-03-30.

Amendment: act_head MUST be in RTRL graph (not fixed random).
Fixed head = random projection of prediction-optimized features = flat entropy = same MLP failure.
...
STEP        = 1361
N_DRAWS     = 10
DRAW_SEEDS  = [13620 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
ACT_LR_SCALE    = 1.0      # relative weight of action loss vs obs prediction loss
...
def run_draw(draw_idx, draw_seed, max_steps):
    ...
    for game_name, label in zip(games, game_labels.values()):
        ...
        row = {
            ...
            'try1_act_ent_2000': stats['try1_act_ent'].get(2000),
            ...
            'try2_act_ent_2000': stats['try2_act_ent'].get(2000),
            'try1_state_norm_100':  stats['try1_state_norm'].get(100),
            'try1_state_norm_2000': stats['try1_state_norm'].get(2000),
            'try2_state_norm_100':  stats['try2_state_norm'].get(100),
            'try2_state_norm_2000': stats['try2_state_norm'].get(2000),
            'pred_loss_mean':  stats['pred_loss_mean'],
            'pred_loss_final': stats['pred_loss_final'],
            'act_loss_mean':   stats['act_loss_mean'],
            'act_loss_final':  stats['act_loss_final'],
        }
