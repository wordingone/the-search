"""
Step 1367 — SSM forward rollout for action selection (model-predictive control).
Leo mail 3902, 2026-03-30.

Same SSM as 1364 (disconnected, 2L, D=128, N=32, RTRL). Action selection changed:
at each real step, simulate K=10 dream steps per N_CANDIDATES=32 candidate actions.
Score = cumulative prediction divergence during dream. Select argmin (predictable)
or argmax (unpredictable).

Dream step interpretation: first step uses real obs + candidate action embed; subsequent
K-1 steps use SSM's own prediction as next obs input (no real obs needed).
Score = sum(||pred_{k+1} - pred_k||) over K-1 dream transitions.

Two conditions: ROLLOUT-ARGMIN (lowest divergence = most predictable trajectory),
ROLLOUT-ARGMAX (highest divergence = most novel/exploratory).

WARMUP reduced to 200 (from 500) because rollout adds per-step overhead;
Tier 1 timing may cap max_steps lower than 2K.
"""
...
STEP        = 1367
N_DRAWS     = 10
DRAW_SEEDS  = [13670 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# Rollout config
K_ROLLOUT    = 10    # dream steps per candidate
N_CANDIDATES = 32    # candidate actions sampled per real step

# Conditions
CONDITIONS = ['ROLLOUT-ARGMIN', 'ROLLOUT-ARGMAX']
...
def main():
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: SSM forward rollout MPC ===")
    ...
    for cond_name in CONDITIONS:
        rollout_mode = 'argmin' if 'ARGMIN' in cond_name else 'argmax'
        print(f"\n--- Condition: {cond_name} ---")
