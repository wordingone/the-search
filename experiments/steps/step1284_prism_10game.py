"""
Step 1284 — KB action injection diagnostic.

Wall identified: L1 on 7/10 games requires KB actions (indices 0-6 in 4103-action space).
PE-weighted argmin never selects KB actions because they produce minimal encoding change.

Diagnostic: every 50 steps, override PE-weighted argmin with a random KB action (index 0-6).
- 200 forced KB visits in 10K steps (~29 visits per KB action)
- 9800 steps: normal PE-guided click exploration
- Answers: is KB underexploration the wall, or do games need KB action SEQUENCES?

This is a DIAGNOSTIC, not a composition. Hardcoded KB index range is a frozen-frame hack.
If it works, we need a substrate-intrinsic mechanism that achieves the same effect.

Conditions:
  kb_inject — PE-weighted argmin + forced KB action every 50 steps
  lpl_pe    — PE-weighted argmin only (Step 1282 eta_h=0.05 baseline)

Kill criterion: FT09 L1 < 3/5 OR R3 regression on any currently passing game.
...
KB_INDEX_MAX = 7       # KB actions: indices 0-6 inclusive
KB_INJECT_INTERVAL = 50  # Every 50 steps, force a random KB action
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1284')
...
class LPLPESubstrate:
    """LPL PE substrate (Step 1282 composition: eta_h=0.05, alpha=0.1).

    Supports optional forced_action for KB injection diagnostic.
    """
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        self._prev_enc = None
        self.step = 0
        self.W_pred = rng_w.randn(ENC_DIM, ENC_DIM).astype(np.float32) * 0.01  # 256×256
    ...
    def process(self, obs_raw, forced_action: int = None) -> int:
        obs = np.asarray(obs_raw, dtype=np.float32)
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    use_kb_inject = (condition == 'kb_inject')
    kb_rng = np.random.RandomState(seed + 99999) if use_kb_inject else None
    ...
    while steps < MAX_STEPS:
        ...
        # KB injection: every 50 steps, force a random KB action (index 0-6)
        if use_kb_inject and steps % KB_INJECT_INTERVAL == 0:
            kb_action = int(kb_rng.randint(0, KB_INDEX_MAX))
            action = substrate.process(obs_arr, forced_action=kb_action) % n_actions
            kb_injection_count += 1
            kb_actions_injected.append(kb_action)
        else:
    ...
    label = 'KBI' if condition == 'kb_inject' else 'LPE'
    ...
    kb_str = f"KB={kb_injection_count}" if use_kb_inject else ""
    print(f" [{label}] Lmax={max_level} | {i3_str} | {r3_str} | {arc_str} | {kb_str} | {elapsed:.1f}s")
    ...
    kb_visit_counts = {}
    if use_kb_inject:
        for a in range(KB_INDEX_MAX):
            kb_visit_counts[a] = int(substrate.action_counts[a])
    ...
    return {
        ...
        'kb_visit_counts': kb_visit_counts,
    }
...
def main():
    ...
    print("=" * 70)
    print("STEP 1284 — KB action injection diagnostic")
    print(f"KB range: indices 0-{KB_INDEX_MAX-1}, injected every {KB_INJECT_INTERVAL} steps")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    for game_name in GAMES:
        ...
        for condition in ['kb_inject', 'lpl_pe']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'kb_inject' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1284 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # --- Summary ---
    print("L1 Summary (key diagnostic metric):")
    for game in GAMES:
        for cond in ['kb_inject', 'lpl_pe']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'KBI' if cond == 'kb_inject' else 'LPE'
    ...
    print("\nKB visit counts (kb_inject condition):")
    for game in GAMES:
        runs = [r for r in all_results if r['game'] == game and r['condition'] == 'kb_inject']
        if runs and runs[0].get('kb_visit_counts'):
            avg_visits = {k: round(float(np.mean([r['kb_visit_counts'].get(str(k), 0) for r in runs])), 1)
                         for k in range(KB_INDEX_MAX)}
            print(f"  {game.upper()}: {avg_visits}")

    with open(os.path.join(RESULTS_DIR, 'step1284_results.json'), 'w') as f:
