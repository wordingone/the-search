"""
Step 1372 — K=1 single-step prediction divergence.
Leo mail 3913, 2026-03-30.

Score = ||SSM_step(obs, embed[a_i]).predicted_obs - current_obs|| for N=8 candidates.
argmax = action predicted to most change the observation.
No dream compounding — single step only.

Same 30 seeds 13700-13729. Paired vs SSM-DISCONNECTED.

Reference: K=3 N=8 (1370): ROLLOUT 4/30, DISC 2/30 (NOT MONOTONIC).
           K=5 N=8 (1371): ROLLOUT 1/30, DISC 5/30 (WORSE).
"""
...
STEP        = 1372
N_DRAWS     = 30
DRAW_SEEDS  = [13700 + i for i in range(N_DRAWS)]  # Same as 1370/1371
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
# K=1: single step, N=8 candidates
N_CANDIDATES = 8

CONDITIONS = ['K1-ARGMAX', 'SSM-DISCONNECTED']
...
class SSMLayer:
    ...
    def dream_forward(self, x):
        """Forward without RTRL storage. Updates h in place."""
        delta_pre = np.clip(self.W_delta @ x + self.b_delta, -20.0, 20.0)
        delta = np.log1p(np.exp(delta_pre))
        A_diag = np.exp(-delta * self.A_param)
...
class MambaSSMSubstrate:
    ...
    def __init__(self, n_actions, mode='k1_argmax'):
        ...
        self._prev_action = 0
        self._prev_y      = None
        self._prev_proj_obs = None  # needed for K=1 scoring
        ...
        self._try1_score_mean = {}
        self._try2_score_mean = {}
    ...
    def _k1_score(self, proj_obs, a_cand):
        """K=1: score = ||pred_after_action - current_obs||.
        Measures how much the SSM predicts this action will change the observation."""
        ...
        pred = self._W_pred @ y + self._b_pred
        score = float(np.linalg.norm(pred - proj_obs))
    ...
    def process(self, obs_arr):
        ...
        if self._step < WARMUP:
            ...
            # K=1 argmax: pick action with highest predicted change
            candidates = np.random.randint(0, self.n_actions, size=N_CANDIDATES)
            scores = np.array([self._k1_score(proj_obs, int(c)) for c in candidates],
                              dtype=np.float32)
        ...
        if self._step in ENT_CHECKPOINTS and self._step >= WARMUP:
            ...
            test_scores = [self._k1_score(proj_obs, int(c)) for c in test_cands]
            ...
            if self._in_try2:
                ...
                self._try2_score_mean[self._step] = round(score_mean, 6)
            else:
                self._try1_ent[self._step] = ent_proxy
                self._try1_score_mean[self._step] = round(score_mean, 6)
    ...
    def get_stats(self):
        ...
        return {
            ...
            'try1_score_mean': self._try1_score_mean,
            'try2_score_mean': self._try2_score_mean,
...
def main():
    # ── Tier 1: timing check ──────────────────────────────────────────────
    print("=== TIER 1: timing check (K=1 single-step scoring) ===")
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, mode='k1_argmax')
    ...
    est_k1_s = ms_per_step / 1000 * TRY1_STEPS * n_eps_per_cond
    est_disc_s = ms_per_step * 0.15 / 1000 * TRY1_STEPS * n_eps_per_cond
    est_total_s = est_k1_s + est_disc_s

    print(f"  {TIER1_STEPS} steps (K=1 N=8): {tier1_elapsed:.2f}s  ({ms_per_step:.2f}ms/step)")
    print(f"  Estimated: K1={est_k1_s:.0f}s  DISC~{est_disc_s:.0f}s  TOTAL={est_total_s:.0f}s ({est_total_s/60:.1f} min)")
    print(f"  Comparison: K=3 was 0.60ms, K=5 was 0.96ms, K=1 est={ms_per_step:.2f}ms")

    if est_total_s > MAX_SECONDS:
        max_steps = max(WARMUP + 50, int((MAX_SECONDS * 0.85) / (ms_per_step / 1000 * n_eps_per_cond)))
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: K=1 prediction divergence vs disconnected, 30 draws ===")
    print(f"Score = ||pred(obs, a_i) - current_obs||. No dream compounding.")
    print(f"N_CANDIDATES={N_CANDIDATES}, WARMUP={WARMUP}, N_DRAWS={N_DRAWS}")
    print(f"SSM: D={D}, N_STATE={N_STATE}, N_LAYERS={N_LAYERS}")
    print(f"Reference: K=3 N=8 (1370) ROLLOUT 4/30 DISC 2/30 | K=5 N=8 (1371) ROLLOUT 1/30 DISC 5/30")
    ...
    for cond_name in CONDITIONS:
        mode = 'entropy' if 'DISCONNECTED' in cond_name else 'k1_argmax'
    ...
    summary = {
        ...
        'ms_per_step_k1': round(ms_per_step, 2),
    ...
    # Paired comparison
    if 'K1-ARGMAX' in rhae_by_cond and 'SSM-DISCONNECTED' in rhae_by_cond:
        k1 = rhae_by_cond['K1-ARGMAX']
        disc = rhae_by_cond['SSM-DISCONNECTED']
        k1_wins = sum(1 for r, d in zip(k1, disc) if r > d)
        disc_wins = sum(1 for r, d in zip(k1, disc) if d > r)
        ties = sum(1 for r, d in zip(k1, disc) if r == d)
        print(f"\n  Paired: K1 wins {k1_wins}/{N_DRAWS}, DISCONNECTED wins {disc_wins}/{N_DRAWS}, ties {ties}/{N_DRAWS}")
        print(f"  Reference: K=3 N=8 (1370): ROLLOUT 4, DISC 2 | K=5 N=8 (1371): ROLLOUT 1, DISC 5")

        if disc_wins == 0 and k1_wins >= 6:
            verdict_str = 'CONFIRMED_MONOTONIC'
        elif k1_wins > 4 and disc_wins < 2:
            verdict_str = 'IMPROVED_OVER_K3'
        elif disc_wins > k1_wins:
            verdict_str = 'WORSE_THAN_DISC'
        elif k1_wins > disc_wins:
            verdict_str = 'K1_BETTER'
        else:
            verdict_str = 'NO_ADVANTAGE'

        print(f"  Verdict: {verdict_str}")
        summary['paired_k1_wins'] = k1_wins
        summary['paired_disc_wins'] = disc_wins
        summary['paired_ties'] = ties
        summary['verdict'] = verdict_str
