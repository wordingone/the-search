"""
Step 1258 — Combined Attention post-recurrent on ext_enc (320D) (Option A)

Architecture change from 1257: apply combined attention AFTER recurrent, on ext_enc (320D).

  1. enc computed (C4 + C22)
  2. h = tanh(W_h @ h + W_x @ enc)   — no attention before recurrent
  3. ext_enc = [enc, h]               — 320D unattended
  4. C25: alpha_pe (EXT_DIM) updated from prediction error on ext_enc
  5. via_att_extended = [via_attention (ENC_DIM), ALPHA_FLOOR_H × ones(H_DIM)]
     where ALPHA_FLOOR_H = 1.0 (h-dims not attenuated by viability floor)
  6. combined = alpha_pe × via_att_extended  (EXT_DIM)
  7. attended = ext_enc * combined           (EXT_DIM, unnormalized)

ALPHA_FLOOR_H = 1.0 (NOT ALPHA_FLOOR = 0.10):
  With ALPHA_FLOOR on h-dims: combined_fresh[h] = 1.0 × 0.1 = 0.1, same 10x reduction.
  With ALPHA_FLOOR_H=1.0: combined_fresh[h] = 1.0 × 1.0 = 1.0, same scale as 1254.
  The h-component Jacobian diff = (alpha_pe * sech²_exp - sech²_fresh) × W_x × J_enc
  — identical to 1254's R3 mechanism.

Fresh R3 baseline: alpha_pe=ones, via_alpha=zeros, h=zeros.
  attended_fresh[h-dims] = h_new_fresh * (1.0 × 1.0) = tanh(W_x @ enc_fresh)  ← nonzero
  attended_exp[h-dims] = h_new_exp * (alpha_pe × 1.0) — large difference when h saturates.

  1254: C25 on ext_enc post-recurrent → R3=1.00, SAL=0.20
  1255-1257: pre-recurrent on enc → R3=0.00 (ALPHA_FLOOR attenuates 10x)
  1258: C25 × via_att post-recurrent on ext_enc, ALPHA_FLOOR_H=1.0 → R3=?, SAL=?
"""
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1258')
...
def main():
    ...
    print("=" * 70)
    print("STEP 1258 — COMBINED ATTENTION POST-RECURRENT on ext_enc (Option A)")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1258 COMPLETE — {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")
    ...
    out_path = os.path.join(RESULTS_DIR, 'step1258_results.json')
