"""
Step 1323 — CNN SELFSUP-ENT probe: does ANY substrate show second_exposure_speedup?
Leo mail 3746, 2026-03-29.

## Question

R2-compliant prediction (LPL K=5, K=50) cannot close the compression gap vs CNN.
Hypothesis: shared representation needs strong compression to produce useful h1.
Before searching for R2-compliant replacements, test the probe: does CNN (KNOWN to
compress 98% and reach L1) show speedup > 1 on second exposure?

If yes: learning-from-experience IS possible, find R2-compliant version.
If no: even CNN doesn't learn from experience. Problem is deeper than R2.

## Architecture

CNN SELFSUP-ENT from step 1305/1306:
...
- 2K steps try1, 2K steps try2 (reduced from spec's 5K for 5-min cap compliance)
- Runtime note: CNN is ~1 min/episode on CPU vs Leo's 40s estimate.
  2K steps × 2 tries × 3 games ≈ 6 episodes × ~60s = ~6 min.
  Using MAX_SECONDS=60 per episode to enforce cap.
- One metric: second_exposure_speedup
- 3 runs (3 games × 1 condition), each with try1+try2

## Kill criteria
- speedup null on ALL games → bad game draw (CNN can't reach L1 in 2K steps here)
- speedup ≤ 1.0 → CNN doesn't learn from experience → problem is deeper than R2
- speedup > 1.0 → CNN learns from experience → find R2-compliant alternative
"""
...
GAMES, GAME_LABELS = select_games(seed=1323)

STEP = 1323
MAX_STEPS   = 2_000   # per try — reduced from spec's 5K for runtime cap compliance
MAX_SECONDS = 90      # per episode (hard cap to keep total < 5 min)

CONDITIONS = ['cnn_ent']
LABELS     = {'cnn_ent': 'CNN-ENT'}

RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1323')
...
# CNN hyperparameters (same as step 1306)
...
def main():
    ...
    print(f"Step {STEP} — CNN SELFSUP-ENT probe (R2-violating, explicit flag)")
    ...
    print(f"R2 VIOLATION FLAGGED — this is a capability probe, not a candidate.")
    print(f"Primary metric: second_exposure_speedup (try1 L1 / try2 L1, same substrate)")
    print(f"3 runs (3 games × 1 condition), try1+try2 per run ({MAX_STEPS} steps each).")
    print()
    ...
    for game in GAMES:
        ...
        speedup_by_game[label] = result['second_exposure_speedup']
    ...
    # Single condition — speedup chain aggregate
    speedup_vals = [v for v in speedup_by_game.values()
                    if v is not None and v != float('inf') and v > 0]
    if speedup_vals:
        chain_speedup = round(float(np.mean(speedup_vals)), 4)
    elif any(v == float('inf') for v in speedup_by_game.values() if v is not None):
    ...
    sep = '=' * 100
    print(f"\n{sep}")
    print(f"STEP {STEP} — RESULT (R2-violating probe)\n")
    print(f"  CNN-ENT: second_exposure_speedup = {format_speedup(chain_speedup)}")
    print()
    print("  Per-game breakdown:")
    for label in sorted(speedup_by_game):
        sp = speedup_by_game[label]
    ...
    if chain_speedup is None:
        print("  >>> KILL: speedup=N/A (no L1 in 2K steps on any game)")
        print("  >>> Bad game draw or steps too few. CNN can't answer the question here.")
    elif chain_speedup == float('inf'):
        print("  >>> SIGNAL: speedup=inf (try1 failed, try2 succeeded) — possible learning effect")
        print("  >>> NO KILL — CNN reaches L1 on second exposure. Investigate further.")
    elif chain_speedup > 1.0:
        print(f"  >>> SIGNAL: speedup={chain_speedup:.4f} > 1.0 — CNN LEARNS FROM EXPERIENCE")
        print("  >>> NO KILL — Find R2-compliant alternative that produces same effect.")
    else:
        print(f"  >>> KILL: speedup={chain_speedup:.4f} ≤ 1.0 — CNN does NOT learn from experience")
        print("  >>> Problem is deeper than R2. Even CNN can't produce speedup.")
    print(sep)
    print(f"\nResults: {RESULTS_DIR}\n")
