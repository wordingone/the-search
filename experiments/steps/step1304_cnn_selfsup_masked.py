"""
Step 1304 — CNN Self-Supervised Forward Prediction (Masked PRISM)
Leo mail 3675+3676, Jun directive, 2026-03-29.

Same mechanism as Step 1303 (CNN backbone + forward prediction loss, no reward).
Corrected protocol:
  1. 3 games selected RANDOMLY from solved pool (seed 1304)
  2. Output masked: stdout shows "Game A/B/C" — never real IDs
  3. Kill criteria: R3, RHAE, weight drift, action KL — no L1 counts
  4. Chain-level aggregates only in output
  5. New metrics: weight_drift (total param change from init), action_KL (early vs late distribution)

Kill criteria:
  1. R3 < 0.01 AND wdrift < 0.1 on all games → CNN genuinely not learning → KILL
  2. R3 < 0.01 BUT wdrift > 1.0 → R3 miscalibrated on CNN, investigate, don't kill
  3. RHAE < 1e-6 chain mean → RHAE-dead → KILL
  4. action_KL < 0.01 on all games → no behavioral change → KILL
"""
...
# --- Random game selection from solved pool (seed 1304) ---
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1304')
...
class SgSelfSupSubstrate:
    ...
    def compute_weight_drift(self):
        """Total parameter norm change from init weights."""
        drift = 0.0
        for name, param in self._model.named_parameters():
            init_val = self._init_state_dict[name]
...
def compute_action_kl(action_log, n_actions):
    ...
    Returns None if insufficient data. Small KL = behavior unchanged.
    """
...
def run_episode(env, substrate, condition, n_actions, solver_level_steps, seed,
    ...
    rhae = arc_score  # RHAE = arc_score (efficiency²)
...
def main():
    ...
    print(f"Step 1304 — CNN Self-Supervised Forward Prediction (Masked PRISM)")
    ...
    for game in GAMES:
        try:
            ...
            # Masked progress: don't print per-game baselines
    ...
    chain = {c: {'r3_vals': [], 'rhae_vals': [], 'wdrift_vals': [],
    ...
    for game in GAMES:
        ...
        for draw in range(N_DRAWS):
            ...
            # Masked progress line: no game name, just label and metrics
            ss = draw_results.get('sg_selfsup', {}).get('episode_A', {})
            pe = draw_results.get('argmin_pe', {}).get('episode_A', {})
            print(f"  {label} draw {draw}: "
                  f"SS[R3={ss.get('R3')},wdrift={ss.get('wdrift')},kl={ss.get('action_kl')},RHAE={ss.get('RHAE',0):.2e}] "
        ...
        out_path = os.path.join(RESULTS_DIR, f"{game}_1304.jsonl")
    ...
    # ---------------------------------------------------------------------------
    # Chain-level summary (aggregate only — no per-game breakdown)
    # ---------------------------------------------------------------------------
    print("\n" + "="*100)
    print("STEP 1304 — CHAIN-LEVEL SUMMARY (aggregate across all games)")
    print()
    ...
    for condition in CONDITIONS:
        ...
        print(f"    mean_R3={mean_r3}  mean_RHAE={mean_rhae:.2e}  mean_wdrift={mean_wdrift}  mean_action_KL={mean_action_kl}  mean_I3cv={mean_i3cv}")
    ...
    pe = chain_summary.get('argmin_pe', {})

    ss_r3 = ss.get('mean_R3')
    ...
    if ss_r3 is not None and ss_wdrift is not None:
        if ss_r3 < 0.01 and ss_wdrift < 0.1:
            print(f"  >>> KILL: R3={ss_r3} < 0.01 AND wdrift={ss_wdrift} < 0.1 → CNN not learning")
            killed = True
        elif ss_r3 < 0.01 and ss_wdrift > 1.0:
            print(f"  >>> INVESTIGATE: R3={ss_r3} < 0.01 BUT wdrift={ss_wdrift} > 1.0 → R3 miscalibrated for deep CNN")
        elif ss_r3 >= 0.01:
            print(f"  >>> R3 PASS: R3={ss_r3} ≥ 0.01")

    if ss_rhae is not None and ss_rhae < 1e-6:
        print(f"  >>> KILL: RHAE={ss_rhae:.2e} < 1e-6 → RHAE-dead")
        killed = True

    if ss_kl is not None and ss_kl < 0.01:
        print(f"  >>> KILL: action_KL={ss_kl} < 0.01 → no behavioral change")
        killed = True

    if not killed:
        print("  >>> NO KILL triggered — proceed to full PRISM chain if R3 or wdrift shows signal")
    ...
    with open(summary_path, 'w') as f:
        json.dump({
            ...
            'n_draws': N_DRAWS,
            'games_label_only': list(GAME_LABELS.values()),  # masked
        }, f, indent=2)
