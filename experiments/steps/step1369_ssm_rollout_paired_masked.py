"""
Step 1369 — Paired comparison: ROLLOUT-ARGMAX vs SSM-DISCONNECTED on same seeds.
Leo mail 3907, 2026-03-30.

Within-experiment paired comparison to control for draw variance.
Same 10 seeds (13690-13699) for BOTH conditions.
Only difference: action selection (rollout argmax vs entropy).

ROLLOUT-ARGMAX: K=3, N=8 (1368 config). 2K steps.
SSM-DISCONNECTED: same SSM, entropy softmax action selection. 2K steps.

Kill: ROLLOUT == DISCONNECTED on same draws → rollout adds nothing.
Signal: ROLLOUT > DISCONNECTED (more nz draws or higher chain mean).
"""
...
STEP        = 1369
N_DRAWS     = 10
DRAW_SEEDS  = [13690 + i for i in range(N_DRAWS)]
TRY1_STEPS  = 2000
TRY2_STEPS  = 2000
TRY2_SEED   = 4
MAX_SECONDS = 280
...
WARMUP        = 200   # reduced: rollout overhead may cap max_steps lower
SSM_LR        = 1e-3  # RTRL learning rate

# Rollout config
K_ROLLOUT    = 3     # dream steps per candidate (reduced from 10)
N_CANDIDATES = 8     # candidate actions sampled per real step (reduced from 32)

# Conditions — both run on SAME draws (paired comparison)
...
class SSMLayer:
    ...
    def forward(self, x):
        """Forward pass with RTRL storage."""
        self._x = x
    ...
    def dream_forward(self, x):
        """Forward pass without RTRL storage. Updates self.h in place."""
        delta_pre = np.clip(self.W_delta @ x + self.b_delta, -20.0, 20.0)
        delta = np.log1p(np.exp(delta_pre))
        A_diag = np.exp(-delta * self.A_param)
...
class MambaSSMSubstrate:
    """SSM with forward-rollout action selection."""

    def __init__(self, n_actions, rollout_mode='argmin', max_steps=TRY1_STEPS):
        ...
        # Fixed random action head (used only for entropy/disconnected mode)
        ...
        # Rollout score tracking at checkpoints
    ...
    def _rollout_score(self, proj_obs, a_cand):
        """K-step dream rollout for candidate action.
        Returns cumulative prediction divergence: sum(||pred_{k+1} - pred_k||)."""
        # Save layer hidden states
        saved_h = [layer.h.copy() for layer in self._layers]

        # Step 1: candidate action + real obs
        ...
        # Steps 2..K: dream with SSM's own predictions as obs
        score = 0.0
    ...
    def process(self, obs_arr):
        ...
        if self._step < WARMUP:
            ...
            # SSM-DISCONNECTED: entropy softmax on fixed random W_act
        ...
        if self._step in ENT_CHECKPOINTS and self._step >= WARMUP:
            # Logits from W_act-equivalent: use W_pred @ y as proxy logit (just for entropy reporting)
            # Actually report rollout score variance across 8 samples
            ...
            ent_proxy = round(score_std / (score_mean + 1e-10), 4)  # CV of scores
            if self._in_try2:
    ...
    def get_stats(self):
        ...
        return {
            'try1_score_cv':       self._try1_ent,        # CV of rollout scores (proxy for entropy)
...
def main():
    # ── Tier 1: timing check (includes rollout overhead) ─────────────────
    print("=== TIER 1: timing check (with rollout) ===")
    ...
    sub_t1 = MambaSSMSubstrate(n_actions=na_t1, rollout_mode='argmin')
    # Pre-warmup (no rollout, warmup=200 so use 50 random steps)
    run_episode(env_t1, sub_t1, na_t1, seed=0, max_steps=50)
    # Time TIER1_STEPS steps starting past warmup
    # Force past warmup so rollout activates
    sub_t1._step = WARMUP
    t_tier1 = time.time()
    ...
    print(f"  {TIER1_STEPS} steps (with rollout): {tier1_elapsed:.2f}s  ({ms_per_step:.2f}ms/step)")
    print(f"  Estimated full run at {TRY1_STEPS} steps: {est_total_s:.0f}s ({est_total_s/60:.1f} min)")
    print(f"  (2 conditions: {est_total_s*2:.0f}s total)")

    if est_total_s * 2 > BUDGET_SECONDS:
        max_steps = max(WARMUP + 50, int(BUDGET_SECONDS / 2 / (ms_per_step / 1000 * n_episodes)))
    ...
    # ── Full run ──────────────────────────────────────────────────────────
    print(f"\n=== STEP {STEP}: Paired comparison — ROLLOUT-ARGMAX vs SSM-DISCONNECTED ===")
    print(f"Same 10 draws for BOTH conditions. Eliminates draw variance.")
    print(f"K_ROLLOUT={K_ROLLOUT}, N_CANDIDATES={N_CANDIDATES}, WARMUP={WARMUP}")
    print(f"SSM: D={D}, N_STATE={N_STATE}, N_LAYERS={N_LAYERS}")
    print(f"MLP+TP baseline (1349): {MLP_TP_BASELINE:.2e}  |  1368-ARGMAX: 4.08e-5")
    ...
    if 'ROLLOUT-ARGMAX' in rhae_by_cond and 'SSM-DISCONNECTED' in rhae_by_cond:
        ...
        print(f"\n  Paired: ROLLOUT wins {rollout_wins}/10, DISCONNECTED wins {disc_wins}/10, ties {ties}/10")
