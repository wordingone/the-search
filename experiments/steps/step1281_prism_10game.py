"""
Step 1281 — pe_delta (compression progress) as action selection signal.

Replaces pe_ema with pe_delta = pe_ema_prev - pe_ema_current.
Actions where prediction error is DROPPING fastest get priority.
This measures compression progress (learning speed), not surprise (current loss).

Schmidhuber (1991) distinction: interesting = compressible progress,
not merely surprising. pe_ema = surprise. pe_delta = compression progress.

Selection: score = action_counts - SELECTION_ALPHA * (pe_ema_prev - pe_ema)
pe_delta > 0 when pe is decreasing (model actively learning this transition).

Composition: LPLPEDeltaSubstrate (lpl_pe_delta)
...
ETA_PRED = 0.01
PE_EMA_ALPHA = 0.05
SELECTION_ALPHA = 0.1  # same as 1276/1280
...
RESULTS_DIR = os.path.join('B:/M/the-search/experiments/compositions', 'results_1281')
...
class LPLPEDeltaSubstrate:
    """Step 1281 — LPL with pe_delta (compression progress) as action selection.

    pe_delta[a] = pe_ema_prev[a] - pe_ema[a]
    Positive when pe is DECREASING = encoding is actively learning this transition.
    Selection: score = action_counts - SELECTION_ALPHA * pe_delta
    Actions where pe is dropping fastest get lower score = higher priority.

    Schmidhuber (1991): compression progress > surprise as exploration signal.
    """
    ...
    def __init__(self, n_actions: int, seed: int):
        ...
        self.pe_ema_prev = np.zeros(n_actions, np.float32)
    ...
    def process(self, obs_raw) -> int:
        ...
        # pe_delta = pe_ema_prev - pe_ema: positive when pe is decreasing (learning)
        pe_delta = self.pe_ema_prev - self.pe_ema
        score = self.action_counts - SELECTION_ALPHA * pe_delta
    ...
    def update_flow(self, next_obs_raw):
        ...
        a = self._last_action
        # Save previous EMA before updating, then update
        self.pe_ema_prev[a] = self.pe_ema[a]
...
def compute_i3_cv(action_counts_200, n_actions):
    """I3: coefficient of variation of visit counts. Lower = more uniform."""
...
def run_single(game_name, condition, draw, seed, n_actions, kb_delta, solver_level_steps):
    ...
    if condition == 'lpl_pe_delta':
        substrate = LPLPEDeltaSubstrate(n_actions=n_actions, seed=seed)
    ...
    while steps < MAX_STEPS:
        ...
        if steps in PE_SNAPSHOT_STEPS and condition == 'lpl_pe_delta':
            pe_d = substrate.pe_ema_prev - substrate.pe_ema  # pe_delta
            pe_e = substrate.pe_ema
            n7 = min(7, n_actions)
            pe_snapshots[steps] = {
                'pe_ema_mean': round(float(pe_e.mean()), 6),
                'pe_ema_max': round(float(pe_e.max()), 6),
                'pe_delta_mean': round(float(pe_d.mean()), 6),
                'pe_delta_max': round(float(pe_d.max()), 6),
                'pe_delta_top7': [round(float(pe_d[i]), 6) for i in range(n7)],
            }
    ...
    if condition == 'lpl_pe_delta' and r3_snapshot and r3_obs_sample:
    ...
    cond_labels = {'lpl_pe_delta': 'LPED', 'control_c': 'CTL'}
...
def main():
    ...
    print("=" * 70)
    print("STEP 1281 — LPL PE_DELTA (compression progress) vs Control C")
    print("pe_delta = pe_ema_prev - pe_ema. Actions where pe drops fastest get priority.")
    print("I3 = CV metric (not rho).")
    print("=" * 70)
    print(f"Games: {GAMES}")
    ...
    for game_name in GAMES:
        ...
        for condition in ['lpl_pe_delta', 'control_c']:
            print(f"\n  Condition: {condition}")
            for draw in range(1, N_DRAWS + 1):
                seed = draw * 100 + (1 if condition == 'lpl_pe_delta' else 0)
                result = run_single(
                    game_name=game_name,
                    condition=condition,
                    draw=draw,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 1281 COMPLETE -- {len(all_results)} runs in {total_elapsed:.1f}s")
    print(f"{'='*70}\n")

    # --- Summary ---
    print("I3 CV Summary (primary metric):")
    for game in GAMES:
        for cond in ['lpl_pe_delta', 'control_c']:
            runs = [r for r in all_results if r['game'] == game and r['condition'] == cond]
            label = 'LPED' if cond == 'lpl_pe_delta' else 'CTL'
    ...
    with open(os.path.join(RESULTS_DIR, 'step1281_results.json'), 'w') as f:
