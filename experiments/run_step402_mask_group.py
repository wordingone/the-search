#!/usr/bin/env python3
"""
Step 402 -- Mask + group adjacent active dims. Self-derived spatial encoding.

Change-rate mask finds ~100 active dims (sprite region).
Group every K=4 adjacent active dims and average -> ~25 super-dims.
Process() on 25-dim encoded vectors. Cosine in 25D.

LS20. 64x64. 50K steps.
Script: scripts/run_step402_mask_group.py
"""
...
def main():
    t0=time.time()
    print(f"Step 402 -- Mask + group adjacent dims. 64x64. 50K. LS20.", flush=True)
    print(f"Device: {DEVICE}  warmup={WARMUP}  K_group={K_GROUP}", flush=True); print(flush=True)
    ...
    print("STEP 402 SUMMARY", flush=True); print("="*60, flush=True)
