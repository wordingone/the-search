"""
Step 606b -- LS20 L4 push (fix: G preserved across episodes, reset only on level transition).

LS20 already at L3 (572u). Push to L4.

Changes from 572u:
- Graph reset on level transition (G = {} at each on_l1/on_l2/on_l3)
- Level-aware state tracking extended to L4 (l4_freq, l4_mode)
- on_l3() now sets game_level=3 for L4 navigation
- L4 nav: argmin + mode-map cluster targeting (same structure as L0, fresh graph)
- Accumulate per-level statistics (freq maps not reset, only graph is reset)

R3 test: does graph reset on transition allow fresh navigation discovery at L4?
Kill: L4=0/5 after fresh graph — mgu waypoints mandatory for L4 too.
Signal: L4 >= 1/5.

Protocol: 5 seeds x 500K steps (60s/seed cap), arcagi3 LS20.
"""
import numpy as np
import time
import sys
...
class SubDual:
    def __init__(self, seed=0):
        ...
        # Per-level freq/mode maps — NOT reset on transition, only accumulated
    ...
    def observe(self, frame):
        ...
        # Handle both old arcagi3 obs format and new format
        if arr.ndim == 1:
    ...
    def on_l1(self):
        ...
        self.visited = []
        self.G = {}  # graph reset on level transition
        self._pn = None
    ...
    def on_l2(self):
        ...
        self._puq_death_seen = False
        self._puq_xhp_step = False
        if not self._puq_wall_frozen and self.l2_count >= N_MAP_PUQ and self.l3_frames >= WARMUP:
    ...
    def _puq_dr_action(self):
        ...
        for _ in range(len(PUQ_WAYPOINTS) + 2):
            if self._puq_wp_idx >= len(PUQ_WAYPOINTS):
                return None
            start = self._puq_dr_pos
            goal = PUQ_WAYPOINTS[self._puq_wp_idx]
    ...
    def act(self):
        ...
        # L4 (game_level=3): argmin + mode-map cluster targeting (fresh graph after reset)
...
def fire_level_handlers(sub, cl, prev_cl, step, seed, go, l1_step_ref, l2_first_ref,
                         l3_steps_ref, l4_steps_ref):
    if cl == 1 and prev_cl < 1:
        ...
        print(f"  s{seed} L3@{step}! l3c={sub.l3_count} deaths={sub._puq_death_count} go={go}",
              flush=True)
    elif cl == 4 and prev_cl < 4:
        sub.on_l4()
        l4_steps_ref.append(step)
        print(f"  s{seed} L4@{step}!! l4c={sub.l4_count} go={go}", flush=True)
...
def main():
    ...
    print("Step 606b: LS20 L4 push (graph reset on level transition only)", flush=True)
    print(f"  Fix: G preserved across episodes, reset on level change only", flush=True)
    ...
    for seed in range(n_seeds):
        ...
        for step in range(1, 500_001):
            ...
            if prev_cl == -1:
                fire_level_handlers(sub, cl, -1, step, seed, go, l1_step, l2_first,
                                    l3_steps, l4_steps)
                prev_cl = cl
            elif cl > prev_cl:
                fire_level_handlers(sub, cl, prev_cl, step, seed, go, l1_step, l2_first,
                                    l3_steps, l4_steps)
                prev_cl = cl
            else:
                prev_cl = cl
            ...
            if time.time() - t_seed > per_seed_cap:
                print(f"  cap @{step} go={go} l2c={sub.l2_count} l3c={sub.l3_count} "
                      f"l4c={sub.l4_count} wall={sub._puq_wall_frozen} "
                      f"deaths={sub._puq_death_count} wp_idx={sub._puq_wp_idx} "
        ...
        R.append({'seed': seed, 'l2c': sub.l2_count, 'l3c': sub.l3_count,
                  ...
                  'puq_wall': sub._puq_wall_frozen, 'bfs_fails': sub.puq_bfs_fails,
                  'lhs_ap': sub._puq_lhs_ap, 'deaths': sub._puq_death_count,
                  'wp_idx': sub._puq_wp_idx})

    print(f"\n{'='*60}", flush=True)
    print(f"Step 606b: LS20 L4 push (graph reset on level transition only)", flush=True)
    ...
    for r in R:
        print(f"  s{r['seed']}: L2={r['l2c']} L3={r['l3c']} L4={r['l4c']} "
              f"wall={r['puq_wall']} deaths={r['deaths']} bfs_f={r['bfs_fails']} "
              f"lhs={r['lhs_ap']} wp={r['wp_idx']} "
              f"L3@{r['l3_steps'][:3] if r['l3_steps'] else []} "
              f"L4@{r['l4_steps'][:3] if r['l4_steps'] else []}", flush=True)
    print(f"  total_t={time.time()-t_start:.1f}s", flush=True)
    if l4_wins >= 1:
        print("  SIGNAL: L4 reached. Fresh graph navigation works.", flush=True)
    elif l3_wins >= 3:
        print("  L3 confirmed. L4 needs waypoint analysis.", flush=True)
    else:
        print("  L3 not reached. Check mgu pipeline compat.", flush=True)
