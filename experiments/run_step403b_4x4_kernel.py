#!/usr/bin/env python3
"""
Step 403b -- Force 4x4 kernel on self-discovered signal region.

1. Track change rates -> find active pixels (same as 403)
2. Bounding box -> signal region
3. Force 4x4 avgpool on signal patch -> 32D encoding
4. Standard process() — dynamics should work (thresh~0.917)

LS20. 64x64. 50K steps.
Script: scripts/run_step403b_4x4_kernel.py
"""
...
def main():
    t0=time.time()
    print("Step 403b -- Force 4x4 kernel on self-discovered signal region. LS20.", flush=True)
    ...
    # Stage 2: Force 4x4 kernel
    kernel=4
    d=len(encode_patch(np.zeros((64,64),dtype=np.float32), r0,r1,c0,c1, kernel))
    print(f"\n=== Stage 2: Force {kernel}x{kernel} kernel -> {d}D encoding ===", flush=True)
    ...
    print("STEP 403b SUMMARY", flush=True); print("="*60, flush=True)
    print(f"steps={ts}  go={go}  levels={lvls}", flush=True)
    print(f"Signal region: rows [{r0},{r1}), cols [{c0},{c1})", flush=True)
    print(f"Kernel: {kernel}x{kernel}  dims={d}", flush=True)
