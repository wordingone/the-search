"""
sub1056_defense_v10.py — Defense v10: strategy-level internal restarts (ℓ₁).

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1056 --substrate experiments/sub1056_defense_v10.py

FAMILY: parametric-goal
R3 HYPOTHESIS: ℓ₁ with STRATEGY-level restarts. Same epoch structure as
  prosecution v9 (kb/click/seq/exploit), but SPSA-tuned goal drives the
  transfer between strategies. change_map learned in epoch 1 (keyboard)
  informs SPSA goal in epoch 3 (sequences). SPSA tunes what the goal IS,
  not just how to attend.

  Defense counter: change_map IS a sufficient transfer mechanism. Keyboard
  effects in epoch 1 tell the goal function which regions matter. SPSA then
  tunes the goal toward those regions. Attention is redundant when the goal
  function already incorporates spatial signal via change_map weighting.

Epoch 1 (0-2500): KEYBOARD STRATEGY — build kb_influence + change_map.
Epoch 2 (2500-5000): CLICK STRATEGY — build click regions + SPSA updates.
Epoch 3 (5000-7500): SEQUENCE STRATEGY — evo sequences using discovered actions.
Epoch 4 (7500-10000): EXPLOIT — best sequences + SPSA-guided fallback.
...
class DefenseV10Substrate:
    """
    ℓ₁ R3 v10: strategy-level internal restarts.
    Same epoch structure as prosecution v9.
    SPSA-tuned goal transfers across strategies via change_map weighting.
    """
...
CONFIG = {
    ...
    "spsa_lr": SPSA_LR,
    "v10_features": "strategy-level restarts — kb/click/seq epochs + SPSA goal transfer (l_1)",
}

SUBSTRATE_CLASS = DefenseV10Substrate
