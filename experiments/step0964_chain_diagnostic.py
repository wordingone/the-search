"""Step 964: 916 chain diagnostic — full chain CIFAR→LS20→FT09→VC33→CIFAR @ 10K.

FAMILY: 916-chain (diagnostic run, not a new mechanism)

R3 HYPOTHESIS: N/A — diagnostic. Measures WHERE 916 breaks on the full chain.
Uses 916's fixed-random reservoir + 800b on ext_enc. Chain-compatible: W_pred
is (320,320) not (320,320+n_actions), so substrate persists across games.
delta_per_action resets on set_game(). h, W_h, W_x, alpha, W_pred persist.

Kill: N/A (diagnostic). Report per-game.
"""
import sys, time
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 964 — 916 CHAIN DIAGNOSTIC (CIFAR→LS20→FT09→VC33→CIFAR)")
    print("=" * 70)
    t0 = time.time()
