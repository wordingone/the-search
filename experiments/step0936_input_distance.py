"""
step0936_input_distance.py -- Input-distance confidence (position-dependent epistemic value).

R3 hypothesis: Input-distance confidence is position-dependent through ext_enc (which
includes h). Unlike pred_error_ema (which equalizes across all actions), the distance
to the mean training input DOESN'T equalize — rare positions stay far from the mean.
G[a] = predicted_change[a] / (||input_a - mean_input|| + 0.01) where mean_input is
the global running mean of all W_pred training inputs. Deep/rare positions have high
distance → low confidence → high G → substrate naturally explores novel trajectory.

Architecture: Same as 934b (916 recurrent h + clamped alpha). ONLY change: confidence.
934b (killed): confidence_a = 1 / (pred_error_ema[a] + 0.01)  — equalizes in games
936:           confidence_a = 1 / (||[alpha*ext_enc, one_hot(a)] - mean_input|| + 0.01)
...
class Chain936:
    """916 recurrent h + clamped alpha. Epistemic G[a] with input-distance confidence.

    confidence_a = 1 / (||[alpha*ext_enc, one_hot(a)] - mean_input|| + 0.01)
    mean_input = running mean of all W_pred training inputs (global statistic).

    Alpha + running_mean persist across games. W_pred + h + mean_input reset per game.
    """
    ...
    def __init__(self, seed=0):
        ...
        self._mean_input = None  # running mean of W_pred inputs (320+n)
        self._n_inputs = 0
        self._prev_ext = None
    ...
    def set_game(self, n_actions):
        ...
        self.h = np.zeros(H_DIM, dtype=np.float32)
        self._mean_input = np.zeros(EXT_DIM + n_actions, dtype=np.float32)
        self._n_inputs = 0
        self._prev_ext = None
        self._prev_action = None
        self._phase_step = 0
    ...
    def reset_seed(self, seed):
        ...
        self.h = np.zeros(H_DIM, dtype=np.float32)
        self._mean_input = None; self._n_inputs = 0
    ...
    def _compute_epistemic(self, ext_enc):
        """G[a] = predicted_change[a] / (||input_a - mean_input|| + 0.01).

        input_a = [alpha*ext_enc, one_hot(a)].
        confidence_a = 1 / distance_a (rare positions stay far from mean).
        """
        weighted = ext_enc * self.alpha   # 320D
        one_hots = np.eye(self._n, dtype=np.float32)
        tiled = np.tile(weighted, (self._n, 1))
        inputs = np.concatenate([tiled, one_hots], axis=1)  # n × (320+n)

        # Predicted changes
        predicted_nexts = inputs @ self.W_pred.T  # n × 320
        changes = np.linalg.norm(predicted_nexts - weighted[np.newaxis, :], axis=1)

        # Input-distance confidence (position-dependent via h in weighted)
        if self._n_inputs > 0:
            dists = np.linalg.norm(inputs - self._mean_input[np.newaxis, :], axis=1)  # n
        else:
            dists = np.ones(self._n, dtype=np.float32)

        # G[a] = predicted_change / (dist + 0.01)  — high when rare AND big predicted change
        G = changes / (dists + 0.01)
    ...
    def process(self, obs):
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            # Update mean_input (running mean of all training inputs)
            self._n_inputs += 1
            a = 1.0 / self._n_inputs
            self._mean_input = (1 - a) * self._mean_input + a * inp

        G, dists = self._compute_epistemic(ext_enc)

        if self._phase_step in DIAG_STEPS:
            self._diag_log[self._phase_step] = {
                "G_rel": float((G.max() - G.min()) / (G.mean() + 1e-8)),
                "dist_min": float(dists.min()), "dist_max": float(dists.max()),
        ...
        if self._rng.random() < EPSILON:
            ...
            action = softmax_action(G, SOFTMAX_TEMP, self._rng)
    ...
    def diag_str(self):
        ...
        for step in sorted(self._diag_log):
            ...
            parts.append(f"@{step//1000}K: G_rel={d['G_rel']:.2f}"
                         f" dist=[{d['dist_min']:.1f},{d['dist_max']:.1f}]"
                         f" spread={d['dist_spread']:.1f}")
...
print("=" * 70)
print("STEP 936 — INPUT-DISTANCE CONFIDENCE (position-dependent epistemic)")
print("=" * 70)
print("R3: G[a] = predicted_change / (||input_a - mean_input|| + 0.01).")
print("Rare positions stay far from mean_input → low conf → high G → explore.")
t0 = time.time()
...
print(f"\n{'=' * 70}")
print(f"STEP 936 RESULTS (input-dist conf, PRISM-light, 10K/phase, 5 seeds):")
...
print(f"  934b (pred_err conf):  LS20=9.2   KILLED")
print(f"  936 (input-dist conf): LS20={np.mean(ls20):.1f}  FT09={np.mean(ft09):.1f}  VC33={np.mean(vc33):.1f}")
