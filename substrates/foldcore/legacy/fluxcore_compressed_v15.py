#!/usr/bin/env python3
"""
FluxCore Compressed v15 — Dual-Representation, LVQ Competitive Codebook
...
class CompressedKernel:
    ...
    def step(self, r=None):
        ...
        if r is not None:
            ...
            # LVQ competitive codebook update (CHANGED from v14: winner-only -> LVQ)
            # Winner: pulled toward input
            v_w = self.codebook[winner]
            self.codebook[winner] = _normalize(
                _vec_add(v_w, _vec_scale(_vec_add(r, _vec_scale(v_w, -1.0)), self.lr_codebook))
            )
            # k_neg nearest losers (by codebook cosine): pushed away
