...
| PB26 | ℓ_π (encoding modification) vs ℓ₁ (parameter modification) on responsive games. Head-to-head on same game draws: 2 prosecution wins, 2 defense wins, 1 mixed (Steps 1040-1043, 1051). Aggregate L1 ratio ~1.5:1 prosecution, confounded by different random game draws across experiments. Both sides achieved L1>0 on 8 distinct games each. Consistent with ANIL (Raghu et al., ICLR 2020) direction but not decisive. Defense v11 achieved best single-game result (100% on one game). | PROVISIONAL (Steps 1042-1064, 20+ experiments. Game-draw confound prevents controlled comparison.) |
