...
| PB32 | **0% games produce ZERO pixel change from ANY single action (Steps 1141, 1144).** Systematic pixel scan (4096 positions) + raw 4096D encoding: no detectable change on 0% games from keyboard, block-center clicks, OR individual pixel clicks. NOT encoding (tested avgpool4/8, raw pixel, Sobel, temporal diff). NOT action targeting (every pixel tested). NOT goal function (7 variants tested). 44 debate experiments confirm. Remaining hypothesis: multi-step action SEQUENCES. | **CONFIRMED** (Steps 1104-1144, 44 experiments. Definitive: single-action probing cannot reach ~60% of random games.) |

### Summary

Prediction transfer region non-empty (PB5). Navigation transfer region empty. Gap is structural (Prop 21). **0% wall = ZERO PIXEL CHANGE on ~60% of game draws (PB32, DEFINITIVE).** v30 avgpool4 reactive = ONLY draw-robust result (ARC=0.33 on keyboard games). PB31 CONFIRMED (encoding resolution). PB26 CONFIRMED (ℓ_π ≈ ℓ₁ parity). PB30 simplicity (n=6+). 44 debate experiments systematically ruled out encoding, goal function, action space, click targeting, forward models, nonlinear features, novelty seeking. **Single-action probing is the structural bottleneck. Next: multi-step sequences.** ALL BANS LIFTED. ARC Prize scoring. Distinct architectures mandated.
