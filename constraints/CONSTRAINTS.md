...
2. **L1 is perception-limited, not action-limited.** Argmin ≈ random for L1 (Step 653: 3/20 each, perfectly symmetric). 35+ experiments across 8 intervention types (477-482, 581d, 620-639, 652-686) — every action-selection modification hurt L1. Only perception changes improve L1: centering = 75% of gain (Step 712), hash resolution = 25% (674). "Argmin is locally optimal" is vacuously true — argmin = random, and every structured alternative is worse than random. The lever is encoding/perception, not action selection. This is consistent with state representation learning literature (Lesort et al. 2018): representation quality dominates policy quality in navigation tasks. **Implication for R3:** R3 for action selection is a dead end (confirmed by Steps 713-719, 13 experiments). R3 for encoding/perception is the productive target — the 5 encoding U elements in 674 (avgpool16, channel_0_only, mean_centering, H_nav_planes, H_fine_planes) are where self-modification has leverage.
