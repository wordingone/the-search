...
CURRENT STEP: **Steps 1104-1124: 21 experiments, 1 draw-robust result (v30).** v40 raw pixels NOT draw-robust (GAME_2 solve was luck). v31 direction learning inconsistent (2/4 at 3/5). Only v30 avgpool4 reactive = draw-robust (ARC=0.33). Raw pixels too sensitive (noise = false progress). **The 0% wall is GAME DIVERSITY** — random draws include fundamentally different game types that no single mechanism covers. Win condition (ALL 3 games) requires a meta-mechanism that adapts to game type. 1124+ experiments.
