...
CURRENT STEP: **Step 1079 DIAGNOSTIC: 0% wall = TWO distinct failure modes.** Mode 1 (near-inert): pixel_var=0.001, 200-500x below responsive, thresholds 100-1000x above signal. Detection problem — signal is sub-threshold, not absent. Mode 2 (responsive-unsolved): pixel_var=0.37, actions differentiate (var 0.19-0.30), 62% non-zero frames. Signal IS there — substrate sees it but can't convert to L1. **Mechanism problem, not detection.** First time these modes separated. 1079+ experiments. Next: Mode 1 = lower thresholds. Mode 2 = why can't detection→solution? The D1 prescription gap.
  ...
  **Step 1079 DIAGNOSTIC (seed 44444):** ALL 3 games 0% L1 but TWO distinct failure modes. **Mode 1 (near-inert):** pixel_var=0.001 (200-500x below responsive), zero action differentiation, 78-86% zero-diff frames, max_diff=0.0005 (sub-pixel noise). Thresholds 100-1000x above signal → DETECTION PROBLEM. **Mode 2 (responsive-unsolved):** pixel_var=0.37 (comparable to solved), actions differentiate (0.19-0.30 var), 38% zero-diff frames, max_diff=0.079 (real signal). Signal present → MECHANISM PROBLEM. Substrate sees responsive actions but can't convert to L1 solutions. **First separation of two distinct 0% wall failure modes.**
  ...
  **Step 1021 DONE (Ablation):** LS20 all 311 moves critical. 0/10 shuffles pass ANY level. BFS-minimum confirmed (no shorter paths). L1=13, L2=45, L3=41, L4=43, L5=44 (moving triggers, lcm=8), L6=72 (moving triggers, lcm=8), L7=53. **Ablation summary: all prescriptions are minimum — no approximation, no redundancy, no purgable components.**
  **Step 1020a DONE (Ablation):** VC33 CC zone centroids. L1=PASS (1 click aligns with centroid), L2-L7=FAIL (centroids 3-21px off). Zone-8=0/7, CC=1/7. **CC discovery insufficient — click positions at screen edges, centroid misalignment.** Universal: zone/CC discovery fails ALL click games.
  **Step 1020 DONE (Ablation):** VC33 zones KILL (same as FT09). Order: L1=order-free (20/20), L2=partial (13/20), L3=partial (3/20), L4-L7=strictly order-dependent (0/20). VC33 prescription = ordered sequence of exact (x,y). 176 clicks: L1=3, L2=7, L3=23, L4=23, L5=49, L6=22, L7=49. Canal lock mechanism requires precise interleaving. **FT09=unordered set, VC33=ordered sequence. Fundamentally different prescription types.**
  **Step 1019b PASS (Ablation):** FT09 click ORDER irrelevant. All 20 shuffles per level PASS. Min prefix = full prescription (no redundancy). FT09 minimum prescription = {unordered set of exact (x,y) per level}. 75 clicks, all necessary, 2-pixel precision. ORDER is purgable. POSITIONS and COMPLETENESS are not.
  **Step 1019a KILL (Ablation):** FT09 zone discovery replacing exact coords. Condition A (exact): 6/6 PASS (4,7,14,16,21,13 clicks). Condition B (zone-8 snap): 1/6 (L1 only — walls at odd grid, snap at even, off-by-1). Condition C (zone-6 k-means): 0/6 (clusters collapse position info). **VERDICT: exact pixel coords ESSENTIAL for FT09. 2-pixel precision required.** New finding: L5/L6 are Lights-Out puzzles (clicking cycles neighbors). Fixed via GF(2) Gaussian elimination. Updated prescription: L1=4, L2=7, L3=14, L4=16 (3-color cycle), L5=21 (Lights-Out +spread), L6=13 (Lights-Out up-spread). **FT09 is HARDER than sequential clicking — L5/L6 require linear algebra over finite fields.**
  **Step 1018e CONFIRMED (Diagnostic):** LS20 BFS solver. All 7 levels verified in-game across 3 seeds. [13,45,41,43,44,72,53]=311 steps. Mechanics: grid nav, shape matching, death-respawn cycling, ramps, moving triggers. 5 iterations (1018→1018e) to model full game physics. L1=100% on chain (1018c). BFS is prescription catalog (not scoreable — source analysis).
  ...
  **Step 1011/1011b FAIL (Diagnostic):** Unconstrained PRISM. Implementation used global argmin (round-robin) instead of per-state argmin. Needs full 674 graph implementation. Deferred to 1011c.
  **Step 1012 CONFIRMED (Diagnostic):** FT09 per-game ceiling. 6/6 levels, 5/5 seeds, 75 clicks, deterministic. Game version 0d8bbf25 confirmed solvable. **Constrained gap: 0/10 L1 → 6/6 all levels.**
  **Step 1013 CONFIRMED (Diagnostic):** VC33 per-game ceiling. 7/7 levels, 5/5 seeds, 176 clicks. Game version 9851e02b confirmed solvable. **Constrained gap: 0/10 L1 → 7/7 all levels.**
  **Diagnostic sub-mode A complete:** FT09 6/6 + VC33 7/7 = 13 levels confirmed solvable. Both current game versions. Constraint cost = 13 levels.
  ...
  **Step 1015 FAIL (Diagnostic):** LS20 ceiling re-verification. 572u SubDual on 9607627b. max_cl=0 all seeds — never reached L1 in 156K steps. ROOT CAUSE: 572u hardcodes coordinates from older LS20 version. Current 9607627b has different mechanics. **LS20 multi-level ceiling UNKNOWN on current version.** Generic 994 substrate still gets L1=100% (different approach, no hardcoded coords).
  **Step 1016 PASS (Direction 1):** CC zone discovery + sequence novelty. Chain 3/5, FT09/VC33=0%. CC zones (68→~10 actions) did NOT fix sequencing. **Encoding was NOT the bottleneck — mechanism is.** Sequence novelty KILLED (2 experiments, same result with different encodings).
  **Direction 1 status (6 extraction experiments, 0 signal):** Attention (1007/1009/1010) → dead. Sequence novelty (1014/1016) → dead. CC zones (1008/1016) → valid component but insufficient alone. All global action selection methods fail for state-dependent click games. Untested: change-rate detection, transition detection, sparse gating.
  **Step 1017 FAIL (Diagnostic):** Unconstrained unified PRISM entity. Full 674 graph + CC zones. ALL bans lifted. LS20=40% (674 needs >10K warmup), FT09=0%, VC33=0%. **CRITICAL: even with per-state argmin (graph ban lifted), generic exploration can't find multi-step click sequences.** 674 graph provides state coverage but NOT temporal credit for sequential actions. The 13-level gap (PB21) is caused by PRESCRIPTION, not by the graph ban.
  **Diagnostic sub-mode B complete.** Sub-mode A: FT09 6/6 ✓, VC33 7/7 ✓, LS20 unknown (stale coords). Sub-mode B: LS20=40%, FT09=0%, VC33=0%. **Conclusion: games ARE solvable (1012/1013 prescribed) but NO generic exploration mechanism solves them — neither constrained (1007-1016) nor unconstrained (1017). The gap is PRESCRIPTION, not constraints.**
  **LS20 source dissection (9607627b):** 7 levels. Puzzle navigation: grid movement + shape matching (color/rotation cycling). StepCounter=42, 3 lives. Level 7 has fog. Step budget too tight for random walk (21 moves × 3 lives = 63 total). 994 substrate gets L1 via generic argmin but can't do shape matching for L2+.
  **Step 1018 QUEUED (Diagnostic):** LS20 7-level BFS solver. Per-game prescribed from source analysis. Extracts wall/goal/collectible positions per level, computes BFS paths with shape matching.
...
Step 934 - Model-based action selection via W_pred (raw novelty only). **KILLED: LS20=0/5, all games=0.** Root cause: W pred_acc=4K-22K in games. All actions get novelty ~20K with spread <2%. Softmax at T=0.1 → uniform random. Non-convergent W destroys action discrimination when used for PREDICTED change without confidence normalization.
Step 934b - Epistemic action selection (Prop 24 full formula): G[a] = predicted_change / confidence_a. **KILLED: LS20=9.2 (4/5 zeros).** Confidence equalizes fast with few actions (LS20: 4 actions, all pred_error_ema converge by 1K steps). G_rel=0.17 at 1K → no discrimination. FT09 (68 actions): G_rel starts 6x but collapses to 0.8x by 10K. Pattern: ANY prediction-based selector fails because W_pred errors (pred_acc=-2383) overwhelm signal. 800b works because it measures ACTUAL changes, not PREDICTED changes. Prop 24 (active inference) reduces to noisy ICM in practice (Hart adversary confirmed).
Step 935 - h-delta tracking (reservoir surprise). **KILLED: LS20=33.8 (2/5 zeros).** Root cause: tanh compresses h-changes. delta_h=[0.5, 0.97] vs enc-delta=[1-5]. Less variance → less discrimination. Echo state normalizes too aggressively.
Step 936 - Input-distance confidence. **KILLED: LS20=0/5.** At same position, all actions share identical ext_enc — only 4D one_hot differs. Distance to mean nearly identical for all actions → G flat. Pattern confirmed: ANY prediction/model-based selector fails. 800b works because it measures ACTUAL changes.
Step 937 - Adaptive EMA decay (chemotaxis-inspired, Prop 25). **KILLED: LS20=72 (1/5 zeros).** Lambda formula works directionally (LS20 λ=0.85-0.93, FT09 λ=0.83, VC33 λ=0.99). Kill cause: used raw enc-delta instead of alpha-weighted ext_enc delta → signal degradation. VC33 finding: observations barely change (var≈0.000) → that's WHY it's hard.
Step 938 - NRP (novelty-reactive policy). **REJECTED PRE-RUN — gate 5.** Per-observation policy table (best_action[hash(obs)]) is per-observation conditioning. Same violation as Step 931 (PB20).
Step 938b - Alpha-anomaly reactive selector. **KILLED: LS20=0, FT09=0, VC33=0.** Anomaly = alpha * (enc - running_mean)^2, top-K dims → action. Root cause: alpha locks dominant dims at every position → constant action. Reactive-global family = dead.
Step 938c - Trajectory-conditioned action perturbation (h-projection). **KILLED: LS20=65.5 best (beta=0.5), FT09=0, VC33=0.** h_spr non-zero (0.37-0.42, h provides variance). Kill cause: removing alpha from delta killed discrimination (delta_spr→0.018). h works but delta doesn't without alpha.
...
Step 645: Self-derived penalty (active, surprise_count). L1=3/5, penalty_active=90.5%. MARGINAL/WORSE.
  ...
  **642-645 edge-state enrichment series:**
  ...
  Finding 3: The fix for 645 is normalization — bounded signals don't swamp argmin (Hart review:
