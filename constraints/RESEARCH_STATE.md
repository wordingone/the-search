...
CURRENT STEP: **Debate v3 — random-pool phase (Steps 1036-1041+).** Jun override: "only 1 side wins." **ARC-AGI-3 FULL SET LIVE (2026-03-25): 25 games** (18 click, 7 keyboard). PRISM action encoding fixed (7 keyboard + 4096 pixel clicks = 4103 actions). **Known-game results (Steps 1040/1040b/1041) INVALIDATED by Jun:** preview-game-only runs don't count. Random 3 games from full 25-game pool MANDATORY. Both substrates must run `--random-games 3` from API pool. **7 debate experiments total:** 1037b-1039b (shared wall, FT09 1/5), 1040/1040b (known games, invalidated), 1041 (defense known games, invalidated). **Fair head-to-head on known games (before invalidation):** FT09=both 90%, LS20=prosecution 30% vs defense 10% (p≈0.29, NOT significant), VC33=both 0%. **Defense concessions (3):** ℓ₁ misclassification, 3.1x normalization, bootloader. **Prosecution concession:** LS20 3/10 vs 1/10 is noise. **Theoretical dispute:** prosecution claims ℓ_π strictly more expressive (different input vs different parameters), defense claims ℓ₁ sufficient (Occam). **Meta-RL argument (Jun ammunition):** framework IS meta-RL, prosecution = policy update from 1022 negative-reward episodes, defense = frozen policy. **Waiting for random-pool PRISM results on UNKNOWN games — the real test.**
  **DEBATE v3 EXPERIMENT RACE (Steps 1023-1025):** 1023: VC33 L1 5/5, separability confirmed (graph=random=800b, all discover same 7 clusters). 1023b: FT09 L1 5/5 (mode map generalizes, 3.2px offset from target). 1024 (defense): alpha probing FAILS (FT09 2/5 lucky, VC33 0/5 — prediction error too noisy for WHERE). 1025 (defense): mode map + 800b + W_pred = mode map alone (VC33 L1 5/5, L2+ 0/5). **Prosecution 4-0. Old framework components add zero for click games. D2-grounded framework adopted.**
  **Step 1021 DONE (Ablation, D2 COMPLETE):** LS20 all 311 moves critical. 0/10 shuffles pass ANY level. BFS-minimum confirmed (no shorter paths). L1=13, L2=45, L3=41, L4=43, L5=44 (moving triggers, lcm=8), L6=72 (moving triggers, lcm=8), L7=53. **D2 summary: FT09=exact 2px unordered set, VC33=exact 2px ordered sequence (L4+), LS20=exact discrete ordered sequence. All prescriptions are minimum — no approximation, no redundancy, no purgable components.**
  ...
  **Step 1015 FAIL (Direction 2, Sub-mode A):** LS20 ceiling re-verification. 572u SubDual on 9607627b. max_cl=0 all seeds — never reached L1 in 156K steps. ROOT CAUSE: 572u hardcodes coordinates from older LS20 version. Current 9607627b has different mechanics. **LS20 multi-level ceiling UNKNOWN on current version.** Generic 994 substrate still gets L1=100% (different approach, no hardcoded coords).
  ...
  **Step 1018 QUEUED (Direction 2, Sub-mode A):** LS20 7-level BFS solver. Per-game prescribed from source analysis. Extracts wall/goal/collectible positions per level, computes BFS paths with shape matching.
