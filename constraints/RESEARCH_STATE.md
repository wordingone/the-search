...
CURRENT STEP: **Steps 1104-1135: 33 experiments DEFINITIVE. v30 ONLY draw-robust result.** ALL click successes draw-dependent (v40, v43, v46 — each solved 1 click game on lucky draw, failed reproduction). 0% wall = STRUCTURAL. ~60% of draws contain games no probing strategy reaches. Defense ceiling: v30 ARC=0.33 on keyboard games. Click games: unsolved robustly by either side. Defense ℓ₁ EXHAUSTED (32 experiments, all zero-param variants tested). Prosecution v34 (random feature forward model) also fails — keyboard forward models irrelevant for click-only games.
