...
CURRENT STEP: **Steps 1104-1140: 40 experiments. 0% wall = PERCEPTION bottleneck.** v30 ONLY draw-robust result (ARC=0.33). Defense ℓ₁ EXHAUSTED (37 experiments). Prosecution ℓ_π also fails on 0% games (5 experiments). NOVELTY AXIS TESTED (Steps 1139-1140): both LSH raw novelty (defense) and alpha-weighted novelty (prosecution) = same 2/5 wall. The bottleneck is BELOW all mechanisms tested — the substrate cannot detect the effect of its actions on ~60% of games. Neither encoding, goal function, action space, click targeting, forward models, nonlinear features, NOR novelty-seeking breaks the wall. 1140+ experiments.
