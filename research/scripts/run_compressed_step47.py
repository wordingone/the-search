#!/usr/bin/env python3
"""
Step 47: Crowding-aware autonomy (last angle on coverage ceiling).
One variable from Step 38: add crowding_thresh=0.7.

crowding_autonomy_i = count(j: cos(M_i, M_j) > 0.7) / n
effective_autonomy_i = max(eigenform_autonomy, crowding_autonomy)

Crowded cells resist perception pull. Coupling unchanged → generation preserved.
If this doesn't improve coverage, document tradeoff as fundamental.

Baselines:
  Step 38: 19/33, energy=0.116
  Step 45: 21/33, energy=0.024 (coupling suppress)
  Step 46: 7/33,  energy=0.124 (sparse perception)
"""
...
from fluxcore_compressed_v9 import CompressedKernel
...
def main():
    ...
    kernel = CompressedKernel(
        ...
        crowding_thresh=crowding_thresh
    )

    print(f"Config: n=8, k={k}, d={d}, tau=0.3, k_couple=5, crowding_thresh={crowding_thresh}")
    print(f"(Step 38 identical except crowding_thresh={crowding_thresh})\n")
    ...
    # Crowding stats at final state
    n = nc_final
    cells = kernel.cells
    crowding_counts = []
    for i in range(n):
        cnt = sum(1 for j in range(n) if j != i and mcosine(cells[i], cells[j]) > crowding_thresh)
        crowding_counts.append(cnt)
    mean_crowding = sum(crowding_counts) / n if n > 0 else 0
    high_crowd = sum(1 for c in crowding_counts if c > 3)
    ...
    print(f"  Mean crowding:    {mean_crowding:.2f} neighbors/cell (thresh={crowding_thresh})")
    print(f"  High-crowd cells: {high_crowd}/{nc_final} (>3 neighbors)")
    print(f"\n  Step 38 baseline:  19/33, energy=0.116")
    print(f"  Step 45 (suppress):21/33, energy=0.024")
    print(f"  Step 46 (sparse p):7/33,  energy=0.124")
    ...
    print(f"\n{'='*70}")
    print(f"  STEP 47 FINAL")
    print(f"{'='*70}")
    print(f"  crowding_thresh={crowding_thresh}: {len(covered_divs)}/33 coverage")
    ...
    if delta_38 > 2 and final_energy > 0.05:
        print(f"  -> SIGNIFICANT IMPROVEMENT with preserved generation. Crowding autonomy is the fix.")
    elif delta_38 > 0 and final_energy > 0.05:
        print(f"  -> Marginal improvement, strong generation. Better than suppression tradeoff.")
    elif delta_38 > 0:
        print(f"  -> Coverage improved but generation weakened again.")
    elif delta_38 == 0:
        print(f"  -> No improvement. Coverage/generation tradeoff is fundamental.")
    else:
        print(f"  -> Coverage worsened. Crowding autonomy hurts discovery.")
