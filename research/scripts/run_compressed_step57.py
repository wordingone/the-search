#!/usr/bin/env python3
"""
Step 57: Many-to-few n_matrix=4 sweep.

Same v17 architecture. Only change: n_matrix=4.
Hypothesis: fewer cells -> more concentrated perception -> stronger or weaker generation?
...
def main():
    ...
    kernel = ManyToFewKernel(
        ...
        tau=0.3, k_couple=3,  # k_couple capped at n_matrix-1
        spawn_thresh=0.5, merge_thresh=0.95, lr_codebook=0.015
    )

    print(f"\nStep 57: Many-to-few n_matrix={n_matrix}")
    ...
    print(f"\n{'='*70}")
    print(f"  STEP 57 SUMMARY (n_matrix={n_matrix})")
    print(f"{'='*70}")
    print(f"  Step 55 (n_matrix=8):  33/33, energy=0.081")
    print(f"  Step 57 (n_matrix={n_matrix}):  {cov}/33, energy={final_energy:.6f}")
