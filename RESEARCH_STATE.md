...
| **522** | **K-means graph cross-game transfer (LS20→FT09/VC33)** | — | **FT09 3/3 (degenerate)** | LS20 1/3, FT09 3/3, VC33 0/3. **BUT**: FT09/VC33 frames ALL map to 1/300 centroids — centroid collapse, not transfer. FT09 wins via round-robin (50K/69 actions=725 cycles). VC33 fails despite same collapse (magic pixel timing). **Cross-game transfer NOT confirmed.** Codebook attract update IS load-bearing — keeps centroids in navigation-relevant regions. Frozen k-means can't adapt. Dynamic growth (Step 507/508) is mechanism-dependent, not geometry-dependent. |
