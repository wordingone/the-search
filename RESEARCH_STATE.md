...
| **511** | **Meta-clustering chain centroids (meta-k sweep)** | — | **NMI=0.19** | **Hierarchy adds nothing at threshold=0.3.** 9998 centroids ≈ 1 per image. Meta-k=100 NMI=0.191 ≈ direct k=100 (0.188). Meta-k=200 NMI=0.239 — just more clusters, not hierarchy. The spawn threshold controls centroid count. **Threshold 0.3 → 10K centroids (CIFAR explodes). Need higher threshold for CIFAR classification, but higher threshold breaks ARC navigation (L2 mean=0.5 < threshold).** Chain tension: one threshold can't serve both domains. |
