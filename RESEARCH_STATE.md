...
| **462** | **LLM benchmark (haiku/sonnet/opus on LS20)** | — | — | **0/3 models navigated. 0 levels.** Haiku: 97 steps, 100% dom ACTION1, wrong mental model (ARC reasoning vs navigation). Sonnet: 4 steps, spatial awareness ("blue square toward exit") but cheated (read codebase). Opus: 11 steps on ft09, confused, gave up. **KEY FINDING: LLMs have local continuity + persistence but LACK systematic exploration mechanism.** Argmin-driven exploration (LSH/codebook) covers state space unintelligently but systematically. LLMs exploit by default — reason about "best" action, don't systematically try alternatives. Intelligence without exploration = exploitation. Exploration without intelligence = random walk. The atomic substrate needs both. |
