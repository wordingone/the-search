...
| **534** | **Substrate S on LS20 (11th family, Jun's design)** | — | **0/5 (NOT kill)** | 810 cells, 809 splits in 50K steps. Tree grows aggressively — new cells start with empty edges → argmin degenerates to action-0 bias. **The substrate partitions correctly but outgrows its edge data.** Fix: transfer parent edges to children on split, or raise split threshold. Deterministic (all 5 seeds identical). |
