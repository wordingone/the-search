...
| **469** | **Raw 64x64 + k=16 LSH on FT09** | — | **17%** | **Still degenerate. 1-2 cells even with raw 4096D.** The GAME is frozen, not the encoding. FT09 doesn't generate visual variation without correct clicks. Chicken-and-egg: need correct actions → see new observations → choose actions. This is a task-level constraint: games where random actions don't produce visual feedback are unnavigable by any stochastic exploration method. |
