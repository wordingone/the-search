...
| **516** | **LSH chain CIFAR→LS20→CIFAR** | — | **WIN@1116** | LSH k=12 passes chain. **10x faster than codebook** (WIN@1116 vs WIN@11170). **Action-scope isolation**: LS20 queries n_actions=4, CIFAR edges on actions 4-99 invisible. Zero catastrophic interference. Two independent chain mechanisms: encoding-space separation (codebook) OR action-scope isolation (LSH). |
| **522** | **K-means cross-game transfer (LS20→FT09/VC33)** | — | **FT09 3/3 (degenerate)** | FT09/VC33 frames ALL map to 1/300 centroids — centroid collapse, not transfer. FT09 wins via round-robin. VC33 0/3. **Cross-game transfer NOT confirmed.** Frozen k-means can't adapt. |
