...
| **481** | **Prediction-error action selection** | — | **25%** | **0/10. Smart exploration KILLS navigation.** Same local trap as 478 — agent chases uncertain transitions locally. Every "smart" strategy (destination novelty, prediction error, UCB1, softmax) performs WORSE than argmin. Argmin works via UNIFORM coverage without signal-chasing. The 6/10 = success rate of uniform random walk with local covering. Global coverage without random luck is the open question. |
