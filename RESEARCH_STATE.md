...
| **509** | **CIFAR encoding diagnostic (3 encodings × 2 k-values)** | — | **1%** | **Encoding IS the wall.** within/between-class L2 ratio: avgpool16=1.059x, avgpool8=1.075x, raw32=1.051x. ALL near 1.0 — classes not separable at any fixed pooling resolution. k=300 cluster purity: raw32 best at 66/300 >50% pure (22%). k=100 collapses to chance. Fixed spatial averaging cannot separate 100 fine-grained visual classes. **Classification requires learned representations (SCAN ~50%, ECCV 2020) or R3-compliant encoding discovery.** |
