...
| **513** | **Domain-adaptive threshold via local density** | — | **ARC OK / CIFAR fail** | **Partial success.** ARC auto-calibrates (threshold_median=0.308, matches optimal fixed). CIFAR: 9973 centroids (still ~1/image), threshold_median=0.908. K=10 local density doesn't reflect class structure in sparse 256D — CIFAR images are uniformly far apart. LS20 WIN@32775. 2.6x overhead from pairwise computation. **Finding: domain-adaptive threshold works for dense domains (ARC) but fails for sparse domains (CIFAR). The encoding is the bottleneck, not the threshold mechanism.** |
| **522** | **K-means graph cross-game transfer (LS20→FT09/VC33)** | PENDING | — | Non-codebook chain replication. K-means centroids fitted on LS20, frozen, tested on FT09 (69-action) and VC33 (3-zone). Tests whether codebook's cross-game transfer (Step 508: FT09 reused LS20 space) is universal or mechanism-specific. Predictions: FT09 2/3, VC33 1/3. Kill: FT09 0/3 → transfer is codebook-specific. Hart challenge integrated: "Step 522 is designed to confirm, not learn" → revised from CIFAR→ARC (obvious failure) to LS20→FT09 (genuinely unknown). |
