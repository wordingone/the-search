...
| **453** | **LSH graph (fixed random hyperplanes, k=10)** | — | **25%** | **3/10 at 30K. NAVIGATES.** Steps=[4997, 14737, 18244]. Zero codebook DNA (random hyperplanes, no learning, no attract). 89-354 unique cells (of 1024). Ratio=0.003-0.012. **Faster than cosine graph (median ~15K vs 25K).** Properties 1+2+4 (deterministic, locally continuous, persistent) without property 3 (adaptive density). Cell persistence (P4) is the key — kd-tree failed because splits destroyed edges. LSH cells are fixed and accumulate forever. **U27 revised: adaptive density NOT necessary. Persistent + locally continuous state abstraction suffices.** |
