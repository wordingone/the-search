...
**POMDP reframing and selective π-refinement (Steps 652-674).** L1 is a hidden-state conjunction problem: the agent visits the exit cell avg 152 times before L1 triggers (Step 652). The bottleneck is hash resolution at the exit cell, not action selection (Proposition 15). Step 672 (diagnostic): k=20 separates hidden states that k=12 conflates — slow seeds have 20-43 k=20 sub-cells at the exit cell vs 6.79 average. **Step 674 (transition-triggered dual-hash): 9/10 L1 — best autonomous L1 speed at 25s budget (matches Step 485's 9/10 at 120K via π-refinement rather than budget).** Cells with inconsistent transitions (|successor_set| ≥ 2) get fine k=20 hash; all others use k=12. Seed 8: 192x faster (24235→126). The aliased-cell count determines success: <130 aliased cells → faster, >180 → slower or fails. This is ℓ_π: the observation mapping is refined from the system's own transition statistics, R1-compliantly.
