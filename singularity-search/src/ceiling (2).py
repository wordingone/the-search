...
def run_sequence(order, signals, alphas, n_org=500, n_per_sig=None,
                 n_settle_each=40, n_settle_final=80, base_seed=42, trial=0):
    k = len(order)
    if n_per_sig is None:
        n_per_sig = max(80, 300 // max(k // 3, 1))
...
def run_sequence_trace(order, signals, alphas, n_org=500, n_per_sig=None,
                       n_settle_each=40, base_seed=42, trial=0):
    k = len(order)
    if n_per_sig is None:
        n_per_sig = max(80, 300 // max(k // 3, 1))
    random.seed(base_seed)
