...
| ~~U16~~ → **PENDING** | Must encode differences from expectation | Centering prevents cosine saturation in high-D codebooks. **UNVERIFIED: did Step 453 LSH use centered_enc preprocessing? If yes, this reclassification is wrong.** Awaiting Eli's confirmation. If LSH used raw (uncentered) observations, then centering is cosine-specific. If LSH inherited centered_enc, the constraint may be broader. | Verification mail sent 2026-03-18. Do not treat as resolved until confirmed. |
| ~~U17~~ → S25 | Fixed-capacity memory exhausts exploration | LSH has fixed CELL count (1024) but edge counts grow unboundedly. So total information stored DOES grow — the "fixed capacity" claim applies to cells, not to total memory. The codebook needs unbounded ENTRY growth because its exploration depends on spawning. LSH's exploration depends on edge-count diversity in persistent cells. | LSH Step 454: 1024 fixed cells, 4/10. **Nuance:** edge memory is unbounded. The constraint may be "growth of SOME kind is needed" rather than "growth of entries is needed." |
