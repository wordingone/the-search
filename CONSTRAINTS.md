...
**Game version stability (confirmed 2026-03-21).** ARC Prize API updated LS20 and FT09 game content on March 20 (different sprite definitions, different layouts). VC33 unchanged. Pre-619 and post-619 results on LS20/FT09 are on DIFFERENT games — not directly comparable. The 572u L3=5/5 result (March 20, commit 0463b3c) was valid on the original game version (cb3b57cc) but that game no longer exists on the API. Waypoint-based pipelines (mgu/puq) are game-version-specific and break on update. Step 620 (eigenform self-observation, L1=5/5 on new game) adapts organically — no prescribed waypoints. **Implication: version-agnostic substrates (graph statistics, self-observation) are more robust than version-specific pipelines (hardcoded coordinates). Game updates are a free R3 robustness test.**

**Self-observation signal (Step 620, eigenform, 2026-03-21).** Eigenform self-observation on L1 (new game): substrate reads its own edge count distribution every 500 steps, computes percentile thresholds, assigns AVOID/PREFER/NEUTRAL op codes. L1=5/5 (matches baseline). Op distribution: 94-99% NEUTRAL, 0-2% AVOID, 1-10% PREFER. AVOID grows over time (0% → 8-10% across all seeds) — substrate learns which edges are over-represented. Self-calibration IS working, self-derived, no external targets. Effect minimal on L1 because L1 is argmin-solvable — the real test is L2+ where trap avoidance could matter. Combined with 582 (event-driven ops, SIGNAL 4/5): two independent R3 approaches both show signal. Structure-driven (620) and event-driven (582) self-modification are complementary.

**Honest framing:** Local continuity + persistence explain ALL Level 1 navigation failures. Level 2 failures are explained by the REWARD DISCONNECT — the game topology places L2 beyond the argmin-reachable frontier regardless of mapping architecture, partition granularity, or budget. The mapping properties are necessary for L1 but not sufficient for L2. **Caveat (post March 20): cross-game claims (LS20/FT09/VC33 all solved) need re-verification on new game versions. Only LS20 L1 confirmed on new game (Step 620). VC33 unchanged. FT09 not yet re-tested.**
...
## The State of the Search (612+ experiments, 12 families)
