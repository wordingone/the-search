...
| U10 | Dense memory kills exploration | **RECLASSIFY → S-class** | Recode has 1267 cells and navigates 5/5 (Step 542). Dense state with self-refinement HELPS, not hurts. The original finding was cosine saturation at 4096D, not density per se. LSH with 1605 cells navigates at k=20 (Step 531). Dense state is fine when the mapping preserves local continuity. |
...
| U13 | Additions hurt | **RECLASSIFY → S-class** | SplitTree adds refinement layers and navigates L1 (Step 537). Recode adds learned hyperplanes and navigates 5/5 (Step 542). Additions that ADD INFORMATION (behavioral splits) help. Additions that add PARAMETERS (codebook caps, multi-resolution) hurt. The distinction is informational additions vs parametric additions. Codebook-specific finding. |
