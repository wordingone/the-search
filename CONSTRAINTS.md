...
|---|---|---|---|
| U2 | No separate memory + generation | **RECLASSIFY → S-class** | LSH has separate hash function (read) + edge dict (write) and navigates. Recode has separate LSH base + refinement tree + edge graph. Both have decomposed components. The claim was codebook-specific (process() as single function). |
| U5 | Sparse selection over global aggregation | **RECLASSIFY → S-class** | Hebbian uses GLOBAL aggregation (W.T@x, full dot product across all stored observations) and navigates 5/5 (Step 524). Reservoir failed for rank-1 collapse, not global aggregation. Navigation needs point selection for ACTION (argmin), but the observation→action mapping can be global. |
...
| U18 | Shared action channels contaminate | **UPGRADE → Validated U** | Codebook: Steps 413-413b. LSH chain: Steps 523/533 confirm — CIFAR edges on actions 0-3 contaminate LS20 directional navigation. Per-domain centering (Step 546) mitigates but doesn't eliminate. Cross-family: 2 families (codebook + LSH). |
